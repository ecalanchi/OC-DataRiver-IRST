...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		...
		FILENAME
		<xsl:value-of select="$vStudyName" />
		"~/SAS_DATA.xml";
		FILENAME map "~/SAS_MAP.xml";
		LIBNAME
		<xsl:value-of select="$vStudyName" />
		xml xmlmap=map access=readonly;
		...
		<xsl:for-each
			...
			<xsl:variable name="vFormName">
				<xsl:value-of
					select="substring-before(/odm:ODM/odm:Study/odm:MetaDataVersion/odm:FormDef/odm:ItemGroupRef[@ItemGroupOID=$vitemgrouprefOID]/../@Name,' -')" />
			</xsl:variable>
			<!-- KK variable -->
	...
	<xsl:template name="processtable">
		...
		<xsl:for-each
			...
			<xsl:for-each
				select="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:ItemDef[@OID=$vitemOID]/odm:CodeListRef">
				data
				...
				;
				format
				<xsl:call-template name="get_item_oid">
