...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:odm="http://www.cdisc.org/ns/odm/v1.3" xmlns:OpenClinica="http://www.openclinica.org/ns/odm_ext_v130/v3.1"
	...
	<xsl:output encoding="utf-8" indent="no" method="text" name="plain"
		omit-xml-declaration="yes" />
	<!-- Suppress unmatched nodes. -->
	<xsl:template match="text()" />
	<!-- Get the parent study oid, which is listed first. -->
	<xsl:variable name="study_oid"
		select="substring(concat('S',substring(//odm:Study[position()=1]/@OID, 3)),1,8)" />
	...
	<!-- Catch-all template that feeds 3 result documents. HTML-encoded characters 
		in use: &#xa; (newline), &#9; (tab), &quot; (double quote), &lt; (less than), 
		&gt; (greater than). -->
	<xsl:template match="/*">
	...
	<xsl:template match="odm:ItemGroupDef" mode="map">
		<xsl:variable name="curatedOID">
			<xsl:variable name="formdef"
				select="key('form-name', OpenClinica:ItemGroupDetails/OpenClinica:PresentInForm[1]/@FormOID)" />
			...
			<xsl:if
				test="string-length(@OID) &lt; 36 and not(contains(@OID, 'UNGROUPED'))">
				<xsl:value-of select="concat('_',substring($noprefixoid,1,31))" />
			</xsl:if>
			<xsl:if test="contains(@OID, 'UNGROUPED')">
				<xsl:value-of
					select="replace($formdef/OpenClinica:FormDetails/@ParentFormOID, 'F_', '_')" />
			</xsl:if>
		</xsl:variable>
	...
	<xsl:template match="odm:ItemRef" mode="map">
		...
		<xsl:element name="COLUMN">
			<xsl:attribute name="Name">
                <xsl:variable name="curatedItemOID"
				select="replace($itemdef/@OID, 'I_[A-Z]*_', '')" />
                <xsl:variable name="noprefixTokenizedItemOid"
				select="tokenize($curatedItemOID,'_')" />
			...
			<xsl:element name="PATH">
				<xsl:value-of
					select="concat('/', $study_oid, '/', $item_group_oid, '/', @ItemOID)" />
			</xsl:element>
