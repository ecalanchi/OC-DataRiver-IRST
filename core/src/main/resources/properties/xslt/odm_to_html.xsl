...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/">
		<html>
			<script language="JavaScript1.2">
				...
				newwin =
				window.open('', 'windowname1', 'width=900',
				'height=500','scrollbars=1');
				var divTxt =
			...
			<body>
				...
				<div class="tablebox_center" align="left">
					<table border="1" cellpadding="0" cellspacing="0">
						<!-- Fix for 0007973 -->
						<!-- <tr valign="top"> <td class="table_header_column_top">Database 
							Export Header Metadata</td> <td class="table_cell_top"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
							</td> <td class="table_cell_top"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
							</td> </tr> -->
						<tr>
						...
						<xsl:apply-templates select="//odm:ODM/odm:Study[1]/odm:MetaDataVersion"
							mode="metadataDisplay" />
						<!-- placeholder 1 studyeventdata -->
						<!--<xsl:apply-templates select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData[generate-id() 
							= generate-id(key('studyEvents',@StudyEventOID)[1])]" mode="studyEventData1" 
							/> -->
				...
				<!--<xsl:for-each select="$tokenizedcrfAndDataItemsHeaders"> token<xsl:value-of 
				...
				<div class="tablebox_center" align="center">
					<table border="1" cellpadding="0" cellspacing="0">
						<tr valign="top">
							...
							<!-- place holder 2 -->
							<!-- <xsl:apply-templates select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData[generate-id() 
								= generate-id(key('studyEvents',@StudyEventOID)[1])]" mode="studyeventData2" 
								/> -->
							<!-- Selecting Event CRF column headers -->
							<!-- <xsl:apply-templates select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData" 
								mode="studyEventInfo" /> -->
							<xsl:apply-templates
								select="/odm:ODM/odm:Study[1]/odm:MetaDataVersion/odm:StudyEventDef"
								mode="studyEventInfoHeaders" />
							...
							<xsl:apply-templates
								select="/odm:ODM/odm:Study[1]/odm:MetaDataVersion/odm:StudyEventDef"
								mode="studyFormAndDataItemsHeaders">
								<xsl:with-param name="generateIntHeadersList"
									select="'No'" />
							</xsl:apply-templates>
							<!-- place holder 3 -->
							<!-- Item Headers -->
						...
						<xsl:apply-templates select="/odm:ODM/odm:ClinicalData/odm:SubjectData"
							mode="allSubjectData">
							<xsl:with-param name="tokenizedEventHeaders"
								select="$tokenizedEventHeaders" />
				...
				<br />
				<!-- place holder 6 -->
				<xsl:apply-templates select="/odm:ODM/odm:ClinicalData"
					mode="clinicalMetadata">
	...
	<!-- <xsl:template match="/odm:ODM/odm:ClinicalData/odm:SubjectData" mode="allSubjectData"> 
		<tr valign="top"> <td class="table_cell_left"> <xsl:value-of select="@OpenClinica:StudySubjectID"></xsl:value-of> 
		</td> <xsl:if test="$uniqueIdExist"> <td class="table_cell"> <xsl:choose> 
		<xsl:when test="@OpenClinica:UniqueIdentifier"> <xsl:value-of select="@OpenClinica:UniqueIdentifier"></xsl:value-of> 
		</xsl:when> <xsl:otherwise> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
		</xsl:otherwise> </xsl:choose> </td> </xsl:if> <xsl:if test="$subjectStatusExist"> 
		<td class="table_cell"> <xsl:choose> <xsl:when test="@OpenClinica:Status"> 
		<xsl:value-of select="@OpenClinica:Status"></xsl:value-of> </xsl:when> <xsl:otherwise> 
		<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> </xsl:otherwise> 
		</xsl:choose> </td> </xsl:if> <xsl:if test="$sexExist"> <td class="table_cell"> 
		<xsl:choose> <xsl:when test="@OpenClinica:Sex"> <xsl:value-of select="@OpenClinica:Sex"></xsl:value-of> 
		</xsl:when> <xsl:otherwise> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
		</xsl:otherwise> </xsl:choose> </td> </xsl:if> <xsl:if test="$dobExist"> 
		<td class="table_cell"> <xsl:choose> <xsl:when test="@OpenClinica:DateOfBirth"> 
		<xsl:value-of select="@OpenClinica:DateOfBirth"></xsl:value-of> </xsl:when> 
		<xsl:otherwise> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
		</xsl:otherwise> </xsl:choose> </td> </xsl:if> <xsl:variable name="subjectEvents" 
		select="./odm:StudyEventData" /> <xsl:variable name="subjectItems" select="./odm:StudyEventData/odm:FormData/odm:ItemGroupData/odm:ItemData" 
		/> <xsl:for-each select="$allStudyEventDataElements"> <xsl:variable name="eventOIDLoop" 
		select="@StudyEventOID"/> <xsl:variable name="eventRepeatKeyLoop" select="@StudyEventRepeatKey"/> 
		<xsl:choose> <xsl:when test="$eventRepeatKeyLoop"> <xsl:variable name="subjectEventDataElementInOrder" 
		select="$subjectEvents[$eventOIDLoop= @StudyEventOID and $eventRepeatKeyLoop 
		= @StudyEventRepeatKey]" /> <xsl:if test="$subjectEventDataElementInOrder"> 
		<xsl:apply-templates mode="studyEventInfoData2" select="$subjectEventDataElementInOrder"> 
		<xsl:with-param name="subjectForms" select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData"/> 
		<xsl:with-param name="subjectKey" select="@SubjectKey" /> <xsl:with-param 
		name="subjectEvents" select="$subjectEvents" /> <xsl:with-param name="subjectItems" 
		select="$subjectItems"/> </xsl:apply-templates> </xsl:if> </xsl:when> <xsl:otherwise> 
		<xsl:variable name="subjectEventDataElementInOrder" select="$subjectEvents[$eventOIDLoop= 
		@StudyEventOID]" /> <xsl:if test="$subjectEventDataElementInOrder"> <xsl:apply-templates 
		mode="studyEventInfoData2" select="odm:StudyEventData"> <xsl:with-param name="subjectForms" 
		select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData"/> 
		<xsl:with-param name="subjectKey" select="@SubjectKey" /> <xsl:with-param 
		name="subjectEvents" select="$subjectEvents" /> <xsl:with-param name="subjectItems" 
		select="$subjectItems"/> </xsl:apply-templates> </xsl:if> </xsl:otherwise> 
		</xsl:choose> </xsl:for-each> <xsl:apply-templates select="odm:StudyEventData/odm:FormData" 
		mode="eventCRFData"> <xsl:with-param name="subjectForms" select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData"/> 
		<xsl:with-param name="subjectEvents" select="$subjectEvents"/> <xsl:with-param 
		name="subjectItems" select="$subjectItems"/> </xsl:apply-templates> </tr> 
		</xsl:template> -->
	<xsl:template match="/odm:ODM/odm:ClinicalData/odm:SubjectData"
		...
		<tr valign="top">
			...
			<!--<xsl:for-each select="$tokenizedEventHeaders"> {token<xsl:value-of 
				select="position()"/>: <xsl:value-of select="."/>} <xsl:text>! </xsl:text> 
				</xsl:for-each> -->
			<!-- <xsl:apply-templates select="odm:StudyEventData/odm:FormData" mode="eventCRFData"> 
				<xsl:with-param name="subjectForms" select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData"/> 
				<xsl:with-param name="subjectEvents" select="$subjectEvents"/> <xsl:with-param 
				name="subjectItems" select="$subjectItems"/> </xsl:apply-templates> -->
	...
	<xsl:template mode="eventCRFData2"
		...
		<xsl:variable name="subjectEvent" select="$subjectFormData/.." />
		<xsl:variable name="parentEvent" select=".." />
		<xsl:apply-templates mode="allEventCrfData2"
			...
			<xsl:choose>
				<xsl:when test="@StudyEventRepeatKey">
					...
					<!-- <xsl:for-each select="exsl:node-set($allStudyEvents)/odm:StudyEventData"> 
						<xsl:choose> <xsl:when test="position()=1"> <xsl:choose> <xsl:when test="$subjectFormData/node() 
						and $subjectEvent/@StudyEventOID=@StudyEventOID and $subjectEvent/@StudyEventRepeatKey=@StudyEventRepeatKey"> 
						<xsl:if test="$interviewerNameExist"> <td class="table_cell"> <xsl:value-of 
						select="$currentForm/@OpenClinica:InterviewerName"/> </td> </xsl:if> <xsl:if 
						test="$interviewDateExist"> <td class="table_cell"> <xsl:value-of select="$currentForm/@OpenClinica:InterviewDate"/> 
						</td> </xsl:if> <xsl:if test="$crfStatusExist"> <td class="table_cell"> <xsl:value-of 
						select="$currentForm/@OpenClinica:Status"/> </td> </xsl:if> <xsl:if test="$crfVersionExist"> 
						<td class="table_cell"> <xsl:value-of select="$currentForm/@OpenClinica:Version"/> 
						</td> </xsl:if> </xsl:when> <xsl:otherwise> <xsl:if test="$eventLocationExist"> 
						<td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
						</td> </xsl:if> <xsl:if test="$eventStartDateExist"> <td class="table_cell"> 
						<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> </td> </xsl:if> 
						<xsl:if test="$eventEndDateExist"> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
						</td> </xsl:if> <xsl:if test="$eventStatusExist"> <td class="table_cell"> 
						<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> </td> </xsl:if> 
						<xsl:if test="$ageExist"> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
						</td> </xsl:if> </xsl:otherwise> </xsl:choose> </xsl:when> <xsl:otherwise> 
						<xsl:if test="preceding-sibling::odm:StudyEventData[1]/@StudyEventRepeatKey 
						!= @StudyEventRepeatKey"> <xsl:choose> <xsl:when test="$subjectFormData/node() 
						and $subjectEvent/@StudyEventOID=@StudyEventOID and $subjectEvent/@StudyEventRepeatKey=@StudyEventRepeatKey"> 
						<xsl:if test="$interviewerNameExist"> <td class="table_cell"> <xsl:value-of 
						select="$currentForm/@OpenClinica:InterviewerName"/> </td> </xsl:if> <xsl:if 
						test="$interviewDateExist"> <td class="table_cell"> <xsl:value-of select="$currentForm/@OpenClinica:InterviewDate"/> 
						...
						<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> </td> </xsl:if> 
						<xsl:if test="$ageExist"> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
						</td> </xsl:if> </xsl:otherwise> </xsl:choose> </xsl:if> </xsl:otherwise> 
						</xsl:choose> </xsl:for-each> -->
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="$subjectFormData/node()">
							<!-- <xsl:if test="$eventLocationExist"> <xsl:value-of select="@OpenClinica:StudyEventLocation"></xsl:value-of> 
								<xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if test="$eventStartDateExist"> 
								<xsl:value-of select="@OpenClinica:StartDate"></xsl:value-of> <xsl:value-of 
								select="$delimiter" /> </xsl:if> <xsl:if test="$eventEndDateExist"> <xsl:value-of 
								select="@OpenClinica:EndDate"></xsl:value-of> <xsl:value-of select="$delimiter" 
								/> </xsl:if> <xsl:if test="$eventStatusExist"> <xsl:value-of select="@OpenClinica:Status"></xsl:value-of> 
								<xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if test="$ageExist"> 
								<xsl:value-of select="@OpenClinica:SubjectAgeAtEvent"></xsl:value-of> <xsl:value-of 
								select="$delimiter" /> </xsl:if> -->
						...
						<xsl:otherwise>
							<!-- <xsl:if test="$eventLocationExist"> <xsl:value-of select="$delimiter" 
								/> </xsl:if> <xsl:if test="$eventStartDateExist"> <xsl:value-of select="$delimiter" 
								/> </xsl:if> <xsl:if test="$eventEndDateExist"> <xsl:value-of select="$delimiter" 
								/> </xsl:if> <xsl:if test="$eventStatusExist"> <xsl:value-of select="$delimiter" 
								/> </xsl:if> <xsl:if test="$ageExist"> <xsl:value-of select="$delimiter" 
								/> </xsl:if> -->
	...
	<!-- **************** -->
	<xsl:template mode="allEventCrfData2"
		match="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData">
		<xsl:param name="subjectForms" />
		<xsl:param name="currentForm" />
		<xsl:param name="subjectEvent" />
	...
	<xsl:template mode="studyEventInfoData"
		match="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData">
		<xsl:param name="subjectForms" />
		<xsl:param name="subjectEvents" />
		<xsl:param name="subjectItems" />
		...
		<!--<xsl:apply-templates select="odm:FormData" mode="eventCRFData2"> <xsl:with-param 
			name="subjectForms" select="$subjectForms"/> <xsl:with-param name="subjectEvents" 
			select="$subjectEvents"/> <xsl:with-param name="subjectItems" select="$subjectItems"/> 
			</xsl:apply-templates> -->
	</xsl:template>
	<xsl:template name="studyEventInfoData2">
		<!--<xsl:param name="subjectForms"/> <xsl:param name="subjectItems" /> -->
		<xsl:param name="subjectEvents" />
		<xsl:param name="tokenizedEventHeaders" />
		<!-- -subjectEvents: <xsl:for-each select="$subjectEvents"> <xsl:variable 
			name="eventOID" select="@StudyEventOID" />eventOID:<xsl:value-of select="$eventOID"/> 
			<xsl:variable name="eventPosition"> <xsl:for-each select="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef"> 
			<xsl:if test="@OID = $eventOID"> <xsl:copy-of select="position()" /> </xsl:if> 
			</xsl:for-each> </xsl:variable>eventPosition:<xsl:value-of select="$eventPosition"/> 
			StudyEventRepeatKey:<xsl:value-of select="@StudyEventRepeatKey"/> </xsl:for-each> -->
		<xsl:for-each select="$tokenizedEventHeaders">
			...
			<xsl:if test=". != $tokenizedEventHeaders[last()]">
				...
				<!--<xsl:variable name="colEventPosition"> <xsl:choose> <xsl:when test="contains(., 
					'_')"> <xsl:value-of select="substring-before(.,'_')"/> </xsl:when> <xsl:otherwise> 
					<xsl:value-of select="."/> </xsl:otherwise> </xsl:choose> </xsl:variable> -->
				...
				<!--<xsl:choose> <xsl:when test="$colEventPosition = $eventPosition">even 
					pos matched <xsl:choose> <xsl:when test="$isColForRepeatingEvent"> repeating 
					event column <xsl:choose> <xsl:when test="@StudyEventRepeatKey = $colRepeatEventKey"> 
					writing data for repeatKey: <xsl:value-of select="@StudyEventRepeatKey"/> 
					<xsl:if test="$colType = 'Location'"> <xsl:choose> <xsl:when test="@OpenClinica:StudyEventLocation"> 
					<td class="table_cell"> <xsl:value-of select="@OpenClinica:StudyEventLocation"></xsl:value-of> 
					</td> </xsl:when> <xsl:otherwise> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:otherwise> </xsl:choose> </xsl:if> <xsl:if test="$colType = 'StartDate'"> 
					<xsl:choose> <xsl:when test="@OpenClinica:StartDate"> <td class="table_cell"> 
					<xsl:value-of select="@OpenClinica:StartDate"></xsl:value-of> </td> </xsl:when> 
					<xsl:otherwise> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:otherwise> </xsl:choose> </xsl:if> <xsl:if test="$colType = 'EndDate'"> 
					<xsl:choose> <xsl:when test="@OpenClinica:EndDate"> <td class="table_cell"> 
					<xsl:value-of select="@OpenClinica:EndDate"></xsl:value-of> </td> </xsl:when> 
					<xsl:otherwise> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:otherwise> </xsl:choose> </xsl:if> <xsl:if test="$colType = 'Event 
					Status'"> <xsl:choose> <xsl:when test="@OpenClinica:Status"> <td class="table_cell"> 
					<xsl:value-of select="@OpenClinica:Status"></xsl:value-of> </td> </xsl:when> 
					<xsl:otherwise> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:otherwise> </xsl:choose> </xsl:if> <xsl:if test="$colType = 'Age'"> 
					<xsl:choose> <xsl:when test="@OpenClinica:SubjectAgeAtEvent"> <td class="table_cell"> 
					<xsl:value-of select="@OpenClinica:SubjectAgeAtEvent"></xsl:value-of> </td> 
					</xsl:when> <xsl:otherwise> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:otherwise> </xsl:choose> </xsl:if> </xsl:when> <xsl:otherwise>even 
					repeat key not matched - no data <xsl:if test="$colType = 'Location'"> <td 
					class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:if> <xsl:if test="$colType = 'StartDate'"> <td class="table_cell"> 
					<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> </td> </xsl:if> 
					<xsl:if test="$colType = 'EndDate'"> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:if> <xsl:if test="$colType = 'Event Status'"> <td class="table_cell"> 
					<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> </td> </xsl:if> 
					<xsl:if test="$colType = 'Age'"> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:if> </xsl:otherwise> </xsl:choose> </xsl:when> <xsl:otherwise> 
					<xsl:if test="$colType = 'Location'"> <xsl:choose> <xsl:when test="@OpenClinica:StudyEventLocation"> 
					<td class="table_cell"> <xsl:value-of select="@OpenClinica:StudyEventLocation"></xsl:value-of> 
					</td> </xsl:when> <xsl:otherwise> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					...
					</td> </xsl:otherwise> </xsl:choose> </xsl:if> </xsl:otherwise> </xsl:choose> 
					</xsl:when> <xsl:otherwise>even pos not matched - no data <xsl:if test="$colType 
					= 'Location'"> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:if> <xsl:if test="$colType = 'StartDate'"> <td class="table_cell"> 
					<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> </td> </xsl:if> 
					<xsl:if test="$colType = 'EndDate'"> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:if> <xsl:if test="$colType = 'Event Status'"> <td class="table_cell"> 
					<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> </td> </xsl:if> 
					<xsl:if test="$colType = 'Age'"> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
					</td> </xsl:if> </xsl:otherwise> </xsl:choose> -->
			</xsl:if>
		</xsl:for-each>
	</xsl:template>
										...
										<xsl:choose>
												...
												<xsl:choose>
													<xsl:when test="$colRepeatEventKey = $eventRepeatKey">
														...
														<xsl:for-each select="./odm:FormData">
															...
															<xsl:choose>
																<xsl:when
																	test="$crfPosition = normalize-space($colCrfPosition)">
																	...
																	<xsl:for-each select="./odm:ItemGroupData">
																		...
																		<xsl:choose>
																			<xsl:when test="$isColForRepeatingGrp">
																				<xsl:choose>
																					<xsl:when
																						test="$grpRepeatKey = normalize-space($colRepeatGrpKey)">
											...
											<xsl:otherwise>
												...
												<xsl:for-each select="./odm:FormData">
													...
													<!--formOID:<xsl:value-of select="$formOID"/> -->
													<!-- find crf position -->
													<!--<xsl:variable name="matchingEventDef" select="$allEventDefs[@OID 
														= $eventOID]"/> <xsl:variable name="matchingCRFRef" select="$matchingEventDef/odm:FormRef[@FormOID 
														= $formOID]"/> -->
													<xsl:variable name="matchingCRFRef"
														select="$allEventDefs/odm:FormRef[@FormOID = $formOID and ../@OID = $eventOID]" />
													<xsl:variable name="formRefNodeId"
						...
						<xsl:choose>
							<xsl:when test="contains($ifMatch, $matchSep)">
								<!-- <xsl:variable name="ifMatchTokenized" select="tokenize($ifMatch,'_')"/> 
									<xsl:variable name="eventOID" select="$ifMatchTokenized[2]" /> <xsl:variable 
									name="formOID" select="$ifMatchTokenized[3]" /> <xsl:variable name="grpOID" 
									select="$ifMatchTokenized[4]" /> <xsl:variable name="itemOID" select="$ifMatchTokenized[5]" 
									/> <xsl:variable name="eventRepeatKey"> <xsl:for-each select="$ifMatchTokenized"> 
									<xsl:if test="contains(.,'E')"> <xsl:value-of select="substring-after(.,'E')"/> 
									</xsl:if> </xsl:for-each> </xsl:variable> <xsl:variable name="grpRepeatKey"> 
									<xsl:for-each select="$ifMatchTokenized"> <xsl:if test="contains(.,'G')"> 
									<xsl:value-of select="substring-after(.,'G')"/> </xsl:if> </xsl:for-each> 
									</xsl:variable> -->
								...
								<!--<xsl:variable name="itemData" select="$subjectEvents/odm:FormData/odm:ItemGroupData/odm:ItemData 
									[@ItemOID = $itemOID and ../@ItemGroupOID=$grpOID and ../@ItemGroupRepeatKey 
									= $grpRepeatKey and ../../@FormOID = $formOID and ../../../@StudyEventOID 
									= $eventOID and ../../../@StudyEventRepeatKey = $eventRepeatKey]"/> -->
								<!-- write data -->
								<xsl:choose>
							...
							<xsl:otherwise>
								<!--<xsl:if test="$colType = 'Interviewer'"> <td class="table_cell"> 
									<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> </td> </xsl:if> 
									<xsl:if test="$colType = 'InterviewDate'"> <td class="table_cell"> <xsl:text 
									disable-output-escaping="yes">&amp;nbsp;</xsl:text> </td> </xsl:if> <xsl:if 
									test="$colType = 'CRF Version Status'"> <td class="table_cell"> <xsl:text 
									disable-output-escaping="yes">&amp;nbsp;</xsl:text> </td> </xsl:if> <xsl:if 
									test="$colType = 'Version Name'"> <td class="table_cell"> <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text> 
									</td> </xsl:if> -->
								<td class="table_cell">
									<xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
								</td>
												...
												<xsl:when test="$crfPosition = normalize-space($colCrfPosition)">
													...
													<xsl:choose>
														<xsl:when test="$isColForRepeatingEvent">
															...
															<xsl:choose>
																<xsl:when
																	test="../@StudyEventRepeatKey = normalize-space($colRepeatEventKey)">
	...
	<xsl:template
		...
		<!--<xsl:variable name="eventPosition"> <xsl:for-each select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData[generate-id() 
			= generate-id(key('studyEvents',@StudyEventOID)[1])]"> <xsl:copy-of select="position()" 
		...
		<xsl:variable name="currentNodePositon">
			<xsl:for-each select="$allStudyEventDataElements">
				<xsl:choose>
					<xsl:when test="$studyEventRepeatKey">
						<xsl:if
							test="@StudyEventOID = $StudyEventDataOID and @StudyEventRepeatKey = $studyEventRepeatKey">
		...
		<xsl:variable name="priorStudyElementDataElements"
			select="$allStudyEventDataElements[position() &lt; $currentNodePositon]" />
		...
		<xsl:choose>
			<xsl:when test="$studyEventRepeatKey">
				<!--studyEventRepeatKey<xsl:value-of select="$studyEventRepeatKey"/> 
					prior match cnt: <xsl:value-of select="count($priorStudyElementDataElements[@StudyEventOID 
					= $StudyEventDataOID and @StudyEventRepeatKey = $studyEventRepeatKey]) "/> -->
				<xsl:choose>
					<xsl:when
						test="count($priorStudyElementDataElements[@StudyEventOID = $StudyEventDataOID and @StudyEventRepeatKey = $studyEventRepeatKey]) &gt; 0">
					</xsl:when>
					<xsl:otherwise>
						<!--create column -->
						<xsl:if test="$eventLocationExist">
		...
		<!-- <xsl:if test="$eventLocationExist"> <td class="table_header_row"> 
			<xsl:text>Location_</xsl:text> <xsl:value-of select="$E" /> <xsl:value-of 
			select="$eventPosition" /> <xsl:if test="@StudyEventRepeatKey"> <xsl:text>_</xsl:text> 
			<xsl:value-of select="@StudyEventRepeatKey" /> </xsl:if> </td> </xsl:if> 
			<xsl:if test="$eventStartDateExist"> <td class="table_header_row"> <xsl:text>StartDate_</xsl:text> 
			<xsl:value-of select="$E" /> <xsl:value-of select="$eventPosition" /> <xsl:if 
			test="@StudyEventRepeatKey"> <xsl:text>_</xsl:text> <xsl:value-of select="@StudyEventRepeatKey" 
			/> </xsl:if> <xsl:text>_</xsl:text> </td> </xsl:if> <xsl:if test="$eventEndDateExist"> 
			<td class="table_header_row"> <xsl:text>EndDate_</xsl:text> <xsl:value-of 
			select="$E" /> <xsl:value-of select="$eventPosition" /> <xsl:if test="@StudyEventRepeatKey"> 
			<xsl:text>_</xsl:text> <xsl:value-of select="@StudyEventRepeatKey" /> </xsl:if> 
			<xsl:text>_</xsl:text> </td> </xsl:if> <xsl:if test="$eventStatusExist"> 
			<td class="table_header_row"> <xsl:text>Event Status_</xsl:text> <xsl:value-of 
			select="$E" /> <xsl:value-of select="$eventPosition" /> <xsl:if test="@StudyEventRepeatKey"> 
			<xsl:text>_</xsl:text> <xsl:value-of select="@StudyEventRepeatKey" /> </xsl:if> 
			<xsl:text>_</xsl:text> </td> </xsl:if> <xsl:if test="$ageExist"> <td class="table_header_row"> 
			<xsl:text>Age_</xsl:text> <xsl:value-of select="$E" /> <xsl:value-of select="$eventPosition" 
			/> <xsl:if test="@StudyEventRepeatKey"> <xsl:text>_</xsl:text> <xsl:value-of 
			select="@StudyEventRepeatKey" /> </xsl:if> <xsl:text>_</xsl:text> </td> </xsl:if> -->
		<!-- <xsl:apply-templates select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData" 
			mode="studyEventHeader" /> -->
	</xsl:template>
	<xsl:template mode="studyEventInfoHeaders"
		...
		<xsl:variable name="allStudyEventDataElements"
			select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData[@StudyEventOID = $eventDefOID]" />
		<!--<xsl:variable name="eventLocationExist" select="count($allStudyEventDataElements[@OpenClinica:StudyEventLocation]) 
			&gt; 0"/> <xsl:variable name="eventStartDateExist" select="count($allStudyEventDataElements[@OpenClinica:StartDate]) 
			&gt; 0"/> <xsl:variable name="eventStatusExist" select="count($allStudyEventDataElements[@OpenClinica:Status]) 
			&gt; 0"/> <xsl:variable name="eventEndDateExist" select="count($allStudyEventDataElements[@OpenClinica:EndDate]) 
			&gt; 0"/> <xsl:variable name="ageExist" select="count($allStudyEventDataElements[@OpenClinica:SubjectAgeAtEvent]) 
			&gt; 0"/> -->
			...
			<xsl:when test="$isRepeating = 'Yes'">
				...
				<xsl:apply-templates select="."
					...
					<!--<xsl:with-param name="eventLocationExist" select="$eventLocationExist"/> 
						...
						<xsl:with-param name="eventStatusExist" select="$eventStatusExist"/> <xsl:with-param 
						name="eventEndDateExist" select="$eventEndDateExist"/> <xsl:with-param name="ageExist" 
						select="$ageExist"/> -->
				</xsl:apply-templates>
			</xsl:when>
			<xsl:otherwise>
				...
				<xsl:apply-templates select="."
					mode="createColForNonRepeatingEvent">
					<xsl:with-param name="eventPosition" select="$eventPosition" />
					<!--<xsl:with-param name="eventLocationExist" select="$eventLocationExist"/> 
						<xsl:with-param name="eventStartDateExist" select="$eventStartDateExist"/> 
						<xsl:with-param name="eventStatusExist" select="$eventStatusExist"/> <xsl:with-param 
	...
	<xsl:template name="createColForRepeatingEvent"
		match="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef" mode="createColForRepeatingEvent">
		<xsl:param name="eventOID" />
		<xsl:param name="eventPosition" />
		<xsl:param name="MaxEventRepeatKey" />
		<!--<xsl:param name="eventLocationExist"/> <xsl:param name="eventStartDateExist"/> 
			<xsl:param name="eventStatusExist"/> <xsl:param name="eventEndDateExist"/> 
			<xsl:param name="ageExist"/> -->
		<xsl:param name="eventRepeatCnt" /><!--createColForRepeatingEvent, 
			eventOID<xsl:value-of select="$eventOID"/> -->
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey = $eventRepeatCnt and @OpenClinica:StudyEventLocation]) &gt; 0">
			<!--<xsl:if test="$eventLocationExist"> -->
			<td class="table_header_row">
				<xsl:text>Location_</xsl:text>
				<xsl:value-of select="$E" />
		...
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey = $eventRepeatCnt and @OpenClinica:StartDate]) &gt; 0"><!--col for event startdate -->
			<!--<xsl:if test="$eventStartDateExist"> -->
			<td class="table_header_row">
				<xsl:text>StartDate_</xsl:text>
				<xsl:value-of select="$E" />
		...
		<!--<xsl:if test="$eventEndDateExist"> -->
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey = $eventRepeatCnt and @OpenClinica:EndDate]) &gt; 0">
			<td class="table_header_row">
				<xsl:text>EndDate_</xsl:text>
				<xsl:value-of select="$E" />
				<xsl:value-of select="$eventPosition" />
		...
		<!--<xsl:if test="$eventStatusExist"> -->
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey = $eventRepeatCnt and @OpenClinica:Status]) &gt; 0">
			<td class="table_header_row">
				<xsl:text>Event Status_</xsl:text>
				<xsl:value-of select="$E" />
				<xsl:value-of select="$eventPosition" />
		...
		<!--<xsl:if test="$ageExist"> -->
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey = $eventRepeatCnt and @OpenClinica:SubjectAgeAtEvent]) &gt; 0">
			<td class="table_header_row">
				<xsl:text>Age_</xsl:text>
				<xsl:value-of select="$E" />
				<xsl:value-of select="$eventPosition" />
	...
	<xsl:template mode="createColForNonRepeatingEvent"
		match="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef">
		<xsl:param name="eventPosition" />
		<xsl:param name="eventOID" />
		...
		<!--<xsl:if test="$eventLocationExist"> -->
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @OpenClinica:StudyEventLocation]) &gt; 0">
			<td class="table_header_row">
				<xsl:text>Location_</xsl:text>
				<xsl:value-of select="$E" />
				<xsl:value-of select="$eventPosition" />
		...
		<!--<xsl:if test="$eventStartDateExist"> -->
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @OpenClinica:StartDate]) &gt; 0">
			<td class="table_header_row">
				<xsl:text>StartDate_</xsl:text>
				<xsl:value-of select="$E" />
				<xsl:value-of select="$eventPosition" />
		...
		<!--<xsl:if test="$eventEndDateExist"> -->
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @OpenClinica:EndDate]) &gt; 0">
			<td class="table_header_row">
				<xsl:text>EndDate_</xsl:text>
				<xsl:value-of select="$E" />
				<xsl:value-of select="$eventPosition" />
		...
		<!--<xsl:if test="$eventStatusExist"> -->
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @OpenClinica:Status]) &gt; 0">
			<td class="table_header_row">
				<xsl:text>Event Status_</xsl:text>
				<xsl:value-of select="$E" />
				<xsl:value-of select="$eventPosition" />
		...
		<!--<xsl:if test="$ageExist"> -->
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @OpenClinica:SubjectAgeAtEvent]) &gt; 0">
			<td class="table_header_row">
				<xsl:text>Age_</xsl:text>
				<xsl:value-of select="$E" />
				<xsl:value-of select="$eventPosition" />
	...
	<xsl:template match="/odm:ODM/odm:ClinicalData" mode="clinicalMetadata">
		<xsl:apply-templates select="odm:SubjectData"
			mode="SubjectMetaData" />
	</xsl:template>
	<xsl:template match="/odm:ODM/odm:ClinicalData/odm:SubjectData"
		mode="SubjectMetaData">
		<xsl:apply-templates select="odm:StudyEventData"
			mode="studyEventMetadata" />
	</xsl:template>
	<xsl:template
		match="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData"
		mode="studyEventMetadata">
	...
	<xsl:template
		match="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData"
		mode="FormMetaData">
		<xsl:apply-templates select="odm:ItemGroupData"
			mode="itemGroupMetadata" />
	</xsl:template>
	<xsl:template
		match="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData/odm:ItemGroupData"
		mode="itemGroupMetadata">
		<xsl:apply-templates select="odm:ItemData" mode="itemDataMetadata" />
	</xsl:template>
	<xsl:template
		match="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData/odm:ItemGroupData/odm:ItemData"
		mode="itemDataMetadata">
						...
						<span class="expandFormLink">
							<a>
								...
								<xsl:attribute name="onclick">
                                                                    showDiv('<xsl:value-of
									select="$formOID" />');return
                                                                    false;
                                                                </xsl:attribute>
								<xsl:value-of select="@Name" />
							</a>
	...
	<xsl:template match="//odm:ODM/odm:Study/odm:MetaDataVersion"
		mode="metadataDisplay">
		<xsl:apply-templates select="odm:StudyEventDef"
			...
			<!--<xsl:apply-templates select="./odm:FormRef" mode="formRefToDefMapTemplate"> 
				<xsl:with-param name="crfRefPosition" select="position()"/> </xsl:apply-templates> -->
			<!--</xsl:for-each> -->
		</xsl:apply-templates>
	</xsl:template>
	<!-- <xsl:template match="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData" 
		mode="studyEventData1"> <xsl:variable name="eventPosition" select="position()" 
		/> <xsl:variable name="eventOID" select="@StudyEventOID" /> <xsl:apply-templates 
		select="//odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef[@OID=$eventOID]" 
		mode="studyeventDef1"> <xsl:with-param name="eventPosition" select="$eventPosition" 
		/> <xsl:with-param name="eventOID" select="$eventOID" /> </xsl:apply-templates> 
		<xsl:apply-templates select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData[generate-id() 
		= generate-id(key('eventCRFs',@FormOID)[1])]" mode="formData1"> <xsl:with-param 
		name="eventPosition" select="$eventPosition" /> <xsl:with-param name="eventOID" 
		select="$eventOID" /> </xsl:apply-templates> </xsl:template> -->
	<xsl:template
		match="//odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef"
		mode="studyEventDefinition">
	...
	<xsl:template mode="studyEventDefList"
		...
		<!-- moved the template call to generate list of CRFs within an event here 
			to cover the scenarion correctly where same CRF is included in more than 
			one events -->
		<!--<xsl:variable name="studyEventDef" select="//odm:ODM/odm:Study[1]/odm:MetaDataVersion/odm:StudyEventDef[@OID 
			= $oid]"/> <xsl:apply-templates select="$studyEventDef/odm:FormRef" mode="formRefToDefMapTemplate"> 
			<xsl:with-param name="eventOID" select="$oid"/> <xsl:with-param name="eventDefPosition" 
			select="$studyEventDefPosition"/> </xsl:apply-templates> -->
	</xsl:template>
	<xsl:template priority="1" mode="formRefToDefMapTemplate"
		...
		<xsl:apply-templates mode="formDataTemplate"
			select="//odm:ODM/odm:Study/odm:MetaDataVersion/odm:FormDef[@OID = $FormOID]">
	...
	<xsl:template priority="1" mode="formDataTemplate"
		...
		<!--<xsl:variable name="crfPosition" select="count(preceding-sibling::*) 
			+ 1"/> <xsl:variable name="crfPositionCalc" select="number($crfPosition)-number($eventDefCount)-1"/> -->
		<xsl:apply-templates
	...
	<xsl:template
		...
		<xsl:variable name="eventEndDateExist" select="@OpenClinica:EndDate" />
		<xsl:variable name="studyEventRepeatKey" select="@StudyEventRepeatKey" />
		<!--<xsl:apply-templates select="odm:FormData" mode="formDataHeader"> <xsl:with-param 
			name="eventPosition" select="$eventPosition" /> <xsl:with-param name="studyEventRepeatKey" 
			select="$studyEventRepeatKey"/> </xsl:apply-templates> -->
	...
	<xsl:template
		...
		<!--<xsl:variable name="formRefNodeId" select="generate-id()"/> <xsl:variable 
			name="crfPosition"> <xsl:for-each select="$allFormRefElements"> <xsl:if test="@FormOID 
			= $formRefOID"> <xsl:if test="$formRefNodeId = generate-id()"> <xsl:copy-of 
			select="position()" /> </xsl:if> </xsl:if> </xsl:for-each> </xsl:variable> -->
		<!-- maximum value of StudyEventRepeatKey for an event -->
		<xsl:variable name="MaxEventRepeatKey">
			<xsl:for-each
				select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/@StudyEventRepeatKey">
	...
	<!-- <xsl:template mode="formDataHeader2" match="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData[@FormOID]"> 
		<xsl:param name="eventPosition"/> <xsl:param name="studyEventRepeatKey"/> 
		<xsl:variable name="crfPosition" select="position()" /> <xsl:variable name="parentEvent" 
		select=".." /> <xsl:variable name="currentFormOID" select="@FormOID" /> <xsl:apply-templates 
		mode="studyEventData" select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData[generate-id() 
		= generate-id(key('studyEvents',@StudyEventOID)[1])]"> <xsl:with-param name="crfPosition" 
		select="$crfPosition" /> <xsl:with-param name="parentEvent" select="$parentEvent" 
		/> <xsl:with-param name="eventPosition" select="$eventPosition" /> </xsl:apply-templates> 
		...
		name="currentFormOID" select="$currentFormOID" /> <xsl:with-param name="eventPosition" 
		select="$eventPosition"/> <xsl:with-param name="studyEventRepeatKey" select="$studyEventRepeatKey"/> 
		</xsl:apply-templates> </xsl:template> -->
	<!-- <xsl:template mode="formDataHeader2" match="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData[@FormOID]"> 
		<xsl:param name="eventPosition"/> <xsl:param name="studyEventRepeatKey"/> 
		<xsl:param name="eventOID"/> <xsl:param name="eventNodeId" /> <xsl:variable 
		name="currentFormOID" select="@FormOID" /> <xsl:variable name="crfPositionCalc"> 
		<xsl:for-each select="//odm:ODM/odm:Study/odm:MetaDataVersion/odm:FormDef"> 
		<xsl:if test="@OID = $currentFormOID"> <xsl:copy-of select="position()" /> 
		</xsl:if> </xsl:for-each> </xsl:variable> <xsl:variable name="crfPosition" 
		select="$crfPositionCalc" /> <xsl:variable name="parentEvent" select=".." 
		/> <xsl:variable name="currentEventNodePositon"> <xsl:for-each select="$allStudyEventDataElements"> 
		<xsl:choose> <xsl:when test="$studyEventRepeatKey"> <xsl:if test="@StudyEventOID 
		= $eventOID and @StudyEventRepeatKey = $studyEventRepeatKey"> <xsl:if test="$eventNodeId 
		= generate-id()"> <xsl:copy-of select="position()" /> </xsl:if> </xsl:if> 
		</xsl:when> <xsl:otherwise> <xsl:if test="@StudyEventOID = $eventOID "> <xsl:if 
		test="$eventNodeId = generate-id()"> <xsl:copy-of select="position()" /> 
		</xsl:if> </xsl:if> </xsl:otherwise> </xsl:choose> </xsl:for-each> </xsl:variable> 
		<xsl:variable name="priorStudyElementDataElements" select="$allStudyEventDataElements[position() 
		&lt; $currentEventNodePositon]" /> <xsl:choose> <xsl:when test="$studyEventRepeatKey"> 
		<xsl:choose> <xsl:when test="count($priorStudyElementDataElements[@StudyEventOID 
		= $eventOID and @StudyEventRepeatKey = $studyEventRepeatKey and (odm:FormData/@FormOID 
		= $currentFormOID) ]) &gt; 0"> </xsl:when> <xsl:otherwise> <xsl:call-template 
		name="studyEventData2"> <xsl:with-param name="crfPosition" select="$crfPosition" 
		/> <xsl:with-param name="parentEvent" select="$parentEvent" /> <xsl:with-param 
		name="eventPosition" select="$eventPosition" /> <xsl:with-param name="studyEventRepeatKey" 
		select="$studyEventRepeatKey" /> </xsl:call-template> </xsl:otherwise> </xsl:choose> 
		</xsl:when> <xsl:otherwise> <xsl:choose> <xsl:when test="count($priorStudyElementDataElements[@StudyEventOID 
		= $eventOID and (odm:FormData/@FormOID = $currentFormOID) ]) &gt; 0">do nothing 
		</xsl:when> <xsl:otherwise> <xsl:call-template name="studyEventData2"> <xsl:with-param 
		name="crfPosition" select="$crfPosition" /> <xsl:with-param name="parentEvent" 
		select="$parentEvent" /> <xsl:with-param name="eventPosition" select="$eventPosition" 
		/> <xsl:with-param name="studyEventRepeatKey" select="$studyEventRepeatKey" 
		/> </xsl:call-template> </xsl:otherwise> </xsl:choose> </xsl:otherwise> </xsl:choose> 
		<xsl:apply-templates select="odm:ItemGroupData/odm:ItemData" mode="itemDataColumnHeaders2"> 
		<xsl:with-param name="crfPosition" select="$crfPosition" /> <xsl:with-param 
		name="currentFormOID" select="$currentFormOID" /> <xsl:with-param name="eventPosition" 
		select="$eventPosition"/> <xsl:with-param name="studyEventRepeatKey" select="$studyEventRepeatKey"/> 
		<xsl:with-param name="eventOID" select="$eventOID"/> </xsl:apply-templates> 
	...
	<xsl:template mode="studyFormColumnHeaders"
		...
		<xsl:param name="MaxEventRepeatKey" />
		<!--<xsl:variable name="formRefOID" select="@FormOID"/> -->
		<!-- <xsl:variable name="crfVersionExist" select="count(//odm:FormData[@FormOID 
			= $formRefOID and @OpenClinica:Version]) &gt; 0"/> <xsl:variable name="interviewerNameExist" 
			select="count(//odm:FormData[@FormOID = $formRefOID and @OpenClinica:InterviewerName]) 
			&gt; 0"/> <xsl:variable name="interviewDateExist" select="count(//odm:FormData[@FormOID 
			= $formRefOID and @OpenClinica:InterviewDate]) &gt; 0"/> <xsl:variable name="crfStatusExist" 
			select="count(//odm:FormData[@FormOID = $formRefOID and @OpenClinica:Status]) 
			&gt; 0"/> -->
		<xsl:choose>
					...
					<xsl:if
						...
						<xsl:if test="$interviewerNameExist">
							<td class="table_header_row">
								...
								<xsl:text>Interviewer</xsl:text>
								<xsl:choose>
									<xsl:when test="$generateIntHeadersList = 'Yes'"><!-- Use special constants here than '_E' 
	...
	<xsl:template name="createCRFColForRepeatingEvent" mode="createCRFColForRepeatingEvent"
		...
		<xsl:param name="MaxEventRepeatKey" />
		<!--<xsl:param name="crfPosition" /> <xsl:param name="crfVersionExist"/> 
			<xsl:param name="interviewerNameExist"/> <xsl:param name="interviewDateExist"/> 
			<xsl:param name="crfStatusExist"/> <xsl:variable name="formRefOID" select="@FormOID"/> -->
		<xsl:for-each select="odm:FormRef">
			<xsl:variable name="formRefOID" select="@FormOID" />
			<!-- calculate form def position in metadata -->
			<xsl:variable name="formRefNodeId" select="generate-id()" />
			...
			<xsl:if test="$interviewerNameExist">
				<td class="table_header_row">
					...
					<xsl:text>Interviewer</xsl:text>
					<xsl:choose>
						<xsl:when test="$generateIntHeadersList = 'Yes'"><!-- Use special constants here than '_E' for 
								internal processing -->
							<xsl:value-of select="$itemNameAndEventSep" />
						</xsl:when>
						<xsl:otherwise>
	...
	<xsl:template mode="studyItemDataColumnHeaders"
		...
		<!-- <xsl:variable name="formRefOID" select="@FormOID"/> formRefOID - <xsl:value-of 
			select="$formRefOID" /> <xsl:variable name="formRefNodeId" select="generate-id()"/> 
			<xsl:variable name="crfPosition"> <xsl:for-each select="$allFormRefElements"> 
			<xsl:if test="@FormOID = $formRefOID"> <xsl:if test="$formRefNodeId = generate-id()"> 
			<xsl:copy-of select="position()" /> </xsl:if> </xsl:if> </xsl:for-each> </xsl:variable> -->
		<xsl:variable name="eventPosition">
			<xsl:for-each
				select="/odm:ODM/odm:Study[1]/odm:MetaDataVersion/odm:StudyEventDef">
				<xsl:if test="@OID = $eventOID">
		...
		<!-- maximum value of StudyEventRepeatKey for an event -->
		<!--<xsl:variable name="MaxEventRepeatKey"> <xsl:for-each select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/@StudyEventRepeatKey"> 
			<xsl:sort data-type="number"/> <xsl:if test="position() = last()"> <xsl:value-of 
			select="."/> </xsl:if> </xsl:for-each> </xsl:variable> -->
		<!--<xsl:variable name="StudyEventRepeatKey" select="1"/> -->
		<!--temp hardcoded -->
		<!-- <xsl:apply-templates select="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:FormDef[@OID 
			= $formRefOID]" mode="formRefToDefTemplateForHeaders"> <xsl:with-param name="crfPosition" 
			select="$crfPosition"/> <xsl:with-param name="eventPosition" select="$eventPosition"/> 
			<xsl:with-param name="isEventRepeating" select="$isEventRepeating"/> <xsl:with-param 
			name="eventOID" select="$eventOID"/> <xsl:with-param name="StudyEventRepeatKey" 
			select="$MaxEventRepeatKey"/> </xsl:apply-templates> -->
		<xsl:choose>
			<xsl:when test="$isEventRepeating = 'Yes'">
				...
				<xsl:apply-templates select="."
					...
					<xsl:with-param name="generateIntHeadersList"
						select="$generateIntHeadersList" />
					<!-- this is just need to pass on to further template which is common 
						to repeating and non-repeating events -->
				</xsl:apply-templates>
			</xsl:when>
			<xsl:otherwise>
				<xsl:for-each select="odm:FormRef">
					...
					<xsl:apply-templates mode="formRefToDefTemplateForHeaders"
						...
						<xsl:with-param name="generateIntHeadersList"
							select="$generateIntHeadersList" />
						<!-- this param is of no use for non-repeating column further when 
	...
	<xsl:template mode="ItemGrpRefs"
		...
		<xsl:variable name="grpOID" select="@ItemGroupOID" />
		<xsl:apply-templates mode="ItemGrpRefToDefTemplateForHeaders"
	...
	<xsl:template mode="ItemGrpRefToDefTemplateForHeaders"
		...
		<!--<xsl:apply-templates select="odm:ItemRef" mode="GrpItemRefs"> <xsl:with-param 
			name="crfPosition" select="$crfPosition"/> <xsl:with-param name="eventPosition" 
			select="$eventPosition"/> <xsl:with-param name="isEventRepeating" select="$isEventRepeating"/> 
			<xsl:with-param name="formOID" select="$formOID"/> <xsl:with-param name="grpOID" 
			select="$grpOID"/> <xsl:with-param name="isGrpRepeating" select="$isGrpRepeating"/> 
			<xsl:with-param name="eventOID" select="$eventOID"/> <xsl:with-param name="StudyEventRepeatKey" 
			select="$StudyEventRepeatKey"/> <xsl:with-param name="itemGrpRepeatKey" select="$itemGrpRepeatKey"/> 
			</xsl:apply-templates> -->
	</xsl:template>
	<xsl:template mode="createItemDataColForRepeatingGrps"
		...
		<!--cnt: <xsl:value-of select="count($allStudyEventDataElements[@StudyEventOID 
			= $eventOID and @StudyEventRepeatKey = $StudyEventRepeatKey and odm:FormData/@FormOID 
			= $formOID and odm:FormData/odm:ItemGroupData/@ItemGroupOID = $grpOID and 
			odm:FormData/odm:ItemGroupData/@ItemGroupRepeatKey = $itemGrpRepeatKey])"/> 
			cntModified : <xsl:value-of select="count($allStudyEventDataElements[@StudyEventOID 
			= $eventOID and odm:FormData/@FormOID = $formOID and odm:FormData/odm:ItemGroupData/@ItemGroupOID 
			= $grpOID and odm:FormData/odm:ItemGroupData/@ItemGroupRepeatKey = $itemGrpRepeatKey])"/> -->
		<!--<xsl:variable name="maxGrpRepeatKey"> <xsl:for-each select="$allItemGrpDataDataElements[../../@StudyEventOID 
			= $eventOID and ../../@StudyEventRepeatKey = $StudyEventRepeatKey and ../@FormOID 
			= $formOID and @ItemGroupOID = $grpOID ]/@ItemGroupRepeatKey"> <xsl:sort 
			data-type="number"/> <xsl:if test="position() = last()"> <xsl:value-of select="."/> 
			</xsl:if> </xsl:for-each> </xsl:variable> -->
		<!--grpOID: <xsl:value-of select="$grpOID"/> StudyEventRepeatKey:<xsl:value-of 
			select="$StudyEventRepeatKey"/> itemGrpRepeatKey:<xsl:value-of select="$itemGrpRepeatKey"/> -->

		<xsl:choose>
			<xsl:when test="$isEventRepeating = 'Yes'">
				<xsl:variable name="maxGrpRepeatKey">
					<xsl:for-each
						select="$allItemGrpDataDataElements[../../@StudyEventOID = $eventOID and ../../@StudyEventRepeatKey = $StudyEventRepeatKey 
				...
				<xsl:if
					test="count($allItemGrpDataDataElements[../../@StudyEventOID = $eventOID and ../../@StudyEventRepeatKey = $StudyEventRepeatKey 
			...
			<xsl:otherwise>
				<xsl:variable name="maxGrpRepeatKey">
					<xsl:for-each
						select="$allItemGrpDataDataElements[../../@StudyEventOID = $eventOID  
						and ../@FormOID = $formOID and @ItemGroupOID = $grpOID ]/@ItemGroupRepeatKey">
						<xsl:sort data-type="number" />
						<xsl:if test="position() = last()">
							<xsl:value-of select="." />
				...
				<!--maxGrpRepeatKey:<xsl:value-of select="$maxGrpRepeatKey"/> cnt: <xsl:value-of 
					select="count($allItemGrpDataDataElements[../../@StudyEventOID = $eventOID 
					and ../@FormOID = $formOID and @ItemGroupOID = $grpOID and @ItemGroupRepeatKey 
					= $itemGrpRepeatKey])"/> -->
				<xsl:if
					test="count($allItemGrpDataDataElements[../../@StudyEventOID = $eventOID 
						and ../@FormOID = $formOID and @ItemGroupOID = $grpOID 
						and @ItemGroupRepeatKey = $itemGrpRepeatKey]) &gt; 0">
					<xsl:apply-templates select="odm:ItemRef" mode="GrpItemRefs">
						<xsl:with-param name="crfPosition" select="$crfPosition" />
	...
	<xsl:template mode="GrpItemRefs"
		...
		<!--GrpItemRefs -->
		<xsl:variable name="itemOID" select="@ItemOID" />
		<!--eventOID: *<xsl:value-of select="$eventOID"/>* StudyEventRepeatKey: 
			*<xsl:value-of select="$StudyEventRepeatKey"/>* grpOID: *<xsl:value-of select="$grpOID"/>* 
			itemGrpRepeatKey: *<xsl:value-of select="$itemGrpRepeatKey"/>* formOID: *<xsl:value-of 
			select="$formOID"/>* itemOID: *<xsl:value-of select="$itemOID"/>* -->


		<xsl:choose>
			<xsl:when test="$isEventRepeating = 'Yes'">
				<!--<xsl:if test="count($allStudyEventDataElements[@StudyEventOID = $eventOID 
					and @StudyEventRepeatKey = $StudyEventRepeatKey and odm:FormData/@FormOID 
					= $formOID and odm:FormData/odm:ItemGroupData/@ItemGroupOID = $grpOID and 
					odm:FormData/odm:ItemGroupData/odm:ItemData/@ItemOID = $itemOID]) &gt; 0"> 
					<xsl:apply-templates select="//odm:ODM/odm:Study/odm:MetaDataVersion/odm:ItemDef[@OID=$itemOID]" 
					mode="ItemDefColHeaders2"> <xsl:with-param name="crfPosition" select="$crfPosition"/> 
					<xsl:with-param name="eventPosition" select="$eventPosition"/> <xsl:with-param 
					name="isEventRepeating" select="$isEventRepeating"/> <xsl:with-param name="isGrpRepeating" 
					select="$isGrpRepeating"/> <xsl:with-param name="itemOID" select="$itemOID"/> 
					<xsl:with-param name="eventOID" select="$eventOID"/> <xsl:with-param name="StudyEventRepeatKey" 
					select="$StudyEventRepeatKey"/> <xsl:with-param name="itemGrpRepeatKey" select="$itemGrpRepeatKey"/> 
					<xsl:with-param name="isLastItem" select="$isLastItem"/> <xsl:with-param 
					name="generateIntHeadersList" select="$generateIntHeadersList"/> </xsl:apply-templates> 
					</xsl:if> -->
				<xsl:choose>
			...
			<xsl:otherwise><!--GrpItemRefs: non-repeating event eventOID: <xsl:value-of 
					select="$eventOID"/> formOID: <xsl:value-of select="$formOID"/> grpOID: <xsl:value-of 
					select="$grpOID"/> isGrpRepeating: *<xsl:value-of select="$isGrpRepeating"/>* 
					itemGrpRepeatKey: <xsl:value-of select="$itemGrpRepeatKey"/> itemOID: <xsl:value-of 
					select="$itemOID"/> cnt: <xsl:value-of select="count($allStudyEventDataElements[@StudyEventOID 
					= $eventOID and odm:FormData/@FormOID = $formOID and odm:FormData/odm:ItemGroupData/@ItemGroupOID 
					= $grpOID and odm:FormData/odm:ItemGroupData/odm:ItemData/@ItemOID = $itemOID])"/> 
					cnt2: <xsl:value-of select="count($allItemDataElements[@ItemOID = $itemOID 
					and ../@ItemGroupOID = $grpOID and ../@ItemGroupRepeatKey =$itemGrpRepeatKey 
	...
	<!-- <xsl:template mode="itemDataColumnHeaders2" match="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData/odm:ItemGroupData/odm:ItemData"> 
		<xsl:param name="crfPosition" /> <xsl:param name="currentFormOID" /> <xsl:param 
		name="studyEventRepeatKey" /> <xsl:param name="eventPosition" /> <xsl:param 
		name="eventOID" /> <xsl:variable name="itemData" select="." /> <xsl:variable 
		name="itemOID" select="@ItemOID" /> <xsl:variable name="itemGroupOID" select="../ItemGroupData/@ItemGroupOID" 
		/> <xsl:choose> <xsl:when test="$studyEventRepeatKey"> <xsl:variable name="priorSameItemDataOcrns" 
		select="count($allStudyEventDataElements[(@StudyEventOID = $eventOID) and 
		(@StudyEventRepeatKey = $studyEventRepeatKey) and (odm:FormData/@FormOID 
		= $currentFormOID) and (odm:FormData/odm:ItemGroupData/@ItemGroupOID = $itemGroupOID) 
		and (odm:FormData/odm:ItemGroupData/odm:ItemData/@ItemOID = $itemOID)])"/> 
		<xsl:choose> <xsl:when test="$priorSameItemDataOcrns &gt; 0"> </xsl:when> 
		<xsl:otherwise> <xsl:apply-templates select="//odm:ODM/odm:Study/odm:MetaDataVersion/odm:ItemDef[@OID=$itemOID]" 
		mode="ItemDefColHeaders2"> <xsl:with-param name="crfPosition" select="$crfPosition" 
		/> <xsl:with-param name="currentFormOID" select="$currentFormOID" /> <xsl:with-param 
		name="itemData" select="$itemData" /> <xsl:with-param name="itemOID" select="$itemOID" 
		/> <xsl:with-param name="studyEventRepeatKey" select="$studyEventRepeatKey"/> 
		<xsl:with-param name="ePosition" select="$eventPosition"/> </xsl:apply-templates> 
		</xsl:otherwise> </xsl:choose> </xsl:when> <xsl:otherwise> <xsl:variable 
		name="priorSameItemDataOcrns" select="count($allStudyEventDataElements[(@StudyEventOID 
		= $eventOID) and (odm:FormData/@FormOID = $currentFormOID) and (odm:FormData/odm:ItemGroupData/@ItemGroupOID 
		= $itemGroupOID) and (odm:FormData/odm:ItemGroupData/odm:ItemData/@ItemOID 
		= $itemOID)])"/> <xsl:choose> <xsl:when test="$priorSameItemDataOcrns &gt; 
		0"> </xsl:when> <xsl:otherwise> <xsl:apply-templates select="//odm:ODM/odm:Study/odm:MetaDataVersion/odm:ItemDef[@OID=$itemOID]" 
		mode="ItemDefColHeaders2"> <xsl:with-param name="crfPosition" select="$crfPosition" 
		/> <xsl:with-param name="currentFormOID" select="$currentFormOID" /> <xsl:with-param 
		name="itemData" select="$itemData" /> <xsl:with-param name="itemOID" select="$itemOID" 
		/> <xsl:with-param name="studyEventRepeatKey" select="$studyEventRepeatKey"/> 
	...
	<xsl:template mode="ItemDefColHeaders2"
		...
		<td class="table_header_row">
			<a href="javascript: void(0)">
				...
				<!--<xsl:text>_</xsl:text> <xsl:value-of select="$E"/> <xsl:value-of 
					select="$eventPosition"/> <xsl:if test="$isEventRepeating = 'Yes'"> <xsl:text>_</xsl:text><xsl:value-of 
					select="$StudyEventRepeatKey"/> </xsl:if> -->
			</a>
		</td>
	</xsl:template>
	<xsl:template mode="ItemDefColHeaders"
		...
		<td class="table_header_row">
			<xsl:if test="$currentFormOID = $formOID">
				...
				<a href="javascript: void(0)">
					<xsl:attribute name="onclick">
                                            openWin('<xsl:value-of
						select="$itemOID" />'); return  false;
                                            </xsl:attribute>
					<xsl:value-of select="@Name" />
					<xsl:text>_</xsl:text>
					<xsl:value-of select="$C" />
					...
					<!-- JN: Commenting out the logic for now, not sure if this is right 
						as per Paul's suggestion -->
					<!--<xsl:for-each select="//odm:ItemGroupDef[@OID=$groupOID]"> <xsl:if 
						test="@Name !='Ungrouped'"> <xsl:value-of select="@Name" /> </xsl:if> </xsl:for-each> -->
	...
	<!-- <xsl:template mode="ItemDefColHeaders2" match="//odm:ODM/odm:Study/odm:MetaDataVersion/odm:ItemDef[@OID]"> 
		<xsl:param name="crfPosition" /> <xsl:param name="currentFormOID" /> <xsl:param 
		name="itemData" /> <xsl:param name="itemOID" /> <xsl:param name="studyEventRepeatKey"/> 
		<xsl:param name="ePosition"/> <xsl:variable name="formOID" select="OpenClinica:ItemDetails/OpenClinica:ItemPresentInForm[@FormOID 
		= $itemData/../../@FormOID]/@FormOID" /> <td class="table_header_row"> <xsl:if 
		test="$currentFormOID = $formOID"> <a href="javascript: void(0)"> <xsl:attribute 
		name="onclick"> openWin('<xsl:value-of select="$itemOID" />'); return false; 
		</xsl:attribute> <xsl:value-of select="@Name" /> <xsl:text>_</xsl:text> <xsl:value-of 
		select="$C" /> <xsl:value-of select="$crfPosition" /> <xsl:variable name="group" 
		select="$itemData/parent::node()" /> <xsl:variable name="groupOID" select="$group/@ItemGroupOID" 
		/> <xsl:if test="$group/@ItemGroupRepeatKey"> <xsl:text>_</xsl:text> <xsl:value-of 
		select="$group/@ItemGroupRepeatKey" /> </xsl:if> <xsl:text>_</xsl:text> <xsl:value-of 
		select="$E"/> <xsl:value-of select="$ePosition"/> <xsl:if test="$studyEventRepeatKey"> 
		<xsl:text>_</xsl:text> <xsl:value-of select="$studyEventRepeatKey"/> </xsl:if> 
		</a> </xsl:if> </td> </xsl:template> -->
	<!-- -->
