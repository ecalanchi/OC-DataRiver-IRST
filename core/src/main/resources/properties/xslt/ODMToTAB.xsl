...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/odm:ODM">
		...
		<!--tokenizedcrfAndDataItemsHeaders:<xsl:for-each select="$tokenizedcrfAndDataItemsHeaders"> 
			token<xsl:value-of select="position()"/>:<xsl:value-of select="."/> </xsl:for-each> -->
		<!-- <xsl:apply-templates -->
		...
		<!--<xsl:value-of select="$eol"/> --><!-- Subject Data, item data etc -->
	...
	<!-- <xsl:template priority="1" mode="formDataTemplate" match="//odm:ODM/odm:Study/odm:MetaDataVersion/odm:FormDef"> 
		<xsl:variable name="FormOID" select="@OID" /> <xsl:variable name="formName" 
		select="@Name" /> <xsl:variable name="crfPosition" select="position()"/> 
		<xsl:apply-templates select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData[generate-id() 
		= generate-id(key('eventCRFs',$FormOID)[1])]" mode="CrfInfo"> <xsl:with-param 
		name="oid" select="$FormOID" /> <xsl:with-param name="formName" select="$formName" 
		/> <xsl:with-param name="crfPosition" select="$crfPosition"/> </xsl:apply-templates> 
		</xsl:template> -->
	...
	<xsl:template mode="ItemDefColHeaders"
		...
		<xsl:if test="$currentFormOID = $formOID"> <!-- Changed from$currentFormOID = $formOID -->
			...
			<!-- JN: Commenting out the logic for now, not sure if this is right as 
		...
		<xsl:if test="@StudyEventOID = $parentEvent/@StudyEventOID">
			<xsl:choose>
				<xsl:when test="@StudyEventRepeatKey">
					...
					<!-- PG - commented to test outside OC <xsl:for-each select="exsl:node-set($allStudyEvents)/odm:StudyEventData"> 
						<xsl:choose> <xsl:when test="position()=1"> <xsl:if test="$interviewerNameExist"> 
						<xsl:text>Interviewer_</xsl:text> <xsl:value-of select="$E" /> <xsl:value-of 
						select="$eventPosition" /> <xsl:if test="@StudyEventRepeatKey"> <xsl:text>_</xsl:text> 
						<xsl:value-of select="@StudyEventRepeatKey" /> </xsl:if> <xsl:text>_</xsl:text> 
						<xsl:value-of select="$C" /> <xsl:value-of select="$crfPosition" /> <xsl:value-of 
						select="$delimiter" /> </xsl:if> <xsl:if test="$interviewDateExist"> <xsl:text>Interviewer 
						date</xsl:text> <xsl:value-of select="$E" /> <xsl:value-of select="$eventPosition" 
						/> <xsl:if test="@StudyEventRepeatKey"> <xsl:text>_</xsl:text> <xsl:value-of 
						select="@StudyEventRepeatKey" /> </xsl:if> <xsl:text>_</xsl:text> <xsl:value-of 
						select="$C" /> <xsl:value-of select="$crfPosition" /> <xsl:value-of select="$delimiter" 
						/> </xsl:if> <xsl:if test="$crfStatusExist"> <xsl:text>CRF Version Status_</xsl:text> 
						<xsl:value-of select="$E" /> <xsl:value-of select="$eventPosition" /> <xsl:text>_</xsl:text> 
						<xsl:value-of select="@StudyEventRepeatKey" /> <xsl:text>_</xsl:text> <xsl:value-of 
						select="$C" /> <xsl:value-of select="$crfPosition" /> <xsl:value-of select="$delimiter" 
						/> </xsl:if> <xsl:if test="$crfVersionExist"> <xsl:text>Version Name_</xsl:text> 
						<xsl:value-of select="$E" /> <xsl:value-of select="$eventPosition" /> <xsl:text>_</xsl:text> 
						<xsl:value-of select="@StudyEventRepeatKey" /> <xsl:text>_</xsl:text> <xsl:value-of 
						select="$C" /> <xsl:value-of select="$crfPosition" /> <xsl:value-of select="$delimiter" 
						/> </xsl:if> </xsl:when> <xsl:otherwise> <xsl:if test="preceding-sibling::odm:StudyEventData[1]/@StudyEventRepeatKey 
						!= @StudyEventRepeatKey"><xsl:if test="$interviewerNameExist"> <xsl:text>Interviewer_</xsl:text> 
						...
						/> </xsl:if> <xsl:text>_</xsl:text> <xsl:value-of select="$C" /> <xsl:value-of 
						select="$crfPosition" /> <xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if 
						test="$interviewDateExist"> <xsl:text>Interviewer date</xsl:text> <xsl:value-of 
						...
						<xsl:text>_</xsl:text> <xsl:value-of select="$C" /> <xsl:value-of select="$crfPosition" 
						/> <xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if test="$crfStatusExist"> 
						<xsl:text>CRF Version Status_</xsl:text> <xsl:value-of select="$E" /> <xsl:value-of 
						select="$eventPosition" /> <xsl:text>_</xsl:text> <xsl:value-of select="@StudyEventRepeatKey" 
						/> <xsl:text>_</xsl:text> <xsl:value-of select="$C" /> <xsl:value-of select="$crfPosition" 
						/> <xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if test="$crfVersionExist"> 
						<xsl:text>Version Name_</xsl:text> <xsl:value-of select="$E" /> <xsl:value-of 
						select="$eventPosition" /> <xsl:text>_</xsl:text> <xsl:value-of select="@StudyEventRepeatKey" 
						/> <xsl:text>_</xsl:text> <xsl:value-of select="$C" /> <xsl:value-of select="$crfPosition" 
						/> <xsl:value-of select="$delimiter" /> </xsl:if> </xsl:if> </xsl:otherwise> 
				...
				<xsl:otherwise><!-- <xsl:if test="$eventLocationExist"> <xsl:text>Location_</xsl:text> 
						...
						/> <xsl:text>_</xsl:text> <xsl:value-of select="$C" /> <xsl:value-of select="$crfPosition" 
						/> <xsl:value-of select="$delimiter" /> </xsl:if> -->
			...
			<xsl:choose>
				<xsl:when test="@StudyEventRepeatKey">
					...
					<!-- PG - commented to test outside OC <xsl:for-each select="exsl:node-set($allStudyEvents)/odm:StudyEventData"> 
						...
						<xsl:if test="$interviewerNameExist"> <xsl:value-of select="$currentForm/@OpenClinica:InterviewerName"/> 
						<xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if test="$interviewDateExist"> 
						<xsl:value-of select="$currentForm/@OpenClinica:InterviewDate"/> <xsl:value-of 
						select="$delimiter" /> </xsl:if> <xsl:if test="$crfStatusExist"> <xsl:value-of 
						select="$currentForm/@OpenClinica:Status"/> <xsl:value-of select="$delimiter" 
						/> </xsl:if> <xsl:if test="$crfVersionExist"> <xsl:value-of select="$currentForm/@OpenClinica:Version"/> 
						<xsl:value-of select="$delimiter" /> </xsl:if> </xsl:when> <xsl:otherwise> 
						<xsl:if test="$interviewerNameExist"> <xsl:value-of select="$delimiter" /> 
						</xsl:if> <xsl:if test="$interviewDateExist"> <xsl:value-of select="$delimiter" 
						/> </xsl:if> <xsl:if test="$crfVersionExist"> <xsl:value-of select="$delimiter" 
						/> </xsl:if> <xsl:if test="$crfStatusExist"> <xsl:value-of select="$delimiter" 
						/> </xsl:if> </xsl:otherwise> </xsl:choose> </xsl:when> <xsl:otherwise> <xsl:if 
						test="preceding-sibling::odm:StudyEventData[1]/@StudyEventRepeatKey != @StudyEventRepeatKey"> 
						<xsl:choose> <xsl:when test="$subjectFormData/node() and $subjectEvent/@StudyEventOID=@StudyEventOID 
						and $subjectEvent/@StudyEventRepeatKey=@StudyEventRepeatKey"> <xsl:if test="$interviewerNameExist"> 
						<xsl:value-of select="$currentForm/@OpenClinica:InterviewerName"/> <xsl:value-of 
						select="$delimiter" /> </xsl:if> <xsl:if test="$interviewDateExist"> <xsl:value-of 
						select="$currentForm/@OpenClinica:InterviewDate"/> <xsl:value-of select="$delimiter" 
						/> </xsl:if> <xsl:if test="$crfStatusExist"> <xsl:value-of select="$currentForm/@OpenClinica:Status"/> 
						<xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if test="$crfVersionExist"> 
						<xsl:value-of select="$currentForm/@OpenClinica:Version"/> <xsl:value-of 
						select="$delimiter" /> </xsl:if> </xsl:when> <xsl:otherwise> <xsl:if test="$interviewerNameExist"> 
						<xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if test="$interviewDateExist"> 
						<xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if test="$crfVersionExist"> 
						<xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if test="$crfStatusExist"> 
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="$subjectFormData/node()"><!-- <xsl:if test="$eventLocationExist"> <xsl:value-of 
								select="@OpenClinica:StudyEventLocation"></xsl:value-of> <xsl:value-of select="$delimiter" 
								/> </xsl:if> <xsl:if test="$eventStartDateExist"> <xsl:value-of select="@OpenClinica:StartDate"></xsl:value-of> 
								<xsl:value-of select="$delimiter" /> </xsl:if> <xsl:if test="$eventEndDateExist"> 
								<xsl:value-of select="@OpenClinica:EndDate"></xsl:value-of> <xsl:value-of 
								select="$delimiter" /> </xsl:if> <xsl:if test="$eventStatusExist"> <xsl:value-of 
								select="@OpenClinica:Status"></xsl:value-of> <xsl:value-of select="$delimiter" 
								/> </xsl:if> <xsl:if test="$ageExist"> <xsl:value-of select="@OpenClinica:SubjectAgeAtEvent"></xsl:value-of> 
								<xsl:value-of select="$delimiter" /> </xsl:if> -->
							<xsl:if test="$interviewerNameExist">
						...
						<xsl:otherwise><!-- <xsl:if test="$eventLocationExist"> <xsl:value-of 
								select="$delimiter" /> </xsl:if> <xsl:if test="$eventStartDateExist"> <xsl:value-of 
								select="$delimiter" /> </xsl:if> <xsl:if test="$eventEndDateExist"> <xsl:value-of 
								select="$delimiter" /> </xsl:if> <xsl:if test="$eventStatusExist"> <xsl:value-of 
								select="$delimiter" /> </xsl:if> <xsl:if test="$ageExist"> <xsl:value-of 
	...
	<xsl:template name="createColForRepeatingEvent"
		...
		<!--<xsl:if test="$eventStartDateExist"> -->
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey = $eventRepeatCnt and @OpenClinica:StartDate]) &gt; 0">
			<xsl:text>StartDate_</xsl:text>
			<xsl:value-of select="$E" />
			<xsl:value-of select="$eventPosition" />
			<xsl:text>_</xsl:text>
	...
	<xsl:template mode="createItemDataColForRepeatingGrps"
		...
		<!--grpOID: <xsl:value-of select="$grpOID"/> maxGrpRepeatKey:<xsl:value-of 
			select="$maxGrpRepeatKey"/> StudyEventRepeatKey:<xsl:value-of select="$StudyEventRepeatKey"/> 
			itemGrpRepeatKey:<xsl:value-of select="$itemGrpRepeatKey"/> cnt: <xsl:value-of 
			select="count($allItemGrpDataDataElements[../../@StudyEventOID = $eventOID 
			and ../../@StudyEventRepeatKey = $StudyEventRepeatKey and ../@FormOID = $formOID 
	...
	<xsl:template mode="GrpItemRefs"
		...
		<xsl:choose>
			<xsl:when test="$isEventRepeating = 'Yes'">
				<!-- <xsl:if test="count($allStudyEventDataElements[@StudyEventOID = 
					$eventOID and @StudyEventRepeatKey = $StudyEventRepeatKey and odm:FormData/@FormOID 
					= $formOID and odm:FormData/odm:ItemGroupData/@ItemGroupOID = $grpOID and 
	...
	<xsl:template mode="ItemDefColHeaders2"
		...
		<xsl:value-of select="' '" />
		<xsl:value-of select="@Name" />
		<!--<xsl:text>_</xsl:text> <xsl:value-of select="$E"/> <xsl:value-of select="$eventPosition"/> -->
		<xsl:choose>
			<xsl:when test="$generateIntHeadersList = 'Yes'"><!-- Use special constants here than '_E' for 
					internal processing -->
				<xsl:value-of select="$itemNameAndEventSep" />
			...
			<xsl:if test=". != $tokenizedcrfAndDataItemsHeaders[last()]"><!--not last -->
				...
				<xsl:variable name="numericStart">
					<xsl:choose>
						...
						<xsl:otherwise>
							<!-- @pgawade: Added following conditional logic in order to meet 
								display of both data item values as well as CRF attributes -->
											...
											<xsl:choose>
												<xsl:when test="$crfPosition = normalize-space($colCrfPosition)"><!--crf matched -->
													<xsl:choose>
														...
														<xsl:otherwise><!--<xsl:text>M_</xsl:text> -->
															<xsl:value-of select="$matchSep" />
															<xsl:value-of select="$eventOID" />
															<xsl:value-of select="$mValSeparator1" />
															<xsl:value-of select="$formOID" />
