...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/odm:ODM">
		...
		<!-- JN: Commenting out fix for 0012071: SPSS *.dat file has blank row 
			at the beginning -->
		<!-- xsl:value-of select="$eol"></xsl:value-of --><!-- Subject Data, item data etc -->
	...
	<xsl:template match="/odm:ODM/odm:ClinicalData/odm:SubjectData"
		...
		<xsl:if test="$dobExist">
			<!-- @pgawade 21-Mar-2012 #12213 convert ISO 8601 format date into american 
				date -->
	...
	<xsl:template mode="createItemDataColForRepeatingGrps"
		...
		<!-- <xsl:if test="count($allStudyEventDataElements[@StudyEventOID = $eventOID 
			...
			&gt; 0"> <xsl:apply-templates select="odm:ItemRef" mode="GrpItemRefs"> <xsl:with-param 
			name="crfPosition" select="$crfPosition"/> <xsl:with-param name="eventPosition" 
			select="$eventPosition"/> <xsl:with-param name="isEventRepeating" select="$isEventRepeating"/> 
			<xsl:with-param name="formOID" select="$formOID"/> <xsl:with-param name="grpOID" 
			select="$grpOID"/> <xsl:with-param name="isGrpRepeating" select="$isGrpRepeating"/> 
			<xsl:with-param name="eventOID" select="$eventOID"/> <xsl:with-param name="StudyEventRepeatKey" 
			select="$StudyEventRepeatKey"/> <xsl:with-param name="itemGrpRepeatKey" select="$itemGrpRepeatKey"/> 
			<xsl:with-param name="isLastItem" select="position()=last()" /> </xsl:apply-templates> 
			<xsl:apply-templates mode="createItemDataColForRepeatingGrps" select="."> 
			<xsl:with-param name="crfPosition" select="$crfPosition"/> <xsl:with-param 
			name="eventPosition" select="$eventPosition"/> <xsl:with-param name="isEventRepeating" 
			select="$isEventRepeating"/> <xsl:with-param name="formOID" select="$formOID"/> 
			<xsl:with-param name="grpOID" select="$grpOID"/> <xsl:with-param name="eventOID" 
			select="$eventOID"/> <xsl:with-param name="StudyEventRepeatKey" select="$StudyEventRepeatKey"/> 
			<xsl:with-param name="itemGrpRepeatKey" select="$itemGrpRepeatKey+1"/> <xsl:with-param 
			name="isGrpRepeating" select="$isGrpRepeating"/> </xsl:apply-templates> </xsl:if> -->
			...
			<xsl:if test=". != $tokenizedEventHeaders[last()]"><!--not last -->
				...
				<xsl:variable name="numericStart">
					...
					<xsl:if test="ends-with($nextToken,'EventStatus')">
						<xsl:value-of
							select="substring-before($nextToken,concat($delimiter, 'EventStatus'))" />
					</xsl:if>
						...
						<xsl:choose>
							<xsl:when test="normalize-space($colEventPosition) = $eventPosition"><!--{event matched} -->
								<xsl:choose>
									...
									<xsl:otherwise><!--<xsl:text>M</xsl:text> -->
										<xsl:value-of select="$matchSep" />
										<xsl:value-of select="position()" />
									</xsl:otherwise>
								</xsl:choose>
						...
						<xsl:if test="$colType = 'StartDate'">
							<xsl:choose>
								<xsl:when test="$event/@OpenClinica:StartDate">
									<!-- @pgawade 21-Mar-2012 #12213 convert ISO 8601 format date into 
										american date -->
									<!--<xsl:value-of select="$event/@OpenClinica:StartDate"></xsl:value-of> -->
									<xsl:call-template name="convert-date-ISO8601-to-ADate">
										<xsl:with-param name="dateSrc"
											select="$event/@OpenClinica:StartDate" />
									</xsl:call-template>
									<xsl:value-of select="$delimiter" />
								</xsl:when>
								<xsl:otherwise>
						...
						<xsl:if test="$colType = 'EndDate'">
							<xsl:choose>
								<xsl:when test="$event/@OpenClinica:EndDate">
									...
									<xsl:call-template name="convert-date-ISO8601-to-ADate">
										<xsl:with-param name="dateSrc"
											select="$event/@OpenClinica:EndDate" />
									</xsl:call-template>
									<xsl:value-of select="$delimiter" />
								</xsl:when>
								<xsl:otherwise>
				...
				<xsl:variable name="numericStart">
					<xsl:choose>
						...
						<xsl:when test="ends-with($nextToken,'InterviewDate')">
							<xsl:value-of
								select="substring-before($nextToken,concat(concat($delimiter, ' '), 'InterviewDate'))" />
						</xsl:when>
						<xsl:when test="ends-with($nextToken,'CRFVersionStatus')">
							<xsl:value-of
								select="substring-before($nextToken,concat(concat($delimiter, ' '), 'CRFVersionStatus'))" />
						</xsl:when>
						<xsl:when test="ends-with($nextToken,'VersionName')">
							<xsl:value-of
								select="substring-before($nextToken,concat(concat($delimiter, ' '), 'VersionName'))" />
						</xsl:when>
						<xsl:otherwise>
					...
					<xsl:when test="$colType = 'ItemData'"><!--data column -->
																		...
																		<xsl:choose>
																				...
																				<xsl:choose>
																					<xsl:when
																						...
																						<xsl:for-each select="./odm:ItemData">
																							...
																							<!-- @pgawade 14-May-2012 #13613 prepend the 'v$' 
																								before item name if it starts with some thing other then alphabet -->
																							<xsl:variable name="itemNameValidated">
																								<xsl:choose>
																									<xsl:when
																										test='matches(substring($itemName, 1, 1), "[^A-Za-z]")'>
																										<xsl:value-of
																											select="concat('v$', normalize-space($itemName))" />
																									</xsl:when>
																									<xsl:otherwise>
																										<xsl:value-of select="$itemName" />
																									</xsl:otherwise>
																							...
																							<xsl:choose>
																								<xsl:when
																									test="normalize-space($colItemName) = $itemNameValidated"><!--{item name matched} -->
																			...
																			<xsl:otherwise>
																				...
																				<xsl:for-each select="./odm:ItemData">
																					...
																					<xsl:variable name="itemName"
																						select="//odm:ItemDef[@OID = $itemOID]/@Name" />
																					<!-- @pgawade 14-May-2012 #13613 prepend the 'v$' before 
																						item name if it starts with some thing other then alphabet -->
																					<xsl:variable name="itemNameValidated">
																						<xsl:choose>
																							<xsl:when
																								test='matches(substring($itemName, 1, 1), "[^A-Za-z]")'>
																					...
																					<xsl:choose>
																						<xsl:when
																							test="normalize-space($colItemName) = $itemNameValidated">
																							<!--<xsl:text>M_</xsl:text> -->
																							<xsl:value-of select="$matchSep" />
																							<xsl:value-of select="$eventOID" />
																							<xsl:value-of select="$mValSeparator1" />
																...
																<xsl:choose>
																		...
																		<xsl:choose>
																			<xsl:when
																				...
																				<xsl:for-each select="./odm:ItemData">
																					...
																					<xsl:choose>
																						<xsl:when
																							test="normalize-space($colItemName) = $itemNameValidated"><!-- only grp repeating -->
																							<!--<xsl:text>M_</xsl:text> -->
																							<xsl:value-of select="$matchSep" />
																							<xsl:value-of select="$eventOID" />
																							<xsl:value-of select="$mValSeparator1" />
																	...
																	<xsl:otherwise>
																		...
																		<xsl:for-each select="./odm:ItemData">
																			...
																			<xsl:choose>
																				<xsl:when
																					test="normalize-space($colItemName) = $itemNameValidated"><!-- nothing repeating -->
																					<!--<xsl:text>M_</xsl:text> -->
																					<xsl:value-of select="$matchSep" />
																					<xsl:value-of select="$eventOID" />
																					<xsl:value-of select="$mValSeparator1" />
						...
						<xsl:choose>
							<xsl:when test="contains($ifMatch, $matchSep)">
								...
								<xsl:variable name="itemDataType"
									select="$allItemDefs[@OID = $itemOID]/@DataType" />
									 ...
									 and ../../../@StudyEventOID = $eventOID and ../../../@StudyEventRepeatKey = $eventRepeatKey]" />
										...
										<xsl:choose>
											<xsl:when test="$itemData/@Value">
												<!-- @pgawade 22-Mar-2012 #12213 convert ISO 8601 format date 
													into american date -->
												<xsl:choose>
										...
										<xsl:if test="$colType = 'InterviewDate'">
											<xsl:choose>
												<xsl:when test="$formData/@OpenClinica:InterviewDate">
													<!-- @pgawade 21-Mar-2012 #12213 convert ISO 8601 format date 
														into american date -->
													<!--<xsl:value-of select="$formData/@OpenClinica:InterviewDate"></xsl:value-of> -->
													<xsl:call-template name="convert-date-ISO8601-to-ADate">
														<xsl:with-param name="dateSrc"
															select="$formData/@OpenClinica:InterviewDate" />
													</xsl:call-template>
													<xsl:value-of select="$delimiter" />
												</xsl:when>
												<xsl:otherwise>
	...
	<!-- @pagwade 21-Mar-2012 #12213 template to convert ISO 8601 (YYYY-MM-DD) 
		date value to american date (MM/DD/YYYY) -->
			...
			<xsl:when test="contains($dateSrc, $sepDateSrc)">
				...
				<xsl:choose>
					<xsl:when test="contains($afterSepDateSrc, $sepDateSrc)">
						<xsl:variable name="month"
							select="substring-before($afterSepDateSrc,$sepDateSrc)" />
						<xsl:variable name="date"
							select="substring-after($afterSepDateSrc,$sepDateSrc)" />
						<xsl:value-of select="$month" />
						<xsl:value-of select="$sepDateOp" />
