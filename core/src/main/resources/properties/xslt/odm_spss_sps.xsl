...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/">
		...
		<xsl:text>* NOTE: If you have put this file in a different folder </xsl:text>
		<xsl:value-of select="$seperator" />
		<xsl:text>* from the associated data file, you will have to change the FILE </xsl:text>
		<xsl:value-of select="$seperator" />
		...
		<!-- @pgawade 29-Feb-2012 fix for issue #11796 Do not include data timestamp 
			in the name of .dat file name -->
		<!-- <xsl:variable name="currentDate" select="current-date()"/> <xsl:variable 
			name="currentDateTime" select="current-dateTime()"/> <xsl:value-of select="format-dateTime($currentDateTime, 
			'[Y0001]-[M01]-[D01]-[H01][m01][s00001]')"/> -->
		...
		<!--<xsl:apply-templates select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData[1]" 
			mode="studyDataSPSS"></xsl:apply-templates> -->
		<xsl:apply-templates mode="studyDataSPSS2"
			select="/odm:ODM/odm:Study[1]/odm:MetaDataVersion/odm:StudyEventDef" />

		<!--<xsl:apply-templates mode="formDataSPSS" select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData[generate-id() 
			= generate-id(key('eventCRFs',@FormOID)[1])]"> <xsl:with-param name="crfPosition" 
			select="$crfPosition"></xsl:with-param> </xsl:apply-templates> -->
		<xsl:apply-templates mode="studyFormAndDataItemsHeaders"
			select="/odm:ODM/odm:Study[1]/odm:MetaDataVersion/odm:StudyEventDef" />
		...
		<!--xsl:apply-templates select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData[generate-id() 
			= generate-id(key('studyEvents',@StudyEventOID)[1])]" mode="studyEventDataColumnSPSS" 
			/> -->
		...
		<!--<xsl:apply-templates mode="itemDataValuesSPSS" select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData[generate-id() 
		...
		<xsl:apply-templates mode="studyItemDataColumnHeaders"
			select="/odm:ODM/odm:Study[1]/odm:MetaDataVersion/odm:StudyEventDef">
		...
		<!-- <xsl:text>.</xsl:text> <xsl:text>&#xa;</xsl:text> <xsl:text>VALUE 
			LABELS</xsl:text> <xsl:text>&#xa;</xsl:text> <xsl:apply-templates select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData" 
			mode="allColumnsandLabels"/> -->
		<xsl:text>.</xsl:text>
		<xsl:text>&#xa;</xsl:text>
		<xsl:text>EXECUTE.</xsl:text>
	</xsl:template>

	<xsl:template mode="studyDataSPSS2"
		...
		<xsl:variable name="studyEventData"
			select="/odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData[@StudyEventOID = $eventDefOID]" />
		...
		<xsl:variable name="locationLen">
			<xsl:for-each
				select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/string-length(@OpenClinica:StudyEventLocation)">
		...
		<xsl:variable name="eventStatusLen">
			<xsl:for-each
				select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/string-length(@OpenClinica:Status)">
				<xsl:sort data-type="number" />
				<xsl:if test="position() = last()">
					<xsl:value-of select="." />
				</xsl:if>
		...
		<xsl:variable name="ageLen">
			<xsl:for-each
				select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/string-length(@OpenClinica:SubjectAgeAtEvent)">
				<xsl:sort data-type="number" />
				<xsl:if test="position() = last()">
					<xsl:value-of select="." />
				</xsl:if>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:apply-templates select="."
					mode="studyDataSPSSForNonRepeatingEvent">
					<xsl:with-param name="eventPosition" select="$eventPosition" />
					<xsl:with-param name="eventOID" select="$eventDefOID" />
					<!-- <xsl:with-param name="eventLocationExist" select="$eventLocationExist"/> 
						<xsl:with-param name="eventStartDateExist" select="$eventStartDateExist"/> 
	...
	<xsl:template name="studyDataSPSSForRepeatingEvent"
		match="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef" mode="studyDataSPSSForRepeatingEvent">
		...
		<xsl:param name="MaxEventRepeatKey" /><!--studyDataSPSSForRepeatingEvent, 
		...
		<xsl:if
			...
			<xsl:text> A</xsl:text>
			<!--<xsl:choose> <xsl:when test="number($locationLen) &gt; 8"> <xsl:text>8</xsl:text> 
		...
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey = $eventRepeatCnt and @OpenClinica:StartDate]) &gt; 0"><!--col for event startdate --><!--start date present -->
		...
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey = $eventRepeatCnt and @OpenClinica:EndDate]) &gt; 0"><!--end date presen -->
		...
		<xsl:if
			...
			<xsl:value-of select="$eventRepeatCnt" />
			<xsl:text> A</xsl:text>
			<!--<xsl:choose> <xsl:when test="number($eventStatusLen) &gt; 8"> <xsl:text>8</xsl:text> 
				</xsl:when> <xsl:otherwise> -->
			<xsl:value-of select="$eventStatusLen" />
			<!--sl:otherwise> </xsl:choose> -->
			<xsl:text>&#xa;</xsl:text>
		</xsl:if>
		...
		<xsl:if
			...
			<!--<xsl:choose> <xsl:when test="number($ageLen) &gt; 8"> <xsl:text>8</xsl:text> 
				</xsl:when> <xsl:otherwise> <xsl:value-of select="$ageLen" /> </xsl:otherwise> 
				</xsl:choose> -->
			<xsl:text>&#xa;</xsl:text>
		</xsl:if>
	...
	<xsl:template mode="studyDataSPSSForNonRepeatingEvent"
		match="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef">
		<xsl:param name="eventPosition" />
		<!--<xsl:param name="eventLocationExist"/> <xsl:param name="eventStartDateExist" 
			/> <xsl:param name="eventStatusExist"/> <xsl:param name="eventEndDateExist"/> 
		...
		<!-- @pgawade 15-May-2012 fix for issue 14279 consider the presense of 
			event attribute specific to ordinal -->
		<!--<xsl:if test="$eventLocationExist"> -->
		<xsl:if
		...
		<xsl:if
			...
			<!--<xsl:text>StartDate_</xsl:text> <xsl:value-of select="$E" /> <xsl:value-of 
				select="$eventPosition" /> <xsl:text> ADATE10</xsl:text> <xsl:text>&#xa;</xsl:text> -->
		</xsl:if>
	...
	<xsl:template mode="studyEventDataColumnSPSS2"
		match="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef">
		...
		<xsl:choose>
			<xsl:when test="$isRepeating = 'Yes'">
				...
				<xsl:apply-templates select="."
					...
					<!-- <xsl:with-param name="eventLocationExist" select="$eventLocationExist"/> 
						...
						name="eventEndDateExist" select="$eventEndDateExist"/> <xsl:with-param name="ageExist" 
						select="$ageExist"/> -->
					<!--<xsl:with-param name="locationLen" select="$locationLen"/> <xsl:with-param 
						name="ageLen" select="$ageLen"/> <xsl:with-param name="eventStatusLen" select="$eventStatusLen"/> -->
			...
			<xsl:otherwise>
				<!-- write event data header columns for non repeating event -->
				<xsl:apply-templates select="."
					mode="studyEventDataColumnSPSSForNonRepeatingEvent">
					<xsl:with-param name="eventPosition" select="$eventPosition" />
					<xsl:with-param name="eventOID" select="$eventDefOID" />
					<!--<xsl:with-param name="eventLocationExist" select="$eventLocationExist"/> 
						<xsl:with-param name="eventStartDateExist" select="$eventStartDateExist"/> 
						<xsl:with-param name="eventStatusExist" select="$eventStatusExist"/> <xsl:with-param 
						name="eventEndDateExist" select="$eventEndDateExist"/> <xsl:with-param name="ageExist" 
						select="$ageExist"/> <xsl:with-param name="locationLen" select="$locationLen"/> 
						<xsl:with-param name="ageLen" select="$ageLen"/> <xsl:with-param name="eventStatusLen" 
						select="$eventStatusLen"/> -->
					<xsl:with-param name="eventName" select="$eventName" />
				</xsl:apply-templates>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template name="studyEventDataColumnSPSSForRepeatingEvent"
		match="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef" mode="studyEventDataColumnSPSSForRepeatingEvent">
		<xsl:param name="eventOID" />
		<xsl:param name="eventPosition" />
		<xsl:param name="eventRepeatCnt" />
		<!--<xsl:param name="locationLen"/> <xsl:param name="ageLen"/> <xsl:param 
			name="eventStatusLen"/> -->
		...
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey = $eventRepeatCnt and @OpenClinica:Status]) &gt; 0">
			<!-- @pgawade 14-Mar-2012 #13052 Removed the unwanted space within value 
		...
		<xsl:if
			test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey = ($eventRepeatCnt+1)]) &gt; 0">
	...
	<xsl:template mode="studyEventDataColumnSPSSForNonRepeatingEvent"
		match="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef">
		<xsl:param name="eventPosition" />
		<xsl:param name="eventName" />
		<xsl:param name="eventOID" />
	...
	<xsl:template mode="studyFormAndDataItemsHeaders"
		...
		<xsl:apply-templates mode="studyFormColumnHeaders"
			select=".">
			<xsl:with-param name="eventPosition" select="$eventPosition" />
		...
		<!--<xsl:apply-templates mode="studyItemDataColumnHeaders" select="."> 
			<xsl:with-param name="eventOID" select="$eventOID"/> <xsl:with-param name="isEventRepeating" 
			select="$isRepeatingEvent"/> <xsl:with-param name="calledFor" select="'forCodeListsOnly'"/> 
			</xsl:apply-templates> -->
	</xsl:template>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:choose>
					...
					@OpenClinica:Status]) &gt; 
						0" />
							<!-- <xsl:variable name="interLen"> <xsl:for-each select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData/@OpenClinica:InterviewerName"> 
								<xsl:sort data-type="number"/> <xsl:if test="position() = last()"> <xsl:value-of 
								select="."/> </xsl:if> </xsl:for-each> </xsl:variable> <xsl:variable name="interStatusLen"> 
								<xsl:for-each select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData/@OpenClinica:Status"> 
								<xsl:sort data-type="number"/> <xsl:if test="position() = last()"> <xsl:value-of 
								select="."/> </xsl:if> </xsl:for-each> </xsl:variable> <xsl:variable name="versionLen"> 
					...
					@OpenClinica:Status]) &gt; 
						0" />
							...
							<xsl:variable name="interLen">
								<xsl:for-each
									select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData/string-length(@OpenClinica:InterviewerName)">
									<xsl:sort data-type="number" />
									<xsl:if test="position() = last()">
										<xsl:value-of select="." />
									</xsl:if>
							...
							<xsl:variable name="interStatusLen">
								<xsl:for-each
									select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData/string-length(@OpenClinica:Status)">
									<xsl:sort data-type="number" />
									<xsl:if test="position() = last()">
										<xsl:value-of select="." />
									</xsl:if>
							...
							<xsl:variable name="versionLen">
								<xsl:for-each
									select="//odm:ODM/odm:ClinicalData/odm:SubjectData/odm:StudyEventData/odm:FormData/string-length(@OpenClinica:Version)">
									<xsl:sort data-type="number" />
									<xsl:if test="position() = last()">
										<xsl:value-of select="." />
									</xsl:if>
						...
						$formRefOID]) &gt; 0">
								<xsl:if test="$interviewerNameExist">
									...
									<xsl:value-of select="$crfPosition" />
									<xsl:text> A</xsl:text>
									<!--<xsl:choose> <xsl:when test="$interLen &gt; 8"> <xsl:text>8</xsl:text> 
										</xsl:when> <xsl:otherwise> -->
								...
								<xsl:if test="$crfStatusExist">
									...
									<xsl:value-of select="$crfPosition" />
									<xsl:text> A</xsl:text>
									<!--<xsl:choose> <xsl:when test="$interStatusLen &gt; 8"> <xsl:text>8</xsl:text> 
										</xsl:when> <xsl:otherwise> -->
								...
								<xsl:if test="$crfVersionExist">
									...
									<xsl:value-of select="$crfPosition" />
									<xsl:text> A</xsl:text>
									<!--<xsl:choose> <xsl:when test="$versionLen &gt; 8"> <xsl:text>8</xsl:text> 
										</xsl:when> <xsl:otherwise> -->
		...
		<xsl:choose>
			<xsl:when test="$calledFor = 'itemDataValuesSPSS'">
				<xsl:for-each select="odm:FormRef">
					...
					<!-- @pgawade 14-Mar-2012 commented out display of text "VALUE LABELS" 
						which seems unwanted here. Probably it should be "VARIABLE LABELS" -->
					<!-- <xsl:if test="$crfPosition = 1 "> <xsl:text>.</xsl:text> <xsl:text>&#xa;</xsl:text> 
						<xsl:text>VALUE LABELS</xsl:text> <xsl:text>&#xa;</xsl:text> </xsl:if> -->
					<xsl:variable name="crfVersionExist"
	...
	<xsl:template mode="ItemGrpRefToDefTemplateForHeaders"
		...
		<xsl:param name="StudyEventRepeatKey" />
		<xsl:param name="calledFor" />
		<!--{ItemGrpRefToDefTemplateForHeaders} grpOID:<xsl:value-of select="$grpOID"/> -->
	...
	<xsl:template mode="createItemDataColForRepeatingGrps"
		...
		<xsl:param name="calledFor" />
		<!--{createItemDataColForRepeatingGrps} eventOID:*<xsl:value-of select="$eventOID"/>* 
			formOID:*<xsl:value-of select="$formOID"/>* grpOID:*<xsl:value-of select="$grpOID"/>* 
			itemGrpRepeatKey:*<xsl:value-of select="$itemGrpRepeatKey"/>* cnt21:<xsl:value-of 
		...
		<xsl:choose>
			<xsl:when test="$isEventRepeating = 'Yes'">
				<!-- <xsl:if test="count($allStudyEventDataElements[@StudyEventOID = 
					...
					name="calledFor" select="$calledFor"/> <xsl:with-param name="groupOID" select="$grpOID"/> 
					<xsl:with-param name="formOID" select="$formOID"/> </xsl:apply-templates> 
					</xsl:if> -->
				<xsl:choose>
					<xsl:when test="$isGrpRepeating = 'Yes'"><!--repeating grp -->
						...
						$formOID and ../../../@StudyEventOID = $eventOID and ../../../@StudyEventRepeatKey = $StudyEventRepeatKey]) &gt; 0"><!--create col -->
							<xsl:apply-templates
								...
								<!-- @pgawade 14-Mar-2012 Added the "calledFor" parameter to pass 
									on to print the codelists in "VALUE LABELS" section correctly -->
								<xsl:with-param name="calledFor" select="$calledFor" />
							</xsl:apply-templates>
						</xsl:if>
					</xsl:when>
			...
			<xsl:otherwise><!--{otherwise} -->
				<xsl:choose>
					<xsl:when test="$isGrpRepeating = 'Yes'"><!--repeating grp -->
						<!--eventOID:<xsl:value-of select="$eventOID"/> formOID:<xsl:value-of 
							select="$formOID"/> grpOID:<xsl:value-of select="$grpOID"/> itemOID:<xsl:value-of 
							select="$itemOID"/> cnt2: <xsl:value-of select="count($allItemDataElements[../../../@StudyEventOID 
							= $eventOID and ../../@FormOID = $formOID and ../@ItemGroupOID = $grpOID 
							and ../@ItemGroupRepeatKey = $itemGrpRepeatKey and @ItemOID = $itemOID])"/> -->
						<xsl:if
							test="count($allItemDataElements[@ItemOID = $itemOID and ../@ItemGroupOID = $grpOID and ../@ItemGroupRepeatKey =$itemGrpRepeatKey and ../../@FormOID = 
	...
	<xsl:template mode="ItemDefColHeaders2"
		...
		<!-- @pgawade 14-May-2012 #13613 prepend the 'v$' before item name if it 
			starts with some thing other then alphabet -->
		...
		<xsl:choose>
			<xsl:when test="$calledFor = 'itemDataValuesSPSS'">
				...
				<xsl:value-of
					select='replace(normalize-space($itemNameValidated), "\s", "_")' />
				<xsl:text>_</xsl:text>
				<xsl:value-of select="$E" />
				<xsl:value-of select="$eventPosition" />
				...
				<xsl:value-of select="$crfPosition" />
				<!--<xsl:text>_</xsl:text> -->
				<!--<xsl:variable name="group" select="$itemData/parent::node()" /> <xsl:variable 
					name="groupOID" select="$group/@ItemGroupOID" /> -->
				...
				<!--<xsl:variable name="codeListOID" select="./odm:CodeListRef/@CodeListOID" 
					/> <xsl:for-each select="//odm:CodeList[@OID=$codeListOID]"> <xsl:for-each 
					select="./odm:CodeListItem"> <xsl:value-of select="@CodedValue" /> <xsl:text> 
					"</xsl:text> <xsl:value-of select="./odm:Decode/odm:TranslatedText" /> <xsl:text>"</xsl:text> 
				...
				<xsl:if test="$codeListOID">
					...
					<xsl:for-each select="//odm:CodeList[@OID=$codeListOID]">
						<xsl:for-each select="./odm:CodeListItem">
							<!-- @pgawade 20-Mar-2012 Added the response option value inside single 
								quotes to work properly with SPSS tool -->
							<xsl:text>'</xsl:text>
			...
			<xsl:otherwise>
				...
				<!-- @pgawade 20-Mar-2012 #13109 Fixes to get the correct mapping between 
					OpenClinica and SPSS data types -->
				<xsl:choose>
	...
	<xsl:template name="subjectDataColumnHeaders" mode="subjectDataColumnHeaders"
		match="/odm:ODM/odm:ClinicalData/odm:SubjectData">
		...
		<xsl:variable name="valueLength">
			<xsl:for-each
				select="//odm:ODM/odm:ClinicalData/odm:SubjectData/string-length(@OpenClinica:StudySubjectID)">
				<xsl:sort data-type="number" />
				<xsl:if test="position() = last()">
					<xsl:value-of select="." />
				</xsl:if>
		...
		<xsl:variable name="valueLengthProtocolID">
			<xsl:for-each
				select="//odm:ODM/odm:Study/odm:GlobalVariables/odm:ProtocolName/string-length(.)">
				<xsl:sort data-type="number" />
				<xsl:if test="position() = last()">
					<xsl:value-of select="." />
				</xsl:if>
		...
		<xsl:text> A</xsl:text>
		<xsl:value-of select="$valueLengthProtocolID" />
		<!--<xsl:text> "Study Subject ID"</xsl:text> -->
		<xsl:text>&#xa;</xsl:text>

		<!--<xsl:text>ProtocolID</xsl:text> <xsl:text> "Protocol ID_Site ID"</xsl:text> 
			<xsl:text>&#xa;</xsl:text> -->
		<xsl:if test="$uniqueIdExist">
			<xsl:variable name="valueLength">
				<xsl:for-each
					select="//odm:ODM/odm:ClinicalData/odm:SubjectData/string-length(@OpenClinica:UniqueIdentifier)">
					<xsl:sort data-type="number" />
					<xsl:if test="position() = last()">
						<xsl:value-of select="." />
					</xsl:if>
		...
		<xsl:if test="$subjectSecondaryIdExist">
			<xsl:variable name="valueLength">
				<xsl:for-each
					select="//odm:ODM/odm:ClinicalData/odm:SubjectData/string-length(@OpenClinica:SecondaryID)">
					<xsl:sort data-type="number" />
					<xsl:if test="position() = last()">
						<xsl:value-of select="." />
					</xsl:if>
		...
		<xsl:if test="$subjectStatusExist">
			...
			<xsl:variable name="valueLength">
				<xsl:for-each
					select="//odm:ODM/odm:ClinicalData/odm:SubjectData/string-length(@OpenClinica:Status)">
					<xsl:sort data-type="number" />
					<xsl:if test="position() = last()">
						<xsl:value-of select="." />
					</xsl:if>
		...
		<xsl:if test="$sexExist">
			...
			<xsl:variable name="valueLength">
				<xsl:for-each
					select="//odm:ODM/odm:ClinicalData/odm:SubjectData/string-length(@OpenClinica:Sex)">
					<xsl:sort data-type="number" />
					<xsl:if test="position() = last()">
						<xsl:value-of select="." />
					</xsl:if>
		...
		<xsl:if test="$dobExist">
			...
			<!-- @pgawade 21-Mar-2012 #12213 - Changing data type for date of birth 
				to ADATE10 -->
			<xsl:text> ADATE10</xsl:text>
			<xsl:text>&#xa;</xsl:text>
		</xsl:if>
	...
	<!-- template to replace all invalid SPSS characters (characters other than 
		any letter, any digit, a period, or the symbols @, #, _, or $) with # This 
		is a template meant for future use to handle the special case of if SPSS 
		variables generated do not start with a letter. Associated issue number is 
		13583 -->
	...
	<xsl:template name="replace-invalid-char">
		...
		<xsl:choose>
			<xsl:when test='matches($text, "[^0-9A-Za-z.@#_$]")'>
				<xsl:value-of
					select='replace(normalize-space($text), "[^0-9A-Za-z.@#_$]", "#")' />
			</xsl:when>
			<xsl:otherwise>
