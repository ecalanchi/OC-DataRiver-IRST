...
//            roleMap.put(role.getId(), role.getDescription());
                    ...
                    if (!isLdap) {
                        if ("no".equalsIgnoreCase(displayPwd)) {
                            try {
                            	...
                            	//+DR modified by DataRiver 20/01/2015
                            	String keywords = "";
                            	StudyBean study = (StudyBean) sdao.findByPK(activeStudy);
                                ...
                                if (study.getParentStudyId() > 0) {
                                    ...
                                    keywords = parentStudy.getKeywords();
                                } else {
                                	keywords = study.getKeywords();
                                } 
                                
                            	if (keywords.toLowerCase().contains("fondazione italiana linfomi")){
                            		Boolean emailSent = sendNewUserAccountEmail(createdUserAccountBean, password);
                            		if (emailSent) sendNewUserAccountEmailNotification(createdUserAccountBean);
                            	} else {
                            		sendNewAccountEmail(createdUserAccountBean, password);
                            	}
                            	//+DR end modified by DataRiver                                
     ...
     * @author DataRiver (EC) 20/01/2015
     * 
     */
    private boolean sendNewUserAccountEmail(UserAccountBean createdUserAccountBean, String password) throws Exception {
        logger.info("Sending account creation notification to " + createdUserAccountBean.getName());
        
        boolean success = false;
            
        //get the first element: the optional study-specific email, otherwise the default email
       	DatariverEmailBean deb = getDatariverEmailDao().getNewUserDatariverEmail();
				
		String body = deb.getHtmlBody();
        body = setNewUserAccountEmailParameters(body, createdUserAccountBean, password);
        
		String emailSubject = deb.getSubject();
		emailSubject = setNewUserAccountEmailParameters(emailSubject, createdUserAccountBean, password);
		
        
		String recipients = createdUserAccountBean.getEmail();
	    	
		success = sendEmailWithReplyTo(recipients, deb.getReplyTo(), deb.getBcc(), deb.getSender(), emailSubject, body, true, deb.getAttachmentPath());
 
        //log email with filename [USERNAME]_User_new_user_and_password_yyyy-MM-dd_HHmmssS.html
    	logDatariverEmail(
    		success ? "sent" : "failed", 
    		body, 
    		deb.getEmailId(), 
    ...
    /**
     * Send email to administrators to notify new User user creation
     * @author DataRiver (EC) 20/01/2015
     * 
     */
    private boolean sendNewUserAccountEmailNotification(UserAccountBean createdUserAccountBean) throws Exception {
        
        boolean success = false;        
            
        //get the first element: the optional study-specific email, otherwise the default email
       	DatariverEmailBean deb = getDatariverEmailDao().getNewUserNotificationDatariverEmail();
				
		String body = deb.getHtmlBody();
		//don't send password to administrators
        body = setNewUserAccountEmailParameters(body, createdUserAccountBean, "");
        
		String emailSubject = deb.getSubject();
		//don't send password to administrators
		emailSubject = setNewUserAccountEmailParameters(emailSubject, createdUserAccountBean, "");
        
        String recipients = deb.getRecipients();


		success = sendEmail(recipients, deb.getBcc(), deb.getSender(), emailSubject, body, true, deb.getAttachmentPath());
		
		//log email with filename [USERNAME]_User_new_user_notice_yyyy-MM-dd_HHmmssS.html
    	logDatariverEmail(
    		success ? "sent" : "failed", 
    		body, 
    		deb.getEmailId(), 
