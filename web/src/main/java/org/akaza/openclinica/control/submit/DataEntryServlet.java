                ...
                if (!inSameSection) {// else if not in same section, progress as usual
                    ...
                    */
                    ...
                    if (markComplete && section.isLastSection()) {
                        ...
                        //+DR added by DataRiver 19/03/2012
                        ...
                        if (markSuccessfully) {
                            ...
                            if (allocationKey.equals("randomized")) {
                                ...
                                if (randomized){
                                    ...
                                    System.out.println("addPageMessage( "+respage.getString("subject_randomized")+ssb.getLabel()+subjectRandomizedArms+request);
                                	...
                                	//addPageMessage(MessageFormat.format(respage.getString("subject_not_randomized"),""), request);
                                	 //TODO: se il CRF non ̬ randomizzato randomized=false e il messaggio non ̬ coerente
                                	if (allocationKey.equals("randomized") && getRandomizationVarDao().toBeRandomized(studyEventDefinition.getId(), ecb.getCrf().getId())) {
                                	addPageMessage(MessageFormat.format(respage.getString("subject_not_randomized"),""), request);
                                	}
                                     // request.setAttribute(BEAN_DISPLAY, section);
                                  //    request.setAttribute(BEAN_ANNOTATIONS, fp.getString(INPUT_ANNOTATIONS));
                            ...
                            //+DR added by DataRiver 22/09/2011
                            ...
                            // TODO mail
                          //    DatariverEmailSender des = new DatariverEmailSender(getServletContext(), sm, ecb, eventDefinitionCRFId, currentStudy, subjectRandomizedArms);
                          //    Boolean messageSent = des.sendDatariverEmail(); 
                           //   if (messageSent) {
                         //     	addPageMessage(respage.getString("your_message_sent_succesfully"),request);
                          //    } else {
                         //     	addPageMessage(respage.getString("mail_cannot_be_sent_to_admin"),request);
                         //     }
                          ...
                          //  //+DR end added by DataRiver
                            ...
                            if (!emailArray.isEmpty()){
	                            ...
	                            while(i.hasNext()){
	                            	...
	                            	//check StudyGroupId
	                            	if (deb.getStudyGroupId() != null && Boolean.FALSE.equals(checkInRandomizationArmList(deb.getStudyGroupId(), subjectRandomizedArms))){
	                        			//DatariverEmailBean specify a StudyGroupId that is different from subject's randomizationArm: don't send email.
	                        			mailToBeSent = false; 	                          		
	                            	}
...
//		                                		                                
//		                                if (ub.getEmail() != null) deb.setRecipients(ub.getEmail() + ", " + deb.getRecipients());
