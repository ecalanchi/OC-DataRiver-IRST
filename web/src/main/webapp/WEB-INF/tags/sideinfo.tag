		...
		<c:when
			...
			<c:choose>
				<c:when test="${study.parentStudyId>0}">
                    ...
                    <a
						href="ViewStudy?id=${study.parentStudyId}&viewFull=yes">${study.parentStudyName}</a>
					<br>
					<br>
					<b>Site:</b>&nbsp;
			...
			<c:if test="${studySubject != null}">
				<b><a href="ViewStudySubject?id=${studySubject.id}"><fmt:message
							key="study_subject_ID" bundle="${resword}" /></a>:</b>&nbsp; ${studySubject.label}
...
<c:choose>
	<c:when test="${panel.orderedData}">
		<c:forEach var='line' items="${panel.userOrderedData}">
			<b><c:out value="${line.title}" escapeXml="false" /> :</b>&nbsp;
