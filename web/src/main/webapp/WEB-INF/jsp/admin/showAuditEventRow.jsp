...
<tr valign="top">
	...
	<td class="table_cell"><c:out
			value="${currRow.bean.actionMessage}" />&nbsp;</td>
	<td class="table_cell"><fmt:message
			key="${currRow.bean.auditTable}" bundle="${resaudit}" />&nbsp;</td>
	<td class="table_cell"><c:out value="${currRow.bean.studyName}" />&nbsp;</td>
	<td class="table_cell"><c:out value="${currRow.bean.subjectName}" />&nbsp;</td>
	<td class="table_cell"><c:out
			value="${currRow.bean.reasonForChange}" />&nbsp;<br />
		<table>
			<c:forEach var="rowChanges" items="${currRow.bean.changes}">
				<tr valign="top">
					<td width="45"><b><fmt:message key="${rowChanges.key}"
								bundle="${resaudit}" /></b></td>
					<td width="20"><c:out value="${rowChanges.value}" /></td>
				</tr>
			</c:forEach>
		</table></td>
