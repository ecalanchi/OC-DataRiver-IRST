...
<jsp:useBean scope='session' id='excelErrors'
...
<h1>
	<span class="title_manage"><fmt:message
			key="check_CRF_version_data" bundle="${resword}" /></span>
</h1>
...
<c:choose>
	<c:when test="${empty excelErrors}">
		<c:if test="${!empty warnings}">
			<p>
				<fmt:message key="warnings" bundle="${resword}" />
				:
			<p>
				<c:forEach var="warning" items="${warnings}">
					<span class="alert"><c:out value="${warning}" /></span>
					<br />
				</c:forEach>
		</c:if>
		<%-- Move to alerts
...
<form action="javascript:void(0)"><a href="SectionPreview?tabId=1"  target="_blank"><input type="submit" name="submit" value=
   "Preview CRF" class="button_medium"></a></form>--%>
				<td><b><form
							action="CreateCRFVersion?action=confirmsql&crfId=<c:out value=
"${version.crfId}"/>&name=<c:out value="${version.name}"/>"
							method="post">
    ...
    button is disabled after it's clicked. --%>
		...
		<jsp:include page="../managestudy/viewSectionDataPreview.jsp" /><br>
		<br>
...
"${version.crfId}"/>&name=<c:out value="${version.name}"/>"
	...
	<c:otherwise>
		<br />
		<fmt:message key="there_were_several_invalid_fields"
			bundle="${restext}" />
		<br>
		<fmt:message key="click" bundle="${resword}" />
		<input type="submit" name="submit"
			value="<fmt:message key="go_back" bundle="${restext}"/>"
			class="button"
			onclick="javascript:window.location.href='CreateCRFVersion?module=&crfId=<c:out value="${version.crfId}"/>'">
