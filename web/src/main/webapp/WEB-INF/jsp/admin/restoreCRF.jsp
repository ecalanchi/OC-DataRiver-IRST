...
<jsp:useBean scope='request' id='crfToRestore'
	class='org.akaza.openclinica.bean.admin.CRFBean' />

<h1>
	<span class="title_manage"><fmt:message
			key="confirm_restore_of_CRF" bundle="${resword}" /></span>
</h1>


<p>
	<fmt:message key="you_choose_to_restore_the_following_CRF"
		bundle="${restext}" />
	:
</p>
...
<form
	action='RestoreCRF?module=<c:out value="${module}"/>&action=submit&id=<c:out value="${crfToRestore.id}"/>'
	method="POST">
	<input type="submit" name="submit"
		value="<fmt:message key="restore_CRF" bundle="${resword}"/>"
		class="button_long"
		onClick='return confirm("<fmt:message key="if_you_restore_this_CRF" bundle="${restext}"/>");'>
</form>
