...
<jsp:useBean scope='request' id='user'
	class='org.akaza.openclinica.bean.login.UserAccountBean' />
<jsp:useBean scope='request' id='message' class='java.lang.String' />

<h1>
	<span class="title_manage"> <fmt:message key="view_user_account"
			bundle="${resword}" />
	</span>
</h1>
										...
										<table border="0" cellpadding="0" cellspacing="0" width="100%">
											...
											<tr>
												...
												<td class="table_cell"><c:out
														value="${user.institutionalAffiliation}" />&nbsp;</td>
											</tr>

											<tr>
												<td class="table_header_column"><fmt:message
														key="business_administrator" bundle="${resword}" />:</td>
												<c:choose>
													<c:when test="${user.sysAdmin}">
														<td class="table_cell"><fmt:message key="yes"
																bundle="${resword}" /></td>
													</c:when>
													<c:otherwise>
														<td class="table_cell"><fmt:message key="no"
																bundle="${resword}" /></td>
													</c:otherwise>
												</c:choose>
											</tr>
											<tr>
												<td class="table_header_column"><fmt:message
														key="technical_administrator" bundle="${resword}" />:</td>
												<c:choose>
											...
											<tr>
												...
												<td class="table_cell"><c:out
														value="${user.owner.name}" />&nbsp;</td>
											</tr>
											<tr>
												<td class="table_header_column"><fmt:message
														key="date_updated" bundle="${resword}" />:</td>
												<td class="table_cell"><fmt:formatDate
														value="${user.updatedDate}" type="date"
														pattern="${dteFormat}" />&nbsp;</td>
											</tr>
											<tr>
												<td class="table_header_column"><fmt:message
...
-->
											...
											<tr>
												<td class="table_header_column"><fmt:message
														key="roles" bundle="${resword}" />:</td>
												<td class="table_cell"><c:forEach var="studyUserRole"
														items="${user.roles}">
														<c:out value="${studyUserRole.studyName}" /> - <c:out
															value="${studyUserRole.role.description}" />
														<br />
													</c:forEach></td>
											</tr>
										</table>
...
<table border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td>
			<form action='EditUserAccount?userId=<c:out value="${user.id}" />'
				method="POST">
				<input type="submit" name="submit"
					value="<fmt:message key="edit_this_user_account" bundle="${resword}"/>"
					class="button_long">
			</form>
		</td>
		<td>
			<form action='CreateUserAccount' method="POST">
				<input type="submit" name="submit"
					value="<fmt:message key="create_a_new_user" bundle="${resword}"/>"
					class="button_long">
			</form>
		</td>
		<td>
