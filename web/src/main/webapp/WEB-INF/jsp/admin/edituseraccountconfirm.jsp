...
<h1>
	<span class="title_manage"><fmt:message
			key="edit_a_user_account" bundle="${resword}" /> - <fmt:message
			key="confirmation_screen" bundle="${resword}" /></span>
</h1>
...
<form action="EditUserAccount" method="post">
	...
	<input type="hidden" name="firstName"
		value='<c:out value="${firstName}"/>' /> <input type="hidden"
		name="lastName" value='<c:out value="${lastName}"/>' /> <input
		type="hidden" name="email" value='<c:out value="${email}"/>' /> <input
		type="hidden" name="institutionalAffiliation"
		value='<c:out value="${institutionalAffiliation}"/>' /> <input
		type="hidden" name="userType" value='<c:out value="${userType}"/>' />
	<input type="hidden" name="resetPassword"
		value='<c:out value="${resetPassword}"/>' /> <input type="hidden"
		name="displayPwd" value='<c:out value="${displayPwd}"/>' /> <input
		type="hidden" name="runWebServices"
		value='<c:out value="${runWebServices}"/>' />
				...
				<div class="box_R">
											...
											<table border="0" cellpadding="0" cellspacing="0"
												...
												<tr valign="bottom">
													...
													<td class="table_cell"><c:out
															value="${institutionalAffiliation}" /></td>
												</tr>
												...
												<tr valign="bottom">
													...
													<td class="table_cell"><c:choose>
															<c:when test="${userType == 1}">
																<fmt:message key="business_administrator"
																	bundle="${resword}" />
															</c:when>
															<c:when test="${userType == 3}">
																<fmt:message key="technical_administrator"
																	bundle="${resword}" />
															</c:when>
															<c:otherwise>
												...
												<tr>
													<td class="table_header_column"><fmt:message
															key="authorized_run_web_services" bundle="${resword}" />?</td>
													<td class="table_cell"><c:choose>
                    ...
                    <c:choose>
																	<c:when test="${displayPwd == 'no'}">
																		<fmt:message key="and_send_password_to_user_via_email"
																			bundle="${resword}" />
																	</c:when>
																	<c:otherwise>
																		<fmt:message key="and_show_password_to_system_admin"
																			bundle="${resword}" />
																	</c:otherwise>
																</c:choose>
															</c:when>
	...
	<table border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td><input type="submit" name="submit"
				value="<fmt:message key="back" bundle="${resword}"/>" class="button">
			</td>
			<td><input type="submit" name="submit"
