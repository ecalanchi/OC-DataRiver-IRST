...
<fmt:setBundle basename="org.akaza.openclinica.i18n.audit_events"
	var="resaudit" />
...
<jsp:useBean scope="request" id="currRow"
	class="org.akaza.openclinica.web.bean.AuditEventRow" />
<c:set var="dteFormat">
...
<tr valign="top">
	<td class="table_cell_left"><fmt:formatDate
			value="${currRow.bean.auditDate}" pattern="${dtetmeFormat}" /></td>
	<td class="table_cell"><c:choose>
			<c:when
				test="${currRow.bean.actionMessage == '__job_fired_success' || currRow.bean.actionMessage == '__job_fired_fail'}">
				<fmt:message key="${currRow.bean.actionMessage}"
					bundle="${resaudit}" />&nbsp;
	  </c:when>
			<c:otherwise>
	...
	<td class="table_cell"><c:choose>
			<c:when
				test="${currRow.bean.reasonForChange == '__job_fired_success' || currRow.bean.reasonForChange == '__job_fired_fail'}">
				<fmt:message key="${currRow.bean.reasonForChange}"
					bundle="${resaudit}" />&nbsp;
	  </c:when>
			<c:otherwise>
				<c:out value="${currRow.bean.reasonForChange}" />&nbsp;</c:otherwise>
		</c:choose> <%-- <table>
	...
	<td class="table_cell">
		<!-- actions go here. --> <c:import
			url="../include/showAuditEntityLink.jsp">
