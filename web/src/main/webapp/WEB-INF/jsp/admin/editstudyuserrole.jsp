...
<jsp:useBean scope='request' id='studyUserRole'
...
<h1>
	<span class="title_manage"> <fmt:message key="modify_role_for"
			bundle="${restext}">
...
<form action="EditStudyUserRole" method="post">
	...
	<input type="hidden" name="studyId"
		value="<c:out value="${studyUserRole.studyId}" />" /> <input
		type="hidden" name="userName" value="<c:out value="${userName}" />" />
											...
											<table border="0" cellpadding="0" cellspacing="0"
												...
												<tr>
													<td class="table_header_column_top"><fmt:message
															key="username2" bundle="${resword}" />:</td>
													<td class="table_cell_top"><b><c:out
																value="${userName}" /></b></td>
												</tr>
												<tr>
													...
													<td class="table_cell"><b><c:out
																value="${studyUserRole.studyName}" /></b></td>
												</tr>
												<tr>
													<td class="table_header_column"><fmt:message
																			...
																			<c:forEach var="role" items="${roles}">
																				<c:choose>
																					<c:when test="${chosenRoleId == role.key}">
																						<option value="<c:out value="${role.key}"/>"
																							selected><c:out value="${role.value}" /></option>
																					</c:when>
																					<c:otherwise>
																						<option value="<c:out value="${role.key}"/>"><c:out
