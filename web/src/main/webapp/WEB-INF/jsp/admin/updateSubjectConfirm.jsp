...
<h1>
	<span class="title_manage"><fmt:message
			key="confirm_subject_details" bundle="${resword}" /></span>
</h1>
<P>
...
<form action="UpdateSubject" method="post">
	<input type="hidden" name="action" value="submit"> <input
		type="hidden" name="uniqueIdentifier"
		value="<c:out value="${subjectToUpdate.uniqueIdentifier}"/>">
	<input type="hidden" name="studySubId"
		value="<c:out value="${studySubId}"/>"> <input type="hidden"
		name="id" value="<c:out value="${id}"/>"> <input type="hidden"
		name="gender" value="<c:out value="${subjectToUpdate.gender}"/>">
	<input type="hidden" name="localBirthDateToSave"
		value="<c:out value="${localBirthDateToSave}"/>">
										...
										<div class="textbox_center">
											<table border="0" cellpadding="0" cellspacing="0"
												...
												<c:if
													...
													<tr valign="top">
														...
														<td class="table_cell"><c:out
																value="${subjectToUpdate.uniqueIdentifier}" /></td>
													</tr>
												</c:if>
												...
												<c:if
													...
													<tr valign="top">
														<td class="table_header_column"><c:if
														...
														<td class="table_cell"><c:if
																test="${study.studyParameterConfig.collectDob=='1'}">
																<fmt:formatDate value="${subjectToUpdate.dateOfBirth}"
																	pattern="${dteFormat}" />
															</c:if> <c:if
																test="${study.studyParameterConfig.collectDob=='2'}">
																<c:out value="${localBirthDate}" />
															</c:if></td>
