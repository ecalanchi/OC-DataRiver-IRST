...
<jsp:useBean scope='session' id='itemsHaveData'
	class='java.util.ArrayList' />
<jsp:useBean scope='session' id='eventsForVersion'
	class='java.util.ArrayList' />
...
<h1>
	<span class="title_manage"><fmt:message
			...
			key="remove_previous_same_version_error" bundle="${resword}" /> </span>
</h1>
...
<c:if test="${not empty eventsForVersion}">
	...
	<span class="alert"> <fmt:message
			key="the_previous_CRF_version_has_associated_SE" bundle="${restext}">
			<fmt:param>
				<c:out value="${version.crfId}" />
			</fmt:param>
		</fmt:message>
	</span>
											...
											<table border="0" cellpadding="0" cellspacing="0"
												...
												<tr valign="top">
													<td class="table_header_column_top"><fmt:message
															key="event_ID" bundle="${resword}" /></td>
													<td class="table_header_column_top"><fmt:message
...
<c:if test="${!empty itemsHaveData}">
	<span class="alert"> <fmt:message
			key="some_items_in_the_previous_version_have_related"
			bundle="${restext}">
			<fmt:param>
				<c:out value="${version.crfId}" />
			</fmt:param>
