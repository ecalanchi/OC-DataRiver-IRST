...
<jsp:useBean scope='request' id='crfToRemove'
	class='org.akaza.openclinica.bean.admin.CRFBean' />

<h1>
	<span class="title_manage"><fmt:message
			key="confirm_removal_of_CRF" bundle="${resword}" /></span>
</h1>

<p>
	<fmt:message key="you_choose_to_remove_the_following_CRF"
		bundle="${restext}" />
	:
</p>
										...
										<table border="0" cellpadding="0" cellspacing="0" width="100%">
											...
											<c:forEach var="eventCRF" items="${eventCRFs}">
												<tr valign="top">
													...
													<td class="table_cell"><c:out
															value="${eventCRF.dateInterviewed}" /></td>
													<td class="table_cell"><c:out
...
<br>
<form
	action='RemoveCRF?action=submit&id=<c:out value="${crfToRemove.id}"/>'
	method="POST">
	<input type="submit" name="submit"
		value="<fmt:message key="remove_CRF" bundle="${resword}"/>"
		class="button_long"
		onClick='return confirm("<fmt:message key="if_you_remove_this_CRF" bundle="${restext}"/>");'>
