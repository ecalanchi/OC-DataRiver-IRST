...
<h1>
	<span class="title_manage"><fmt:message
			...
			key="check_item_names" bundle="${resword}" /></span>
</h1>

<%--this information is to be changed--%>
<fmt:message key="item_names_are_shared_across_different_versions"
	bundle="${resword}" />
<br>
<form action="CreateCRFVersion" method="POST">
	<input type="hidden" name="action" value="commitsql"> <input
		type="hidden" name="crfId" value="<c:out value="${version.crfId}"/>">
	<input type="hidden" name="name"
		value="<c:out value="${version.name}"/>">
	<c:forEach var="query" items="${openQueries}">
		<!--<font color="#3891F1"><b><c:out value="${query.key}"/>:</b></font>&nbsp;<c:out value="${query.value}"/><br/>-->
		<b><c:out value="${query.key}" /></b>
		<br />
		<br />
	</c:forEach>

	<input type="submit" name="continue"
		value="<fmt:message key="submit_items" bundle="${resword}"/>"
		class="button_xlong">
</form>
