...
<h1>
	<span class="title_manage"> <fmt:message
			key="batch_crf_version_migration_for" bundle="${resworkflow}" />&nbsp;<c:out
			value="${crf.name}" />
	</span>
</h1>
...
<form
	...
	<table cellpadding="2" cellspacing="2" border="0" class="dataTable">
		...
		<tr></tr>
		<tr>
			<td><fmt:message key="current_version_of" bundle="${resword}" />&nbsp;<c:out
					value="${crf.name}" />&#58;</td>
		...
		<tr>
			<td><fmt:message key="new_version_of" bundle="${resword}" />&nbsp;<c:out
					value="${crf.name}" />&#58;</td>
