...
<h1>
	<span class="title_manage"><fmt:message
			...
			key="version_already_exits" bundle="${resword}" /> </span>
</h1>
						...
						<div class="box_BL">
							<div class="box_BR">
								...
								<div class="alertbox_center">
									<fmt:message
										key="the_CRF_version_you_try_to_upload_already_exists"
										bundle="${restext}">
										<fmt:param>
											<c:out value="${version.crfId}" />
										</fmt:param>
									</fmt:message>
									<fmt:message key="or_you_can_change_the_version_name"
										bundle="${restext}" />
									<br>
									<br>
								</div>
											...
											<c:forEach var="def" items="${definitions}">
												<tr valign="top">
													<td class="table_cell"><c:out
															value="${def.studyEventDefinitionId}" /></td>
													<td class="table_cell"><c:out value="${def.studyId}" /></td>
													<td class="table_cell"><fmt:formatDate
															value="${def.createdDate}" pattern="${dteFormat}" /></td>
													<td class="table_cell"><c:out
															value="${def.owner.name}" /></td>
												</tr>
											</c:forEach>
