...
<jsp:useBean scope="request" id="currRow"
	class="org.akaza.openclinica.web.bean.UserAccountRow" />
						...
						<c:choose>
							<c:when test='${currRow.bean.status.deleted}'>
								<c:set var="confirmQuestion">
									<fmt:message key="are_you_sure_you_want_to_restore"
										bundle="${resword}">
								...
								<c:set var="onClick"
									value="return confirm('${confirmQuestion}');" />
								<td><a
									href="DeleteUser?action=4&userId=<c:out value="${currRow.bean.id}"/>"
									onClick="<c:out value="${onClick}" />"
									onMouseDown="javascript:setImage('bt_Restor3','images/bt_Restore_d.gif');"
									onMouseUp="javascript:setImage('bt_Restore3','images/bt_Restore.gif');">
										<img name="bt_Restore3" src="images/bt_Restore.gif" border="0"
							...
							<c:otherwise>
								<td><a
									href="ViewUserAccount?userId=<c:out value="${currRow.bean.id}"/>"
								...
								<td><a
									href="EditUserAccount?userId=<c:out value="${currRow.bean.id}"/>"
									...
									onMouseUp="javascript:setImage('bt_Edit1','images/bt_Edit.gif');"><img
										...
										align="left" hspace="6"></a></td>
								<td><a
									href="SetUserRole?action=confirm&userId=<c:out value="${currRow.bean.id}"/>"
									...
									onMouseUp="javascript:setImage('bt_SetRole1','images/bt_SetRole.gif');"><img
										name="bt_SetRole1" src="images/bt_SetRole.gif" border="0"
										alt="<fmt:message key="set_role" bundle="${resword}"/>"
										title="<fmt:message key="set_role" bundle="${resword}"/>"
										align="left" hspace="6"></a></td>

								<c:set var="confirmQuestion">
									<fmt:message key="are_you_sure_you_want_to_remove"
										bundle="${resword}">
										<fmt:param value="${currRow.bean.name}" />
									</fmt:message>
								</c:set>
								...
								<c:set var="onClick"
									value="return confirm('${confirmQuestion}');" />
								<td><a
									href="DeleteUser?action=3&userId=<c:out value="${currRow.bean.id}"/>"
									onClick="<c:out value="${onClick}" />"
									onMouseDown="javascript:setImage('bt_Remove1','images/bt_Remove_d.gif');"
									onMouseUp="javascript:setImage('bt_Remove1','images/bt_Remove.gif');">
										<img name="bt_Remove1" src="images/bt_Remove.gif" border="0"
								...
								<c:if test='${currRow.bean.status.locked}'>
									<td><a
										href="UnLockUser?userId=<c:out value="${currRow.bean.id}"/>"
										...
										onMouseUp="javascript:setImage('bt_Unlock1','images/bt_Unlock.gif');"><img
											...
											alt="<fmt:message key="unlock" bundle="${resword}"/>"
											title="<fmt:message key="unlock" bundle="${resword}"/>"
											align="left" hspace="6"></a></td>
								</c:if>
							</c:otherwise>
						</c:choose>
...
<c:choose>
	<c:when test="${empty currRow.bean.roles}">
		<tr valign="top">
			...
			<td class="table_cell" colspan="3"><i><fmt:message
						key="no_roles_assigned" bundle="${resword}" /></i></td>
	...
	<c:otherwise>
		<c:forEach var="sur" items="${currRow.bean.roles}">
			...
			<tr valign="top">
				<td class="table_cell_left">&nbsp;</td>
				<td class="table_cell" colspan="3"><c:if
						test='${sur.status.deleted}'>
						<font color='gray'>
					</c:if> <c:choose>
						<c:when test='${sur.studyName != ""}'>
							<c:out value="${sur.studyName}" />
						</c:when>
						<c:otherwise>Study <c:out value="${sur.studyId}" />
						</c:otherwise>
					</c:choose> - <c:if test="${sur.parentStudyId > 0}">
						<fmt:message key="${siteRoleMap[sur.role.id] }"
							bundle="${resterm}"></fmt:message>
					</c:if> <c:if test="${sur.parentStudyId == 0}">
						<fmt:message key="${studyRoleMap[sur.role.id] }"
							bundle="${resterm}"></fmt:message>
					</c:if> <c:if test='${sur.status.deleted}'>
						</font>
					</c:if></td>
				<td class="table_cell"><c:if test='${!sur.status.deleted}'>
						<a
							href="EditStudyUserRole?studyId=<c:out value="${sur.studyId}" />&userName=<c:out value="${currRow.bean.name}"/>"
							onMouseDown="javascript:setImage('bt_Edit1','images/bt_Edit_d.gif');"
							onMouseUp="javascript:setImage('bt_Edit1','images/bt_Edit.gif');"><img
							name="bt_Edit1" src="images/bt_Edit.gif" border="0"
							alt="<fmt:message key="edit" bundle="${resword}"/>"
						...
						<c:when test='${sur.status.deleted}'>
							<a
								href="DeleteStudyUserRole?studyId=<c:out value="${sur.studyId}" />&userName=<c:out value="${currRow.bean.name}"/>&action=4"
								onClick="<c:out value="${onClick}" />"
								onMouseDown="javascript:setImage('bt_Restor3','images/bt_Restore_d.gif');"
						...
						<c:otherwise>
							<a
								href="DeleteStudyUserRole?studyId=<c:out value="${sur.studyId}" />&userName=<c:out value="${currRow.bean.name}"/>&action=3"
								onClick="<c:out value="${onClick}" />"
								onMouseDown="javascript:setImage('bt_Remove3','images/bt_Remove_d.gif');"
								onMouseUp="javascript:setImage('bt_Remove3','images/bt_Remove.gif');"><img
								name="bt_Remove3" src="images/bt_Remove.gif" border="0"
								alt="<fmt:message key="remove" bundle="${resword}"/>"
								title="<fmt:message key="remove" bundle="${resword}"/>"
								align="left" hspace="6"></a>
						</c:otherwise>
