	...
	<table border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td valign="bottom" id="Tab1'">
                ...
                <div id="Tab1NotSelected" style="display:none"><div class="tab_BG_h"><div class="tab_L_h"><div class="tab_R_h"><span class="tabtext"><fmt:message key="view_by_studysubjectID" bundle="${resword}"/></span></div></div></div></div>--%>
			</td>
				...
				<div id="Tab2NotSelected">
					<div class="tab_BG">
						<div class="tab_L">
							<div class="tab_R">
								<a class="tabtext"
									title="<fmt:message key="view_by_event_CRF" bundle="${resword}"/>"
									href='viewAllSubjectSDVtmp?studyId=${studyId}'
									onclick="javascript:HighlightTab(2);"><fmt:message
										key="view_by_event_CRF" bundle="${resword}" /></a>
							</div>
						</div>
					</div>
				</div>
				<div id="Tab2Selected" style="display: none">
					<div class="tab_BG_h">
						<div class="tab_L_h">
							<div class="tab_R_h">
								<span class="tabtext"><fmt:message
										key="view_by_event_CRF" bundle="${resword}" /></span>
							</div>
						</div>
					</div>
				</div>
							...
							<div class="box_BL">
								<div class="box_BR">
									<div class="textbox_center">
										<form method="GET" action="viewAllSubjectSDVform"
											name="sdvFilterForm">
											<%--<input type="hidden" name="srch" value="y" />--%>
											<input type="hidden" name="studyId" value="${studyId}" />
											<table border="0" cellpadding="0" cellspacing="0">
												<tr valign="top">
													<td><b><fmt:message key="filter_events_by"
																bundle="${resword}" />:</b></td>
												</tr>
												<tr valign="top">
													<td><fmt:message key="study_subject"
															bundle="${resword}" />:</td>
													<td><div class="formfieldL_BG"
															style="margin-right: 10px">
															<input type="text" name="study_subject_id"
																class="formfieldS" />
														</div></td>
													...
													<td>
														<div class="formfieldM_BG" style="margin-right: 10px">
															...
															<select name="eventCRF" class="formfieldM">
																<option value="0">--
																	<fmt:message key="all" bundle="${resword}" />--
																</option>
													...
													<td>
														<div class="formfieldM_BG">
															...
															<select name="studyEventDefinition" class="formfieldM">
																...
																<c:forEach var="studyEventDefinition"
																	items="${studyEventDefinitions}">
																	<option
																		value="<c:out value="${studyEventDefinition.id}"/>"><c:out
																			value="${studyEventDefinition.name}" />
																</c:forEach>
															</select>
														</div>
													</td>
												...
												<tr valign="top">
													<td><fmt:message key="study_event_status"
															bundle="${resword}" />:</td>
													<td>
														<div class="formfieldM_BG" style="margin-right: 10px">
															<%--<c:set var="status1" value="${statusId}"/>--%>
															<select name="studyEventStatus" class="formfieldM">
																...
																<c:forEach var="studyEventStatus"
																	items="${studyEventStatuses}">
																	<option value="<c:out value="${studyEventStatus.id}"/>"><c:out
																			value="${studyEventStatus.name}" />
																</c:forEach>
															</select>
														</div>
													</td>
													<td><fmt:message key="event_crf_status"
															bundle="${resword}" />:</td>
													<td>
														<div class="formfieldM_BG" style="margin-right: 10px">
															<%--<c:set var="status1" value="${statusId}"/>--%>
															<select name="eventCRFStatus" class="formfieldM">
																...
																<c:forEach var="eventCRFStatus"
																	items="${eventCRFDStatuses}">
																	<option value="<c:out value="${eventCRFStatus.id}"/>"><c:out
																			value="${eventCRFStatus.name}" />
																</c:forEach>
															</select>
														</div>
													</td>
													<td><fmt:message key="event_crf_sdv_status"
															bundle="${resword}" />:</td>
													<td>
														<div class="formfieldM_BG">
															<%--<c:set var="status1" value="${statusId}"/>--%>
															<select name="eventcrfSDVStatus" class="formfieldM">
																<option value="N/A"><fmt:message key="N/A"
																		bundle="${resword}" /></option>
																<option value="None"><fmt:message key="none"
																		bundle="${resword}" /></option>
																<option value="Complete"><fmt:message
																		key="complete" bundle="${resword}" /></option>
															</select>
														</div>
													</td>
												</tr>
												<tr valign="top">
													<td><fmt:message key="event_crf_sdv_require"
															bundle="${resword}" />:</td>
													<td>
														<div class="formfieldM_BG" style="margin-right: 10px">
															<%--<c:set var="status1" value="${statusId}"/>--%>
															<select name="sdvRequirement" class="formfieldM">
																...
																<c:forEach var="sdvRequirement"
																	items="${sdvRequirements}">
																	<option value="<c:out value="${sdvRequirement.code}"/>"><c:out
																			value="${sdvRequirement.description}" />
																</c:forEach>
															</select>
														</div>
													</td>
													<td><fmt:message key="event_crf_date_updated"
															bundle="${resword}" />:</td>
													<td>
														<div class="formfieldS_BG" style="width: 157px">
															<input type="text" name="startUpdatedDate"
																value="${startUpdatedDate}" class="formfieldS"
																id="startUpdatedDateField"><A HREF="#"><img
                                        ...
                                        Calendar.setup({inputField  : "startUpdatedDateField", ifFormat    : "<fmt:message key="date_format_calender" bundle="${resformat}"/>", button      : "startDateTrigger" });
													...
													<td><div>
															<b><fmt:message key="To" bundle="${resword}" /></b>
														</div></td>
													<td><div class="formfieldS_BG" style="width: 157px">
															<input type="text" name="endDate" value="${endDate}"
																class="formfieldS" id="endDateField"> <A
																HREF="#"> <img src="../images/bt_Calendar.gif"
																alt="<fmt:message key="show_calendar" bundle="${resword}"/>"
																title="<fmt:message key="show_calendar" bundle="${resword}"/>"
																border="0" id="startDateTriggerB" /> <script
																	type="text/javascript">
...
<div id="subjectSDV">
	<form name='sdvForm'
		action="${pageContext.request.contextPath}/pages/viewAllSubjectSDVform">
		<fmt:message key="select_all_on_page" bundle="${resword}" />
		<input type=checkbox name='checkSDVAll'
			onclick='selectAllChecks(this.form)' /> <br /> <input type="hidden"
			name="studyId" value="${param.studyId}"> <input type="hidden"
			name="study_subject_id" value="${param.study_subject_id}"> <input
			type="hidden" name="eventCRF" value="${param.eventCRF}"> <input
			type="hidden" name="studyEventDefinition"
			value="${param.studyEventDefinition}"> <input type="hidden"
			name="studyEventStatus" value="${param.studyEventStatus}"> <input
			type="hidden" name="eventCRFStatus" value="${param.eventCRFStatus}">
		<input type="hidden" name="eventcrfSDVStatus"
			value="${param.eventcrfSDVStatus}"> <input type="hidden"
			name="sdvRequirement" value="${param.sdvRequirement}"> <input
			type="hidden" name="startUpdatedDate"
			value="${param.startUpdatedDate}"> <input type="hidden"
		...
		${sdvTableAttribute} <br /> <input type="submit"
			name="sdvAllFormSubmit" class="button_medium"
		...
		<input type="submit" name="sdvAllFormCancel" class="button_medium"
