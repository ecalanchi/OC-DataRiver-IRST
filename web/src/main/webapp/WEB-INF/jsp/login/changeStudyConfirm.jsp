...
<jsp:useBean scope="session" id="studyWithRole"
...
<h1>
	<span class="title_manage"><fmt:message
			key="confirm_changing_study" bundle="${resword}" /></span>
</h1>
...
<form action="ChangeStudy" method="post">
	...
	<p>
		<fmt:message key="choosing_switch_to_study" bundle="${restext}" />
		:
		<c:out value="${studyWithRole.studyName}" />
		<fmt:message key="with_a_role_of" bundle="${restext}" />
		<c:out value="${studyWithRole.role.description}" />
		.
		<fmt:message key="the_study_site_status_is" bundle="${restext}" />
		<c:out value="${currentStudy.status.name}" />
	</p>
