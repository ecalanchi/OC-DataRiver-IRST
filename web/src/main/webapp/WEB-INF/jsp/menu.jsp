...
<div id="box" class="dialog">
	<span id="mbm"> <br> <fmt:message
			key="study_frozen_locked_note" bundle="${restext}" />
	</span><br>
...
<h1>
	<span class="title_manage" style="line-height: 5px;"> <fmt:message
		...
		href="javascript:openDocWindow('https://docs.openclinica.com/3.1/openclinica-user-guide/overview-openclinica')">
...
<span class="table_title_Admin" style="line-height: 15px;"> <a
	href="ViewNotes?module=submit&listNotes_f_discrepancyNoteBean.user=<c:out value='${userBean.name}' />"><fmt:message
			key="notes_assigned_to_me" bundle="${restext}" /><span>${assignedDiscrepancies}</span>&nbsp;</a><br />
<br />
</span>

<c:if
	test="${userRole.investigator || userRole.researchAssistant || userRole.researchAssistant2}">
...
<c:if test="${userRole.coordinator || userRole.director}">
    ...
    function onInvokeAction(id,action) {
      ...
      //+DR added by DataRiver 06/04/2012
        if(id.indexOf('randomizationStatusStatistics') == -1)  {
             setExportToLimit(id, '');
        }
       //+DR end added by DataRiver
		...
		<tr>
			<td valign="top">
				<form action="${pageContext.request.contextPath}/MainMenu">
					${studySiteStatistics}</form>
			</td>
			<td valign="top">
				<form action="${pageContext.request.contextPath}/MainMenu">
					${studyStatistics}</form>
			</td>
		</tr>
	</table>
	...
	<table>
		<tr>
			<td valign="top">
				<form action="${pageContext.request.contextPath}/MainMenu">
					${subjectEventStatusStatistics}</form>
			</td>

			<td valign="top">
				<form action="${pageContext.request.contextPath}/MainMenu">
					${studySubjectStatusStatistics}</form>
			</td>
		</tr>
	</table>
...
<c:if test="${userRole.monitor}">
	...
	<div id="subjectSDV">
		<form name='sdvForm'
			action="${pageContext.request.contextPath}/pages/viewAllSubjectSDVtmp">
			<input type="hidden" name="studyId" value="${study.id}"> <input
				type="hidden" name=imagePathPrefix value="">
			...
			${sdvMatrix} <br /> <input type="submit" name="sdvAllFormSubmit"
				class="button_medium"
