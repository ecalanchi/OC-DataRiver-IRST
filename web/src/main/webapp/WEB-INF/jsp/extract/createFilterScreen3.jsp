...
<h1>
	<span class="title_manage"><fmt:message key="create_filter"
			bundle="${resword}" />: <fmt:message key="specify_parameters"
			bundle="${resword}" /> <a
...
<form action="CreateFiltersTwo" method="post" name="cf2">
	<input type="hidden" name="action" value="crfselected" /> &nbsp;<b><fmt:message
			key="CRF" bundle="${resword}" /></b>&nbsp;&nbsp; <select name="crfId">
		<option value="0" selected>--
			<fmt:message key="select_CRF" bundle="${resworkflow}" /> --
		</option>
		...
		value="<fmt:message key="select_crf_and_continue" bundle="${resword}"/>"
		class="button_xlong" />
</form>
