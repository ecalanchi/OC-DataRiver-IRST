...
<table><tr><td class="title_extract">Page loading ... Please wait.</td></tr></table>
	...
	<p>
		<b><fmt:message key="dataset_name" bundle="${resword}" />:</b>
		<c:out value="${dataset.name}" />
		<br> <b><fmt:message key="description" bundle="${resword}" />:</b>
...
--%>
														...
														<table border="0" cellpadding="0" cellspacing="0"
															...
															<tr valign="top">
																<td class="table_header_row"><fmt:message
																		key="file_name" bundle="${resword}" /></td>
																<td class="table_header_row"><fmt:message
																		key="run_time" bundle="${resword}" /></td>
																<td class="table_header_row"><fmt:message
																		key="file_size" bundle="${resword}" /></td>
																<td class="table_header_row"><fmt:message
																		key="created_date" bundle="${resword}" /></td>
																<td class="table_header_row"><fmt:message
															...
															<c:choose>
																...
																<c:otherwise>
																	...
																	<c:forEach var="row" items="${table.rows}">
																		...
																		<tr>
																			<td class="table_cell"><a target="_new"
																				href="AccessFile?fileId=<c:out value="${currRow.bean.id}"/>">
																					<c:out value="${currRow.bean.name}" />
																			</a></td>
																			<td class="table_cell"><c:out
																					value="${currRow.bean.runTime}" /></td>
																			<td class="table_cell"><c:out
																					value="${currRow.bean.fileSize}" /></td>
																			<td class="table_cell"><fmt:formatDate
																					value="${currRow.bean.dateCreated}"
	...
	<p>
		<i><fmt:message key="internet_explorer_users" bundle="${resword}" /></i>:
		<fmt:message key="export_dataset_download4" bundle="${restext}" />
		<a
			href="javascript:openDocWindow('https://docs.openclinica.com/3.1/openclinica-user-guide/export-datasets')"><img
			src="images/bt_Help_Manage.gif" border="0"
			alt="<fmt:message key="help" bundle="${resword}"/>"
			title="<fmt:message key="help" bundle="${resword}"/>"></a>
		<fmt:message key="export_dataset_download5" bundle="${restext}" />
