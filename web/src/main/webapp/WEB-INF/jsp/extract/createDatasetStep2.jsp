	...
	<c:when test="${newDataset.id>0}">
		<h1>
			<span class="title_manage"> <fmt:message key="edit_dataset"
					bundle="${resword}" /> - <fmt:message key="select_items"
					bundle="${resword}" /> <c:choose>
					<c:when test="${newDataset.id<=0}">
						<a
							href="javascript:openDocWindow('https://docs.openclinica.com/3.1/openclinica-user-guide/create-dataset')">
					...
					<c:otherwise>
						<a
							href="javascript:openDocWindow('https://docs.openclinica.com/3.1/openclinica-user-guide/edit-dataset')">
							<img src="images/bt_Help_Manage.gif" border="0"
							alt="<fmt:message key="help" bundle="${resword}"/>"
							title="<fmt:message key="help" bundle="${resword}"/>">
						</a>
					</c:otherwise>
				</c:choose> : <c:out value="${newDataset.name}" />
			</span>
		</h1>
	</c:when>
	<c:otherwise>
...
<p>
	<fmt:message key="please_select_one_CRF_from_the" bundle="${restext}" />
	<b><fmt:message key="left_side_info_panel" bundle="${restext}" /></b>
	<fmt:message key="select_items_in_CRF_include_dataset"
		bundle="${restext}" />
</p>
<p>
	<fmt:message key="click_event_subject_attributes_specify"
		bundle="${restext}" />
</p>
<table border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td><img src="images/arrow_left.gif"
			alt="<fmt:message key="select_CRF_on_the_left" bundle="${restext}"/>"
			title="<fmt:message key="select_CRF_on_the_left" bundle="${restext}"/>"></td>
		<td>
			<div class="box_T">
				<div class="box_L">
					<div class="box_R">
									...
									<div class="box_BL">
										<div class="box_BR">
											<div class="textbox_center" align="center">
												<span class="title_extract"> <b><fmt:message
															key="use_task_pane_to_select_CRF" bundle="${restext}" /></b>
												</span>
											</div>
											...
											<table border="0" cellpadding="0" cellspacing="0"
												...
												<tr valign="top">
													<td class="table_header_column"><fmt:message
															key="event_name" bundle="${resword}" />:</td>
...
<c:if test="${!empty allItems}">
	<form action="CreateDataset" method="post" name="cl">
		<input type="hidden" name="action" value="beginsubmit" /> <input
			type="hidden" name="crfId" value="<c:out value="${crf.id}"/>">
		<input type="hidden" name="defId"
			value="<c:out value="${definition.id}"/>">

		<jsp:include page="../showMessage.jsp"><jsp:param
				name="key" value="all" /></jsp:include>
		<table border="0" cellpadding="0" cellspacing="0">
			<tr>
				<td><input type="checkbox" name="all" value="1"
					onClick="javascript:selectAll();"> <fmt:message
						key="select_all_items" bundle="${resword}" />&nbsp;&nbsp;&nbsp;</td>
				<td><input type="submit" name="save"
					value="<fmt:message key="save_and_add_more_items" bundle="${resword}"/>"
					class="button_xlong" /></td>
				<td><input type="submit" name="saveContinue"
					value="<fmt:message key="save_and_define_scope" bundle="${resword}"/>"
					class="button_xlong" /></td>
		...
		<br />
		<P>
			<B><fmt:message key="show_items_this_dataset" bundle="${restext}" /></b>
		</p>
											...
											<div class="textbox_center" align="center">
												...
												<table border="0" cellpadding="0" cellspacing="0"
													...
													<tr>
														...
														<td class="table_header_column_top"><fmt:message
																key="section_s" bundle="${resword}" /></td>
														<td class="table_header_column_top"><fmt:message
																key="group_s" bundle="${resword}" /></td>
														<td class="table_header_column_top"><fmt:message
																key="data_type" bundle="${resword}" /></td>
														<td class="table_header_column_top"><fmt:message
																key="units" bundle="${resword}" /></td>
														<td class="table_header_column_top"><fmt:message
																key="response_type" bundle="${resword}" /></td>
														<td class="table_header_column_top"><fmt:message
														...
														<td class="table_header_column_top"><fmt:message
																key="default_value" bundle="${resword}" /></td>
														<td class="table_header_column_top"><fmt:message
																key="max_repeats" bundle="${resword}" /></td>
															...
															<td class="table_cell"><c:choose>
																	<c:when test="${item.selected}">
																		<input type="checkbox"
																			...
																			value="yes" onclick="javascript:notSelectAll();">
																	</c:when>
																	<c:otherwise>
																		<input type="checkbox"
																			name="itemSelected<c:out value="${count}"/>"
																			value="yes" onclick="javascript:notSelectAll();">
																	</c:otherwise>
																</c:choose></td>
															<td class="table_cell"><a
																href="javascript: openDocWindow('ViewItemDetail?itemId=<c:out value="${item.id}"/>')"><c:out
   ...
   --%>
															...
															<td class="table_cell"><c:forEach var="meta"
																	...
																	<c:choose>
																		<c:when
																			test="${meta.groupLabel==null || meta.groupLabel==''}"></c:when>
																		<c:when test="${status.last}">
        	...
        	No
															...
															<td class="table_cell"><c:forEach var="meta"
																	...
																	<c:choose>
																		<c:when
																			test="${meta.defaultValue==null || meta.defaultValue==''}"></c:when>
																		<c:when test="${status.last}">
