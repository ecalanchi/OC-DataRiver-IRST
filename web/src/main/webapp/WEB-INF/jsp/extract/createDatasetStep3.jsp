...
<tr id="sidebar_Instructions_closed" style="display: all">
	<td class="sidebar_tab"><a
		...
		<div class="sidebar_tab_content">
			<p>
				<fmt:message key="filter_data_selecting_range" bundle="${restext}" />
			</p>
			<p>
				<fmt:message key="select_event_start_end_however"
					bundle="${restext}" />
			</p>
			<p>
				<fmt:message key="not_filter_study_by_enrollment_leave_blank"
					bundle="${restext}" />
			</p>
		</div></td>
</tr>
...
<p>
	<fmt:message key="select_event_start_end_however" bundle="${restext}" />
</p>
...
<form action="CreateDataset" method="post">
	...
	<table>
		<tr>
			<td class="text"><fmt:message key="beginning_date"
					bundle="${resword}" />:</td>
			<td class="text" valign="top"><select name="firstmonth">
					<option value="0">-
					...
					<c:forEach var="month" items="${months}">
						<c:choose>
							<c:when test="${firstMonth == monthNum}">
								<option value='<c:out value="${monthNum}" />' selected><c:out
										value="${month}" /></option>
							</c:when>
							<c:otherwise>
								<option value='<c:out value="${monthNum}" />'><c:out
										value="${month}" /></option>
							</c:otherwise>
						</c:choose>
						<c:set var="monthNum" value="${monthNum + 1}" />
					</c:forEach>
			</select> <br><jsp:include page="../showMessage.jsp"><jsp:param
						name="key" value="firstmonth" /></jsp:include></td>
			<td class="text" valign="top"><select name="firstyear">
					<option value="1900">-
						<fmt:message key="select" bundle="${resword}" />-
					</option>
					...
					<c:forEach var="year" items="${years}">
						<c:choose>
							<c:when test="${firstYear == yearNum}">
								<option value='<c:out value="${year}" />' selected><c:out
										value="${year}" /></option>
							</c:when>
							<c:otherwise>
		...
		<tr>
			<td class="text" align="right"><fmt:message key="end_date"
					bundle="${resword}" />:</td>
			<td class="text" valign="top"><select name="lastmonth">
					<option value="0">-
						<fmt:message key="select" bundle="${resword}" />-
					</option>
					<c:set var="monthNum" value="${1}" />
					<c:forEach var="month" items="${months}">
						...
						name="key" value="lastmonth" /></jsp:include></td>
			<td class="text" valign="top"><select name="lastyear">
					<option value="2100">-
						<fmt:message key="select" bundle="${resword}" />-
					</option>
					<c:set var="yearNum" value="${1980}" />
	...
	<p>
		<fmt:message key="save_export_dataset_or_apply_filter"
			bundle="${restext}" />
	</p>
	<table>
		<tr>
