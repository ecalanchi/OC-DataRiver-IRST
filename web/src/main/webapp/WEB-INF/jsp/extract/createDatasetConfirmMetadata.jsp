...
<jsp:useBean scope='session' id='newDataset'
	...
	<c:when test="${newDataset.id>0}">
		<h1>
			<span class="title_manage"><fmt:message key="edit_dataset"
					...
					key="confirm_dataset_properties" bundle="${resword}" /> : <c:out
	...
	<c:otherwise>
		<h1>
			<span class="title_manage"><fmt:message key="create_dataset"
					bundle="${resword}" />: <fmt:message
					key="confirm_dataset_properties" bundle="${resword}" /></span>
		</h1>
	</c:otherwise>
</c:choose>
...
--%>
<p>
	<fmt:message key="confirm_dataset_properties" bundle="${restext}" />
<form action="CreateDataset" method="post">
	...
	<table>
		...
		<tr>
			<td class="text" valign="top"><fmt:message key="description"
					bundle="${resword}" /></td>
			<td class="text" valign="top"><b><c:out
						value="${newDataset.description}" /></b></td>
		</tr>
		<%--<tr>
