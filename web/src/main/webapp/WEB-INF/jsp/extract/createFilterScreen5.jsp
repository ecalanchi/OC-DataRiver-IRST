...
<h1>
	<span class="title_manage"><fmt:message key="create_filter"
			bundle="${resword}" />: <fmt:message key="specify_filter_properties"
			bundle="${resword}" /> <a
...
<c:if test='${newExp != null}'>
	<P>
		<fmt:message key="generated_filter" bundle="${resword}" />
		:
		<c:forEach var='str' items='${newExp}'>
			<c:out value='${str}' />
		</c:forEach>
	</P>
</c:if>
<P>
	<fmt:message key="please_enter_all_the_filter" bundle="${restext}" />
	<font color="red"><fmt:message key="all_fields_are_required"
			bundle="${resword}" /></font>
</P>
<form action="CreateFiltersThree">
	...
	<table>
		...
		<tr>
			...
			<td><textarea name="fDesc" cols="40" rows="4"><c:out
						value="${fDesc}" /></textarea></td>
		</tr>
		<tr>
			<td><b><fmt:message key="status" bundle="${resword}" />:</b></td>
			<td><select name="fStatusId">
					<option value="0">--
						<fmt:message key="select_status" bundle="${resword}" /> --
					</option>
	...
	<P>
		<input type="submit"
			value="<fmt:message key="save_filter" bundle="${resword}"/>"
			class="button_xlong" />
</form>
