...
<jsp:useBean scope='session' id='subjectData'
	class='java.util.ArrayList' />
<jsp:useBean scope='session' id='validationErrors'
	class='java.util.HashMap' />
<jsp:useBean scope='session' id='hardValidationErrors'
	class='java.util.HashMap' />
<jsp:useBean scope='session' id='summaryStats'
	class='org.akaza.openclinica.bean.submit.crfdata.SummaryStatsBean' />
										...
										<table border="0" cellpadding="0" cellspacing="0" width="100%">
											...
											<tr valign="top">
												<td class="table_cell_left"><fmt:message
														key="subjects_affected" bundle="${resword}" />: <c:out
														value="${summaryStats.studySubjectCount}" /></td>
											</tr>
											<tr valign="top">
												<td class="table_cell_left"><fmt:message
														key="event_CRFs_affected" bundle="${resword}" />: <c:out
														value="${summaryStats.eventCrfCount}" /></td>
											</tr>
											<c:if test="${empty hardValidationErrors}">
												<tr valign="top">
													<td class="table_cell_left"><fmt:message
															key="event_CRFs_available" bundle="${resword}" />: <c:out
															value="${summaryStats.eventCrfCount - summaryStats.skippedCrfCount}" /></td>
												</tr>
												<tr valign="top">
													<td class="table_cell_left"><fmt:message
															key="event_CRFs_skipped" bundle="${resword}" />: <c:out
															value="${summaryStats.skippedCrfCount}" /></td>
												</tr>
											</c:if>
											<tr valign="top">
												<td class="table_cell_left"><fmt:message
														key="validation_rules_generated" bundle="${resword}" />: <c:out
														value="${summaryStats.discNoteCount}" /></td>
											</tr>
								...
								<div class="box_BR">
									<div class="textbox_center">
										<table border="0" cellpadding="0" cellspacing="0" width="100%">
											<tr>
												<td class="table_header_row"><fmt:message
														key="study_oid" bundle="${resword}" /></td>
												<td class="table_header_row"><fmt:message
												...
												<td class="table_header_row"><fmt:message
														key="event_crf_status" bundle="${resword}" /></td>
											</tr>
											<c:forEach items="${importCrfInfo.importCRFList}"
												var="importCRF">
												<c:if test="${!importCRF.processImport}">
													<tr>
														<td class="table_cell_left"><c:out
																value="${importCRF.studyOID}" /></td>
														<td class="table_cell"><c:out
																value="${importCRF.studySubjectOID}" /></td>
														<td class="table_cell"><c:out
																value="${importCRF.studyEventOID}" /></td>
														<td class="table_cell"><c:out
																value="${importCRF.formOID}" /></td>
														<td class="table_cell"><c:choose>
																<c:when test="${importCRF.preImportStage.initialDE}">
																	<fmt:message key="initial_data_entry"
																		bundle="${resword}" />
																</c:when>
																<c:when
																	test="${importCRF.preImportStage.initialDE_Complete}">
																	<fmt:message key="initial_data_entry_complete"
																		bundle="${resword}" />
																</c:when>
																<c:when test="${importCRF.preImportStage.doubleDE}">
																	<fmt:message key="double_data_entry"
																		bundle="${resword}" />
																</c:when>
																<c:when
																	test="${importCRF.preImportStage.doubleDE_Complete}">
																	<fmt:message key="data_entry_complete"
																		bundle="${resword}" />
																</c:when>
																<c:when test="${importCRF.preImportStage.admin_Editing}">
																	<fmt:message key="administrative_editing"
																		bundle="${resword}" />
																</c:when>
																<c:when test="${importCRF.preImportStage.locked}">
...
<c:choose>
	<c:when test="${not empty hardValidationErrors}">
					...
					<div class="box_R">
						<div class="box_B">
														...
														<c:forEach var="studyEventData"
															...
															<tr valign="top">
																<td class="table_cell_left"><c:out
																		...
																		<c:when
																			test="${studyEventData.studyEventRepeatKey != null}">
		    				...
		    				<c:set var="studyEventRepeatKey"
															...
															<c:forEach var="formData"
																items="${studyEventData.formData}">
																<tr valign="top">
																...
																<tr valign="top">
																	...
																	<td class="table_cell"><c:out
																			value="${formData.formOID}" /></td>
																	<td class="table_cell" colspan="2"></td>
																</tr>
																<c:forEach var="itemGroupData"
																	items="${formData.itemGroupData}">
																	<tr valign="top">
																		<td class="table_header_row"></td>
																		<td class="table_header_row"></td>
																		<td class="table_header_row" colspan="2"><c:out
																				...
																				<c:when
																					test="${itemGroupData.itemGroupRepeatKey != null}">
				    				(<fmt:message key="repeated_key" bundle="${resword}">
																						<fmt:param>
																							<c:out
																		...
																		<c:if test="${not empty hardValidationErrors[oidKey]}">
																			<tr valign="top">
																				...
																				<td class="table_cell"><c:out
																						...
																						value="${hardValidationErrors[oidKey]}" /></td>
																			</tr>
																		</c:if>
																	</c:forEach>
																</c:forEach>
		...
		<form
			action="ImportCRFData?action=confirm&crfId=<c:out value="${version.crfId}"/>&name=<c:out value="${version.name}"/>"
													...
													<table border="0" cellpadding="0" cellspacing="0">
														...
														<tr>
															<td class="formlabel"><fmt:message
																	key="xml_file_to_upload" bundle="${resterm}" />:</td>
															<td>
																<div class="formfieldFile_BG">
																	<input type="file" name="xml_file">
																</div> <br><jsp:include page="../showMessage.jsp"><jsp:param
																		name="key" value="xml_file" /></jsp:include>
															</td>
														</tr>
			...
			<br clear="all"> <input type="submit"
						...
						<div class="box_R">
							<div class="box_B">
											...
											<div class="box_BR">
												<div class="textbox_center">
													<table border="0" cellpadding="0" cellspacing="0"
														...
														<c:forEach var="subjectDataBean" items="${subjectData}">
															<c:set var="currSubjectMap"
																value="${importCrfInfo.importCRFMap[subjectDataBean.subjectOID]}" />
															<c:if test="${currSubjectMap != null}">
																...
																<c:forEach var="studyEventData"
																	items="${subjectDataBean.studyEventData}">
																	<c:set var="currEventMap"
																		value="${currSubjectMap[studyEventData.studyEventOID]}" />
			    				...
			    				<c:set var="studyEventRepeatKey"
																		...
																		<c:forEach var="formData"
																			items="${studyEventData.formData}">
																			<c:set var="currFormMap"
																				value="${currEventMap[formData.formOID]}" />
																					...
																					<c:forEach var="itemData"
																						...
																						<c:if test="${not empty validationErrors[oidKey]}">
																							<tr valign="top">
																								...
																								<td class="table_cell"><c:out
																										value="${itemData.value}" /><br /> <c:out
																										value="${validationErrors[oidKey]}" /></td>
																							</tr>
																						</c:if>
																					</c:forEach>
																				</c:forEach>
