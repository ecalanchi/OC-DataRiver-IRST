...
<c:set var="resolutionStatusFromServlet"
	value="${param.resolutionStatus}" />

<jsp:useBean scope="request" id="currRow"
	class="org.akaza.openclinica.web.bean.DisplayStudySubjectRow" />
			...
			<tr valign="top">
				...
				<c:if test="${count==0}">
					<td class="table_cell_left"
						rowspan="<c:out value="${eventCount}"/>"><c:out
							value="${currRow.bean.studySubject.label}" />&nbsp;</td>
					<c:choose>
						<c:when
							test="${currRow.sortingColumn >= 1 && currRow.sortingColumn < groups}">
							<td class="table_cell" rowspan="<c:out value="${eventCount}"/>"
								style="display: all"
								id="Groups_0_1_<c:out value="${eblRowCount+1}"/>"><c:choose>
							...
							<td class="table_cell" rowspan="<c:out value="${eventCount}"/>"
								style="display: all"
								id="Groups_0_2_<c:out value="${eblRowCount+1}"/>"><c:out
									value="${currRow.bean.studySubject.gender}" />&nbsp;</td>

							<c:set var="groupCount" value="3" />
							<c:forEach var="group" items="${currRow.bean.studyGroups}">
								<td class="table_cell" rowspan="<c:out value="${eventCount}"/>"
									style="display: all"
									id="Groups_0_<c:out value="${groupCount}"/>_<c:out value="${eblRowCount+1}"/>"><c:out
				...
				<td class="table_cell"><c:choose>
						<c:when test="${event.studyEvent.subjectEventStatus.id==1}">
							...
							<img src="images/icon_Scheduled.gif" border="0"
								alt="<c:out value="${event.studyEvent.subjectEventStatus.name}"/>"
								style="position: relative; left: 7px;">

						</c:when>
						<c:when test="${event.studyEvent.subjectEventStatus.id==2}">
							<img src="images/icon_NotStarted.gif"
								alt="${event.studyEvent.subjectEventStatus.name}"
								title="<c:out value="${event.studyEvent.subjectEventStatus.name}"/>"
						...
						<c:when test="${event.studyEvent.subjectEventStatus.id==5}">
							<img src="images/icon_Stopped.gif"
								alt="${event.studyEvent.subjectEventStatus.name}"
								title="<c:out value="${event.studyEvent.subjectEventStatus.name}"/>"
								border="0" style="position: relative; left: 7px;">
						...
						<c:when test="${event.studyEvent.subjectEventStatus.id==6}">
							<img src="images/icon_Skipped.gif"
								alt="${event.studyEvent.subjectEventStatus.name}"
								title="<c:out value="${event.studyEvent.subjectEventStatus.name}"/>"
								border="0" style="position: relative; left: 7px;">
					...
					</c:choose> <%--  </a><img name="ExpandIcon_<c:out value="${currRow.bean.studySubject.label}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount+count}"/>" src="images/icon_blank.gif" width="15" height="15" style="position: relative; left: 8px;">--%>


				</td>
				<td class="table_cell"><fmt:formatDate
						value="${event.studyEvent.dateStarted}" pattern="${dteFormat}" />&nbsp;</td>
...
if dnote.crfName == edc.crfName, then include dnote--%>
    ...
    --%>
				...
				<c:forEach var="eventDefinitionCRF" items="${eventDefCRFs}">
					...
					<c:forEach var="discNoteBean"
						items="${event.studyEvent.discBeanList}">
						<c:forEach var="displayEventCRFBean"
							items="${event.displayEventCRFs}">
							<c:if
								test="${displayEventCRFBean.eventCRF.crf.name eq eventDefinitionCRF.crf.name &&
       ...
       any displayEventCRFBeans--%>
							...
							<c:if test="${! hasCRFBean}">
								<img src="images/icon_NotStarted.gif" border="0"
									alt="<fmt:message key="not_started" bundle="${resterm}"/>"
									title="<fmt:message key="not_started" bundle="${resterm}"/>"
									align="left" />
							</c:if>
                            ...
                            <img
                              ...
                              alt="<fmt:message key="removed" bundle="${resterm}"/>" title="<fmt:message key="removed" bundle="${resterm}"/>" align="left"/>
										...
										<c:when
											test="${! (displayEventCRFBean.eventCRF.stage.name eq '') }">
											<%--event CRF dataentrystage--%>
											<c:choose>
												<c:when test="${displayEventCRFBean.eventCRF.stage.id == 0}">
													<img name="icon_Note" src="images/icon_Invalid.gif"
														border="0"
														alt="<fmt:message key="invalid" bundle="${resterm}"/>"
														title="<fmt:message key="invalid" bundle="${resterm}"/>"
														align="left" />
												</c:when>
												<c:when test="${displayEventCRFBean.eventCRF.stage.id == 1}">
													<img name="icon_Note" src="images/icon_NotStarted.gif"
														border="0"
														alt="<fmt:message key="not_started" bundle="${resterm}"/>"
														title="<fmt:message key="not_started" bundle="${resterm}"/>"
														align="left" />
												...
												<c:when test="${displayEventCRFBean.eventCRF.stage.id == 2}">
													<img name="icon_Note" src="images/icon_InitialDE.gif"
														border="0"
														alt="<fmt:message key="initial_data_entry" bundle="${resterm}"/>"
														title="<fmt:message key="initial_data_entry" bundle="${resterm}"/>"
														align="left" />
												</c:when>
												<c:when test="${displayEventCRFBean.eventCRF.stage.id == 3}">
													<img name="icon_Note"
														src="images/icon_InitialDEcomplete.gif" border="0"
														alt="<fmt:message key="initial_data_entry_complete" bundle="${resterm}"/>"
														title="<fmt:message key="initial_data_entry_complete" bundle="${resterm}"/>"
														align="left" />
												</c:when>
												<c:when test="${displayEventCRFBean.eventCRF.stage.id == 4}">
													<img name="icon_Note" src="images/icon_DDE.gif" border="0"
														alt="<fmt:message key="double_data_entry" bundle="${resterm}"/>"
														title="<fmt:message key="double_data_entry" bundle="${resterm}"/>"
														align="left" />
												</c:when>
												<c:when test="${displayEventCRFBean.eventCRF.stage.id == 5}">
													<img name="icon_Note" src="images/icon_DEcomplete.gif"
														border="0"
														alt="<fmt:message key="data_entry_complete" bundle="${resterm}"/>"
														title="<fmt:message key="data_entry_complete" bundle="${resterm}"/>"
														align="left" />
												</c:when>
												<c:when test="${displayEventCRFBean.eventCRF.stage.id == 6}">
													<img name="icon_Note" src="images/icon_AdminEdit.gif"
														border="0"
														alt="<fmt:message key="administrative_editing" bundle="${resterm}"/>"
														title="<fmt:message key="administrative_editing" bundle="${resterm}"/>"
														align="left" />
												</c:when>
												<c:when test="${displayEventCRFBean.eventCRF.stage.id == 7}">
													<img name="icon_Note" src="images/icon_Locked.gif"
														border="0"
														alt="<fmt:message key="locked" bundle="${resterm}"/>"
														title="<fmt:message key="locked" bundle="${resterm}"/>"
														align="left" />
												</c:when>
												<c:otherwise>
													<img name="icon_Note" src="images/icon_NotStarted.gif"
						...
						</div> <br /> <c:forEach var="discNoteBean"
							items="${event.studyEvent.discBeanList}">
							<c:forEach var="displayEventCRFBean"
								items="${event.displayEventCRFs}">
								<c:if
                        ...
                        displayEventCRFBean.eventCRF.id eq discNoteBean.eventCRFId}">
									...
									<c:choose>
										<c:when test="${discNoteBean.resolutionStatusId == 1}">
											<c:if test="${newDisplayed eq 'false'}">
						...
						</c:forEach> <%-- For Internet Explorer, which doesn't display some table cell's borders if the cells don't
  have any content--%> <c:if test="${! hasDiscNotes}">&nbsp;</c:if>
					</td>
				</c:forEach>
				...
				<c:if test="${count==0}">
					<td class="table_cell" rowspan="<c:out value="${eventCount}"/>">
						...
						<%--ACTIONS... --%> <c:set var="boolResStatus" value="${false}" />
						...
						<c:forEach var="resStatus" items="${resolutionStatusFromServlet}">
							<c:set var="boolResStatus" value="${true}" />
						</c:forEach> <%--    <table border="0" cellpadding="0" cellspacing="0">
        <tr>
            <td>--%> <c:choose>
							<c:when test="${! boolResStatus}">
								<a
									href="ViewNotes?viewForOne=y&id=<c:out value="${currRow.bean.studySubject.id}"/>&module=${module}"
									onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"
									onMouseUp="javascript:setImage('bt_View1','images/bt_View.gif');"><img
									hspace="2" style="float: left" name="bt_View1"
								...
								<c:if test="${hasDiscNotes}">
									<a
										href="javascript:openDocWindow('ChooseDownloadFormat?discNoteType=${discNoteType}&module=${module}&subjectId=${currRow.bean.studySubject.id}')"><img
										hspace="2" name="bt_View1" src="images/bt_Download.gif"
										border="0"
										alt="<fmt:message key="download_discrepancy_notes" bundle="${resword}"/>"
										title="<fmt:message key="download_discrepancy_notes" bundle="${resword}"/>"></a>
                      ...
                      style="float:left" width="24 " height="15" name="bt_View1" src="images/bt_Download.gif" border="0" alt="<fmt:message key="download_all_discrepancy_notes" bundle="${resword}"/>" title="<fmt:message key="download_all_discrepancy_notes" bundle="${resword}"/>"></a>--%>
							...
							<c:otherwise>
								<a
									href="ViewNotes?viewForOne=y&module=${module}&id=<c:out value="${currRow.bean.studySubject.id}"/>&resolutionStatus=<c:out value="${resolutionStatusFromServlet}"/>"
									onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"
									onMouseUp="javascript:setImage('bt_View1','images/bt_View.gif');"><img
									hspace="2" style="float: left" name="bt_View1"
									src="images/bt_View.gif" border="0"
								...
								<c:if test="${hasDiscNotes}">
									<a
										href="javascript:openDocWindow('ChooseDownloadFormat?subjectId=${currRow.bean.studySubject.id}&discNoteType=${discNoteType}&module=${module}&resolutionStatus=${param.resolutionStatus}')"><img
										hspace="2" name="bt_View1" src="images/bt_Download.gif"
										border="0"
										alt="<fmt:message key="download_discrepancy_notes" bundle="${resword}"/>"
										title="<fmt:message key="download_discrepancy_notes" bundle="${resword}"/>"></a>
	...
	<c:otherwise>
		<tr valign="top">
			<c:choose>
				<c:when
					...
					<td class="table_cell_left" width="64"><c:out
							value="${currRow.bean.studySubject.label}" />&nbsp;</td>
					<td class="table_cell" style="display: all"
						id="Groups_0_1_<c:out value="${eblRowCount+1}"/>"><c:choose>
							<c:when test="${currRow.bean.studySubject.status.id==1}">
								<fmt:message key="active" bundle="${resword}" />
							</c:when>
        ...
        <c:forEach var="group" items="${currRow.bean.studyGroups}">
            ...
            <c:set var="groupCount" value="${groupCount+1}"/>
				...
				<c:otherwise>
					...
					<td class="table_cell_left" width="64"><c:out
							value="${currRow.bean.studySubject.label}" />&nbsp;</td>
					<td class="table_cell" style="display: none"
						id="Groups_0_1_<c:out value="${eblRowCount+1}"/>"><c:choose>
							<c:when test="${currRow.bean.studySubject.status.id==1}">
								<fmt:message key="active" bundle="${resword}" />
							</c:when>
        ...
        </c:import>--%> <img src="images/icon_NotStarted.gif"
				title="<fmt:message key="not_scheduled" bundle="${resterm}"/>"
				border="0" style="position: relative; left: 7px;"> <%--
        ...
        --%>
			...
			<c:forEach var="edc" items="${eventDefCRFs}">
				<td class="table_cell"><c:choose>
						<c:when
							test="${currRow.bean.studySubject.status.id == 5 || currRow.bean.studySubject.status.id == 7}">
							<img name="icon_Note" src="images/icon_Invalid.gif" border="0"
								alt="<fmt:message key="not_started" bundle="${resterm}"/>"
								title="<fmt:message key="removed" bundle="${resterm}"/>"
								align="left" />
						</c:when>
						<c:otherwise>
							<img name="icon_Note" src="images/icon_NotStarted.gif" border="0"
								alt="<fmt:message key="not_started" bundle="${resterm}"/>"
								title="<fmt:message key="not_started" bundle="${resterm}"/>"
								align="left" />
						</c:otherwise>
					</c:choose> <%-- no discrepancy notes here, because there haven't been any events --%>
				</td>
				<c:set var="edcNum" value="${edcNum+1}" />

			</c:forEach>
			<td class="table_cell">
				<%-- no content here, because there hasn't been any events --%> <%--ACTIONS--%>
