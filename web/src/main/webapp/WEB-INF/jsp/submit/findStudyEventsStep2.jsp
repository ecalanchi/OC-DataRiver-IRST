...
<jsp:useBean scope="request" id="displayNextPage"
...
<p class="title">
	<fmt:message key="find_an_existing_study_event_by" bundle="${restext}" />
		...
		<c:otherwise>
			...
			<ul>
				<c:forEach var="entity" items="${displayEntities}">
					<li><a
						href="FindStudyEvent?browseBy=<c:out value="${browseBy}"/>&id=<c:out value="${entity.id}"/>"><c:out
								value="${entity.name}" /></a>
				</c:forEach>
			</ul>
			<p>
				<c:if test="${pageNum gt 0}">
					<a
						href="FindStudyEvent?browseBy=<c:out value="${browseBy}"/>&pageNum=<c:out value="${prevPageNum}" />">&lt;&lt;
						<fmt:message key="previous_page" bundle="${resword}" />
					</a>
				</c:if>
				<c:if test='${displayNextPage == "yes"}'>
					<a
						href="FindStudyEvent?browseBy=<c:out value="${browseBy}"/>&pageNum=<c:out value="${nextPageNum}" />"><fmt:message
							key="next_page" bundle="${resword}" /> &gt;&gt;</a>
				</c:if>
		</c:otherwise>
	</c:choose>
