...
<fmt:setBundle basename="org.akaza.openclinica.i18n.format"
	var="resformat" />
<head>
<title>OpenClinica interview</title>
...
<link rel="stylesheet" href="includes/styles.css" type="text/css"
	...
	<link rel="stylesheet" href="includes/print.css" type="text/css"
		...
		<script type="text/javascript"
			src="includes/new_cal/calendar-setup.js"></script>
		<!-- End -->
</head>
<body class="aka_bodywidth"
	...
	<div id="centralContainer"
		...
		<jsp:useBean scope='request' id='formMessages'
		...
		<table border="0" cellpadding="0" cellspacing="0">
			<tr id="CRF_infobox_closed" style="display: none;">
				<td style="padding-top: 3px; padding-left: 6px; width: 90px;" nowrap>
					<a
					href="javascript:leftnavExpand('CRF_infobox_closed'); leftnavExpand('CRF_infobox_open');"><img
						src="images/sidebar_expand.gif" border="0" align="right"
						hspace="10"></a> <b><fmt:message key="CRF_info"
							bundle="${resword}" /></b>
				</td>
			</tr>
											...
											<div class="tab_BG_h">
												<div class="tab_R_h" style="padding-right: 0px;">
													<div class="tab_L_h"
														...
														<a
															href="javascript:leftnavExpand('CRF_infobox_closed'); leftnavExpand('CRF_infobox_open');"><img
															src="images/sidebar_collapse.gif" border="0"
															align="right"></a> <b><fmt:message key="CRF_info"
																bundle="${resword}" /></b>
													</div>
												</div>
											</div>
														...
														<div class="box_BR">
															...
															<div class="tablebox_center">
																...
																<table border="0" cellpadding="0" cellspacing="0"
																	width="450">
																	<tr>
																		<td colspan="2" class="table_header_row_left"><b><a
																				href="ViewCRF?crfId=<c:out value="${toc.crf.id}"/>">
																					<c:out value="${toc.crf.name}" /> <c:out
																		...
																		<td class="table_header_row" style="color: #789EC5"><b><fmt:message
																					key="crf_notes" bundle="${resword}" />:</b></td>
																		<td class="table_header_row_left"
																			style="color: #789EC5"><font color="#CC0000"><c:out
																					value="${openNum}" /> <fmt:message key="open"
																					bundle="${resword}" /></font>, <font color="#D4A718"><c:out
																					value="${updatedNum}" /> <fmt:message key="updated"
																					bundle="${resword}" /></font>, <font color="#7CB98F"><c:out
																					value="${resolvedNum}" /> <fmt:message
																					key="resolved" bundle="${resword}" /></font>, <br><font
																				color="#000000"><c:out value="${closedNum}" />
																					<fmt:message key="closed" bundle="${resword}" /></font>, <font
																				color="#000000"><c:out value="${notAppNum}" />
																					<fmt:message key="not_applicable"
																						bundle="${resword}" /></font></td>

																	</tr>
																	<tr>
																		<td class="table_cell_left" style="color: #789EC5">
																			<b><fmt:message key="study_subject_ID"
																					bundle="${resword}" />:</b><br />
																		</td>
																		<td class="table_cell_left" style="color: #789EC5">
																		...
																		<c:choose>
																			...
																			<c:otherwise>
																				<td class="table_cell" style="color: #789EC5"><b><fmt:message
																							key="person_ID" bundle="${resword}" />:</b></td>
																				<td class="table_cell_left" style="color: #789EC5"><fmt:message
																						key="na" bundle="${resword}" /></td>
																			</c:otherwise>
																		</c:choose>
																	</tr>
																	...
																	<tr>
																		...
																		<td class="table_cell_noborder" style="color: #789EC5">
																			<c:choose>
																				...
																				<c:otherwise>
																					<a
																						href="ViewStudy?id=<c:out value="${study.id}"/>&viewFull=yes">
																				</c:otherwise>
																	...
																	<tr>
																		...
																		<td class="table_cell_noborder" style="color: #789EC5">
																			<fmt:formatDate value="${subject.dateOfBirth}"
																				pattern="${dteFormat}" /> <span
																			style="padding-left: 6px; padding-right: 6px"><b><fmt:message
																						key="gender" bundle="${resword}" />:</b></span> <c:choose>
																		...
																		<td class="table_cell_noborder"
																			style="color: #789EC5; padding-left: 3px">
																			<%--<c:choose>
          ...
          <c:otherwise>
            <c:out value="${subject.gender}" />
																	...
																	<c:if test="${toc.studyEventDefinition.repeating}">
																		<tr>
																			<td class="table_cell_noborder"
																				style="color: #789EC5"><b><fmt:message
																						key="occurrence_number" bundle="${resword}" />:</b></td>
																			<td class="table_cell_noborder"
																				style="color: #789EC5"><c:out
																					value="${toc.studyEvent.sampleOrdinal}" /></td>
																			<td class="table_cell_top" style="color: #789EC5">

																			</td>
																			<td class="table_cell_noborder"
																				style="color: #789EC5"></td>
...
form element in red <c:out value="FORMMESSAGES: ${formMessages} "/><br/>--%>
																	...
																	<tr>
																		<td class="table_cell_left" nowrap><c:choose>
																				...
																				<c:otherwise>
																					<fmt:message key="interviewer_name"
																						bundle="${resword}" />:
            <c:if
																						test="${study.studyParameterConfig.interviewerNameRequired=='true'}">
                *
            </c:if>
            &nbsp;
        </c:otherwise>
                          ...
                          <jsp:param name="key" value="interviewer" />
																					...
																					<td valign="top" nowrap><c:if
																							test="${study.studyParameterConfig.discrepancyManagement=='true'}">
																							<c:if
																								test="${! (enclosingPage eq 'viewSectionData')}">
																								<a href="#"
																									onClick="openDSNoteWindow('CreateDiscrepancyNote?viewData=y&id=<c:out value="${toc.eventCRF.id}"/>&name=eventCrf&field=interviewer&column=interviewer_name','spanAlert-interviewer'); return false;">
																									<c:choose>
																				...
																				<tr>
																					<td valign="top"><span
																						ID="spanAlert-interviewer" class="alert"></span></td>
																				</tr>
																			</table>
																		</td>

																		<td class="table_cell" nowrap><c:choose>
																				...
																				<c:otherwise>
																					<fmt:message key="interview_date"
																						bundle="${resword}" />:
            <c:if
																						test="${study.studyParameterConfig.interviewDateRequired=='true'}">
                *
            </c:if>&nbsp;<br />
																					<%--(<fmt:message key="date_format" bundle="${resformat}"/>)--%>
																				</c:otherwise>
																			...
																			<table border="0" cellpadding="0" cellspacing="0">
																				...
																				<tr>
																					...
																					<td valign="top"><c:choose>
																							...
																							<c:otherwise>
																								<div class="formfieldM_BG">
																									<input id="interviewDate" type="text" disabled
																										size="15"
																										value="<c:out value="${interviewDate}" />"
																										class="formfieldM"> <input
                          ...
                          <jsp:param name="key" value="interviewDate" />
																					...
																					<td valign="top" nowrap><a href="#"> <img
																							...
																							border="0" id="interviewDateTrigger" /></a> <script
                    ...
                    Calendar.setup({inputField  : "interviewDate", ifFormat    : "<fmt:message key="date_format_calender" bundle="${resformat}"/>", button      : "interviewDateTrigger" });
																							...
																							<c:if
																								test="${! (enclosingPage eq 'viewSectionData')}">
																								<a href="#"
																									onClick="openDNoteWindow('CreateDiscrepancyNote?id=<c:out value="${toc.eventCRF.id}"/>&name=eventCrf&field=interviewDate&column=date_interviewed','spanAlert-interviewDate'); return false;">
																									<c:choose>
