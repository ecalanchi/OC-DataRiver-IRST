...
<tr id="sidebar_Instructions_open" style="display: all">
	<td class="sidebar_tab"><a
		...
		<div class="sidebar_tab_content">
			...
			<fmt:message key="select_subject_view_more_details"
				bundle="${restext}" />
	...
	<c:choose>
		<c:when test="${userRole.manageStudy && module=='manage'}">
			<span class="title_manage"> <fmt:message
					key="manage_all_discrepancy_notes_in" bundle="${restext}" /> <c:out
					value="${study.name}" /> <a
				href="javascript:openDocWindow('https://docs.openclinica.com/3.1/openclinica-user-guide/monitor-and-manage-data/notes-and-discrepancies')"><img
					src="images/bt_Help_Manage.gif" border="0"
					alt="<fmt:message key="help" bundle="${restext}"/>"
					title="<fmt:message key="help" bundle="${restext}"/>"></a></span>
</h1>
</c:when>
<c:otherwise>
	<span class="title_manage"> <fmt:message
			key="view_all_discrepancy_notes_in" bundle="${restext}" /> <c:out
			value="${study.name}" /> <a
		href="javascript:openDocWindow('https://docs.openclinica.com/3.1/openclinica-user-guide/monitor-and-manage-data/notes-and-discrepancies')"><img
			src="images/bt_Help_Manage.gif" border="0"
			alt="<fmt:message key="help" bundle="${restext}"/>"
...
<div class="dnKey">
	<strong><fmt:message key="Filter_by_status"
			bundle="${resword}" /> :</strong> <a
		href="ListDiscNotesForCRFServlet?type=${param.type}&module=${module}&defId=<c:out value="${eventDefinitionId}"/>&tab=1"
		<c:if test="${param.type == 50}">style="color:green"</c:if>><fmt:message
			key="all_notes" bundle="${resterm}" /></a>&nbsp; <a
		href="ListDiscNotesForCRFServlet?resolutionStatus=1&type=${param.type}&tab=${param.tab}&module=${module}&defId=<c:out value="${eventDefinitionId}"/>"><img
		name="icon_Note" src="images/icon_Note.gif" border="0"
		alt="<fmt:message key="Open" bundle="${resterm}"/>"
		title="<fmt:message key="Open" bundle="${resterm}"/>" /></a> (
	<fmt:message key="Open" bundle="${resterm}" />
	)&nbsp; <a
		href="ListDiscNotesForCRFServlet?resolutionStatus=2&type=${param.type}&tab=${param.tab}&module=${module}&defId=<c:out value="${eventDefinitionId}"/>"><img
		name="icon_flagYellow" src="images/icon_flagYellow.gif" border="0"
		alt="<fmt:message key="Updated" bundle="${resterm}"/>"
		title="<fmt:message key="Updated" bundle="${resterm}"/>" /></a> (
	<fmt:message key="Updated" bundle="${resterm}" />
	)&nbsp; <a
		href="ListDiscNotesForCRFServlet?resolutionStatus=3&type=${param.type}&tab=${param.tab}&module=${module}&defId=<c:out value="${eventDefinitionId}"/>"><img
		name="icon_flagGreen" src="images/icon_flagGreen.gif" border="0"
		alt="<fmt:message key="Resolved" bundle="${resterm}"/>"
		title="<fmt:message key="Resolved" bundle="${resterm}"/>" /></a> (
	<fmt:message key="Resolved" bundle="${resterm}" />
	)&nbsp; <a
		href="ListDiscNotesForCRFServlet?resolutionStatus=4&type=${param.type}&tab=${param.tab}&module=${module}&defId=<c:out value="${eventDefinitionId}"/>"><img
		name="icon_flagBlack" src="images/icon_flagBlack.gif" border="0"
		alt="<fmt:message key="Closed" bundle="${resterm}"/>"
		title="<fmt:message key="Closed" bundle="${resterm}"/>" /></a> (
	<fmt:message key="Closed" bundle="${resterm}" />
	)&nbsp; <a
		href="ListDiscNotesForCRFServlet?resolutionStatus=5&type=${param.type}&tab=${param.tab}&module=${module}&defId=<c:out value="${eventDefinitionId}"/>"><img
		name="icon_flagNA" src="images/icon_flagWhite.gif" border="0"
		alt="<fmt:message key="Not_Applicable" bundle="${resterm}"/>"
		title="<fmt:message key="Not_Applicable" bundle="${resterm}"/>" /></a> (
	<fmt:message key="Not_Applicable" bundle="${resterm}" />
	)&nbsp; <br />
	<%-- filterSummary['status'] returns a List of Strings--%>
	<c:if
		test="${filterSummary != null && ! (empty filterSummary['status'])}">
		<fmt:message key="You_have_filtered_status" bundle="${resword}" />
		<c:forEach var="statusName" items="${filterSummary['status']}">
			<strong>${statusName}; </strong>
		</c:forEach>
		<a
			href="ListDiscNotesForCRFServlet?type=${param.type}&module=${module}&defId=<c:out value="${eventDefinitionId}"/>&tab=1"><fmt:message
				key="Clear_status_filter" bundle="${resword}" /></a>
	</c:if>
</div>
<div class="dnKey">
	<strong><fmt:message key="Filter_by_note_type"
			bundle="${resword}" /> :</strong> <a
		href="ListDiscNotesForCRFServlet?module=${module}&defId=${eventDefinitionId}&tab=${param.tab}&type=50"
		<c:if test="${param.type == 50}">style="color:green"</c:if>><fmt:message
			key="all_notes" bundle="${resterm}" /></a>&nbsp;|&nbsp; <a
		href="ListDiscNotesForCRFServlet?module=${module}&resolutionStatus=${param.resolutionStatus}&defId=${eventDefinitionId}&tab=${param.tab}&type=2"
		<c:if test="${param.type == 2}">style="color:green"</c:if>><fmt:message
			key="Annotation" bundle="${resterm}" /></a>&nbsp;|&nbsp; <a
		href="ListDiscNotesForCRFServlet?module=${module}&resolutionStatus=${param.resolutionStatus}&defId=${eventDefinitionId}&tab=${param.tab}&type=1"
		<c:if test="${param.type == 1}">style="color:green"</c:if>><fmt:message
			key="Failed_Validation_Check" bundle="${resterm}" /></a>&nbsp;|&nbsp; <a
		href="ListDiscNotesForCRFServlet?module=${module}&resolutionStatus=${param.resolutionStatus}&defId=${eventDefinitionId}&tab=${param.tab}&type=3"
		<c:if test="${param.type == 3}">style="color:green"</c:if>><fmt:message
			key="query" bundle="${resterm}" /></a>&nbsp;|&nbsp; <a
		href="ListDiscNotesForCRFServlet?module=${module}&resolutionStatus=${param.resolutionStatus}&defId=${eventDefinitionId}&tab=${param.tab}&type=4"
		<c:if test="${param.type == 4}">style="color:green"</c:if>><fmt:message
			key="reason_for_change" bundle="${resterm}" /></a> <br />
	<c:if
		test="${filterSummary != null  && ! (empty filterSummary['type'])}">
		<fmt:message key="You_have_filtered_type" bundle="${resword}" />
		<c:forEach var="typeName" items="${filterSummary['type']}">
			<strong>${typeName}; </strong>
		</c:forEach>
		<a
			href="ListDiscNotesForCRFServlet?module=${module}&resolutionStatus=${param.resolutionStatus}&defId=${eventDefinitionId}&tab=${param.tab}&type=50"><fmt:message
				key="Clear_type_filter" bundle="${resword}" /></a>
	</c:if>
</div>
...
<div id="listDiscNotesDiv">
	<form
		action="${pageContext.request.contextPath}/ListDiscNotesForCRFServlet">
		<input type="hidden" name="module" value="submit"> <input
			type="hidden" name="defId" value="${defId}">
																			...
																			<div class="box_BL">
																				<div class="box_BR">
																					...
																					<div class="textbox_center" align="center">
																						...
																						<c:choose>
																							<c:when test="${userRole.manageStudy}">
																								<span class="title_manage"> <a
																									href="ManageStudy"><fmt:message
																											key="manage_study" bundle="${resworkflow}" /></a>
																							</c:when>
																							<c:otherwise>
																								<span class="title_submit"> <a
																									href="ListStudySubjects"><fmt:message
																											key="submit_data" bundle="${resworkflow}" /></a>
																							</c:otherwise>
																						</c:choose>
																			...
																			<div class="box_BL">
																				<div class="box_BR">
																					...
																					<div class="textbox_center" align="center">
																						...
																						<b><fmt:message key="list_discrepancy_notes"
																								bundle="${restext}" /></b> </span>
