	...
	<c:choose>
		<c:when test="${empty displayItem.data.value}">
			<input type="text" id="ft<c:out value="${itemId}"/>"
				name="fileText<c:out value="${itemId}"/>" value="">
...
<c:if test='${inputType == "instant-calculation"}'>
	...
	<c:choose>
		<c:when test="${isInError}">
			...
			<input class="aka_input_error" id="input<c:out value="${itemId}"/>"
...
<c:if test='${inputType == "textarea"}'>
	...
	<c:choose>
		<c:when test="${isInError}">
			<span class="aka_exclaim_error">! </span>
			<textarea class="aka_input_error"
...
<c:if test='${inputType == "single-select"}'>
	...
	<c:choose>
		...
		<c:when test="${isInError}">
			<span class="aka_exclaim_error">! </span>
			<select class="aka_input_error formfield"
				id="input<c:out value="${itemId}"/>"
				tabindex="<c:out value="${tabNum}"/>"
				onChange="this.className='changedField'; javascript:setImage('DataStatus_top','images/icon_UnsavedData.gif'); javascript:setImage('DataStatus_bottom','images/icon_UnsavedData.gif');"
...
<c:if test='${inputType == "multi-select"}'>
	...
	<c:choose>
		<c:when test="${isInError}">
			<span class="aka_exclaim_error">! </span>
			<select class="aka_input_error" id="input<c:out value="${itemId}"/>"
...
<c:if
	...
	<c:choose>
		<c:when test="${displayItem.numDiscrepancyNotes > 0}">
			...
			<td valign="top"><a tabindex="<c:out value="${tabNum + 1000}"/>"
				href="#" onmouseover="callTip(genToolTips(${itemId}));"
				onmouseout="UnTip()"
				onClick="openDNoteWindow('ViewDiscrepancyNote?subjectId=<c:out value="${studySubject.id}" />&itemId=<c:out value="${itemId}" />&id=<c:out value="${displayItem.data.id}"/>&name=itemData&field=input<c:out value="${itemId}"/>&column=value&monitor=1&writeToDB=1&isLocked=<c:out value="${isLocked}"/>','spanAlert-input<c:out value="${itemId}"/>','<c:out value="${errorTxtMessage}"/>'); return false;"><img
					...
					title="<fmt:message key="discrepancy_note" bundle="${resword}"/>"></a></td>
