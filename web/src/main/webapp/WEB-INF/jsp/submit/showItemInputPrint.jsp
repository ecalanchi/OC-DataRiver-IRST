...
<c:if test='${inputType=="file"}'>
	<input type="text" id="ft<c:out value="${itemId}"/>"
		name="fileText<c:out value="${itemId}"/>"
		value="<c:out value="${displayItem.data.value}"/>">
</c:if>
...
<c:if test='${inputType == "textarea"}'>
	<textarea name="input<c:out value="${itemId}" />" rows="5" cols="40"><c:out
			value="${displayItem.metadata.responseSet.value}" /></textarea>
</c:if>
...
<c:if test='${inputType == "single-select"}'>
	<c:choose>
		<c:when
			test="${displayItem.metadata.defaultValue != '' && displayItem.metadata.defaultValue != null}">
	...
	<c:choose>
		<c:when test="${printDefault == 'true'}">
			<input type="checkbox" name="input<c:out value=""/>"
				value="<c:out value="" />" <c:out value=""/> disabled="disabled" />
			...
			<c:forEach var="option"
				...
				<input type="checkbox" name="input<c:out value="${itemId}"/>"
					...
					<c:out value="${checked}"/> disabled="disabled" />
				<c:out value="${option.text}" />
				<br />
			</c:forEach>
		</c:when>
