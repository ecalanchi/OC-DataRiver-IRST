...
<jsp:useBean scope="request" id="currRow"
	class="org.akaza.openclinica.web.bean.DisplayStudySubjectEventsRow" />
	...
	<c:when test="${!empty currRow.bean.studyEvents}">
		...
		<c:forEach var="event" items="${currRow.bean.studyEvents}">
			...
			<tr valign="top">
				...
				<td class="table_cell_shaded"><c:if test="${! crfIsHidden}">
						<c:import url="../submit/eventLayer.jsp">
							...
							<c:param name="eventStatusName"
								value="${event.studyEvent.subjectEventStatus.name}" />
							<c:param name="eventStatus"
								value="${event.studyEvent.subjectEventStatus.name}" />
							<c:param name="eventName"
								value="${event.studyEvent.studyEventDefinition.name}" />
							<c:param name="subjectId" value="${currRow.bean.studySubject.id}" />
							<c:param name="subjectName"
								value="${currRow.bean.studySubject.label}" />
							<c:param name="eventSysStatus"
								value="${currRow.bean.studySubject.status.name}" />
							<c:param name="module" value="${module}" />
						</c:import>
					</c:if> <c:choose>
						<c:when test="${event.studyEvent.subjectEventStatus.id==1}">
							<%-- ${crfIsHidden ? altMessage : ''}--%>
							<img alt="" title="" src="images/icon_Scheduled.gif" border="0"
								style="position: relative; left: 7px;">

						</c:when>
						<c:when test="${event.studyEvent.subjectEventStatus.id==2}">
							<img alt="" title="" src="images/icon_NotStarted.gif" border="0"
								style="position: relative; left: 7px;">


						</c:when>
						<c:when test="${event.studyEvent.subjectEventStatus.id==3}">
							<img alt="" title="" src="images/icon_InitialDE.gif" border="0"
								style="position: relative; left: 7px;">


						</c:when>
						<c:when test="${event.studyEvent.subjectEventStatus.id==4}">
							<img alt="" title="" src="images/icon_DEcomplete.gif" border="0"
								style="position: relative; left: 7px;">

						</c:when>
						<c:when test="${event.studyEvent.subjectEventStatus.id==5}">
							<img alt="" title="" src="images/icon_Stopped.gif" border="0"
								style="position: relative; left: 7px;">


						</c:when>
						<c:when test="${event.studyEvent.subjectEventStatus.id==6}">
							<img alt="" title="" src="images/icon_Skipped.gif" border="0"
								style="position: relative; left: 7px;">

						</c:when>
						<c:when test="${event.studyEvent.subjectEventStatus.id==7}">
							<img alt="" title="" src="images/icon_Locked.gif" border="0"
								style="position: relative; left: 7px;">

						</c:when>
						<c:when test="${event.studyEvent.subjectEventStatus.id==8}">

							<img alt="" title="" src="images/icon_Signed.gif" border="0"
								style="position: relative; left: 7px;">

						</c:when>

					</c:choose> </a><img
					name="ExpandIcon_<c:out value="${currRow.bean.studySubject.label}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount+count}"/>"
					src="images/icon_blank.gif" width="15" height="15"
					style="position: relative; left: 8px;"></td>
					...
					<c:when test="${empty event.allEventCRFs}">
						...
						<c:forEach var="edc" items="${eventDefCRFs}">
							<c:if test="${! edc.hidden}">
								<td class="table_cell_shaded"><c:import
										url="../submit/eventCrfLayer.jsp">
										...
										<c:param name="subjectId"
											value="${currRow.bean.studySubject.id}" />
										...
										<c:param name="studySubject_subjectId"
											value="${currRow.bean.studySubject.subjectId}" />
										<c:param name="studySubjectId"
											value="${currRow.bean.studySubject.id}" />
										<c:param name="studySubjectStatusId"
											value="${currRow.bean.studySubject.status.id}" />
										<c:param name="subjectName"
											value="${currRow.bean.studySubject.label}" />
										<c:param name="module" value="${module}" />
									</c:import> <c:choose>
										<c:when
											...
											<img
												name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${eventCRFNum-1}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
												src="images/icon_NotStarted.gif" border="0">
										</c:when>
										<c:otherwise>
											<img
												name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${eventCRFNum-1}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
												src="images/icon_Invalid.gif" border="0">
										</c:otherwise>
									</c:choose> </a></td>
							</c:if>
              ...
              user logged in at the site level--%> <%--<c:set var="crfIsHidden" value="${dec.eventDefinitionCRF.hidden}" />
                <c:set var="altMessage">
                    <fmt:message key="crf_is_hidden" bundle="${resword}"/>
                </c:set>--%> <c:choose>
									<c:when test="${dec.eventCRF.id > 0}">
										...
										<c:choose>
											<c:when
												...
												<c:choose>
													<c:when
														test="${dec.stage.locked || event.studyEvent.subjectEventStatus.id==7}">
														...
														<img alt="" title=""
															name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
															src="images/CRF_status_icon_Locked.gif" border="0">
														</a>
													</c:when>
													...
													<c:when test="${dec.stage.initialDE}">
														<img alt="" title=""
															name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
															src="images/CRF_status_icon_Started.gif" border="0">
														</a>
													</c:when>
													...
													<c:when test="${dec.stage.initialDE_Complete}">
														<img alt="" title=""
															name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
															src="images/CRF_status_icon_InitialDEcomplete.gif"
															border="0">
														</a>
													</c:when>
													...
													<c:when test="${dec.stage.doubleDE}">
														<img alt="" title=""
															name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
															src="images/CRF_status_icon_DDE.gif" border="0">
														</a>
													</c:when>
													...
													<c:when test="${dec.stage.doubleDE_Complete}">
														<img alt="" title=""
															name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
															src="images/CRF_status_icon_Complete.gif" border="0">
														</a>
													</c:when>
													...
													<c:otherwise>
														<img alt="" title=""
															name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
															src="images/CRF_status_icon_Invalid.gif" border="0">
														</a>
													</c:otherwise>
												</c:choose>
											</c:when>
									...
									<c:otherwise>
										...
										<c:import url="../submit/eventCrfLayer.jsp">
											...
											<c:param name="rowCount" value="${eblRowCount+count}" />
											<c:param name="eventId" value="${event.studyEvent.id}" />
											<c:param name="crfStatus">
												<fmt:message key="not_started" bundle="${resword}" />
											</c:param>
											<c:param name="crfId" value="${0}" />
											<c:param name="edcId" value="${dec.eventDefinitionCRF.id}" />
											<c:param name="crfVersionId"
												value="${dec.eventDefinitionCRF.defaultVersionId}" />
											<c:param name="crfName"
												value="${dec.eventDefinitionCRF.crf.name}" />
											<c:param name="subjectId"
												value="${currRow.bean.studySubject.id}" />
											<%-- create studySubject_subjectId which is subjectId of StudySubjectBean, since "subjectId" has been used in other places -10-18-2007  --%>
											<c:param name="studySubject_subjectId"
										...
										<c:choose>
											...
											<c:when
												...
												<img
													name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
													src="images/CRF_status_icon_Scheduled.gif" border="0">
											</c:when>
											<c:otherwise>
												<img
													name="CRFicon_<c:out value="${eblRowCount+count}"/>_<c:out value="${crfCount}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
				...
				<c:if test="${count==0}">
					<td class="table_cell" rowspan="<c:out value="${eventCount}"/>">
						<table border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td><a
									href="ViewStudySubject?id=<c:out value="${currRow.bean.studySubject.id}"/>"
									...
									<c:choose>
										<c:when test="${!currRow.bean.studySubject.status.deleted}">
											<c:if test="${study.status.available}">
												<td><a
													href="RemoveStudySubject?action=confirm&id=<c:out value="${currRow.bean.studySubject.id}"/>&subjectId=<c:out value="${currRow.bean.studySubject.subjectId}"/>&studyId=<c:out value="${currRow.bean.studySubject.studyId}"/>"
													...
													onMouseUp="javascript:setImage('bt_Remove1','images/bt_Remove.gif');"><img
														...
														align="left" hspace="6"></a></td>
												<td><a
													href="ReassignStudySubject?id=<c:out value="${currRow.bean.studySubject.id}"/>"
													...
													onMouseUp="javascript:setImage('bt_Reassign1','images/bt_Reassign.gif');"><img
														name="bt_Reassign1" src="images/bt_Reassign.gif"
														border="0"
														alt="<fmt:message key="reassign" bundle="${resword}"/>"
														title="<fmt:message key="reassign" bundle="${resword}"/>"
														align="left" hspace="6"></a></td>
											</c:if>
										</c:when>
										<c:otherwise>
											<c:if test="${study.status.available}">
												<td><a
													href="RestoreStudySubject?action=confirm&id=<c:out value="${currRow.bean.studySubject.id}"/>&subjectId=<c:out value="${currRow.bean.studySubject.subjectId}"/>&studyId=<c:out value="${currRow.bean.studySubject.studyId}"/>"
		...
		<tr valign="top">
			...
			<td class="table_cell_shaded"><c:import
                ...
                --%>
					...
					<c:param name="module" value="${module}" />
				</c:import> <img src="images/icon_NotStarted.gif" border="0"
				style="position: relative; left: 7px;"> </a><img
				name="ExpandIcon_<c:out value="${currRow.bean.studySubject.label}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>"
				src="images/icon_blank.gif" width="15" height="15"
				style="position: relative; left: 8px;"></td>
			...
			<c:forEach var="edc" items="${eventDefCRFs}">
				<td class="table_cell_shaded"><c:import
						...
						<c:when
							test="${currRow.bean.studySubject.status.id != 5 && currRow.bean.studySubject.status.id != 7}">
							<img
								name="CRFicon_<c:out value="${eblRowCount}"/>_<c:out value="${eventCRFNum-1}"/>_<c:out value="${currRow.bean.studySubject.label}"/>"
								src="images/CRF_status_icon_Scheduled.gif" border="0">
						</c:when>
						<c:otherwise>
							<img
			...
			<td class="table_cell">
				<table border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td><a
							href="ViewStudySubject?module=<c:out value="${module}"/>&id=<c:out value="${currRow.bean.studySubject.id}"/>"
							onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"
							onMouseUp="javascript:setImage('bt_View1','images/bt_View.gif');"><img
								name="bt_View1" src="images/bt_View.gif" border="0"
								alt="<fmt:message key="view" bundle="${resword}"/>" align="left"
								hspace="6"></a></td>
						<c:if test="${module == 'manage'}">
							<c:choose>
								...
								<c:when test="${!currRow.bean.studySubject.status.deleted}">
									<c:if test="${study.status.available}">
										...
										<td><a
											...
											onMouseDown="javascript:setImage('bt_Reassign1','images/bt_Reassign_d.gif');"
											onMouseUp="javascript:setImage('bt_Reassign1','images/bt_Reassign.gif');"><img
												name="bt_Reassign1" src="images/bt_Reassign.gif" border="0"
												alt="<fmt:message key="reassign" bundle="${resword}"/>"
												title="<fmt:message key="reassign" bundle="${resword}"/>"
												align="left" hspace="6"></a></td>
									</c:if>
