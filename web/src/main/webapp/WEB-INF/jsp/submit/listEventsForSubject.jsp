...
<c:set var="hasExtendedQuery"
	value="${! (paginatingQuery == null || paginatingQuery eq '')}" />
	...
	<c:choose>
		<c:when test="${userRole.manageStudy && module=='manage'}">
			<span class="title_manage"> <fmt:message
					key="manage_all_subjects_in" bundle="${restext}" /> <c:out
					value="${study.name}" /> <a
				href="javascript:openDocWindow('https://docs.openclinica.com/3.1/openclinica-user-guide/submit-data-module-overview/subject-matrix')"><img
...
<c:otherwise>
	<span class="title_manage"> <fmt:message
			key="view_all_subjects_in" bundle="${restext}" /> <c:out
			value="${study.name}" /> <a
		href="javascript:openDocWindow('https://docs.openclinica.com/3.1/openclinica-user-guide/submit-data-module-overview/subject-matrix')"><img
			src="images/bt_Help_Manage.gif" border="0"
			alt="<fmt:message key="help" bundle="${restext}"/>"
...
<table border="0" cellpadding="0" cellspacing="0">
	<tr>
						...
						<div class="tab_R">
							...
							<c:choose>
								<c:when test="${userRole.manageStudy}">
									<a class="tabtext"
										href="ListStudySubject?${tmpExtendedQuery}&module=${module}"
										onclick="javascript:HighlightTab(0);"><fmt:message
											key="all_events" bundle="${restext}" /></a>
								</c:when>
								<c:otherwise>
									<a class="tabtext"
										href="ListStudySubjects?${tmpExtendedQuery}&module=${module}"
										onclick="javascript:HighlightTab(0);"><fmt:message
											key="all_events" bundle="${restext}" /></a>
								</c:otherwise>
							</c:choose>
				...
				<div class="tab_BG">
					<div class="tab_L">
						<div class="tab_R">
							...
							<c:choose>
								<c:when test="${userRole.manageStudy}">
									<a class="tabtext" href="ListStudySubject?${tmpExtendedQuery}"
										onclick="javascript:HighlightTab(0);"><fmt:message
											key="all_events" bundle="${restext}" /></a>
								</c:when>
								<c:otherwise>
									<a class="tabtext" href="ListStudySubjects?${tmpExtendedQuery}"
										onclick="javascript:HighlightTab(0);"><fmt:message
											key="all_events" bundle="${restext}" /></a>
								</c:otherwise>
							</c:choose>
		...
		<td align="right" style="padding-left: 12px; display: all"
			id="TabsBackDis"><img src="images/arrow_back_dis.gif" border="0"></td>
		...
		<td align="right" id="TabsNext" style="display: all"><a
			href="javascript:TabsForward()"><img src="images/arrow_next.gif"
				border="0"></a></td>
