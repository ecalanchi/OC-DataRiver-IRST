...
<jsp:useBean scope='request' id='whichResStatus'
...
<form name="oneDNForm" method="POST" action="CreateOneDiscrepancyNote">
	<table border="0" cellpadding="0" cellspacing="0"
		style="float:left;display:<c:out value="${displayAll}"/>"
		id="${boxId}">
		<input type="hidden" name="parentId" value="${parentId}" />
		<input type="hidden" name="viewDNLink${parentId}"
			value="${viewDNLink}" />
											...
											<table border="0" width="580">
												<c:if test="${parentId>0}">
													<div style="float: left">
														<fmt:message key="respond_this_Discrepancy_Note"
															bundle="${restext}" />
													</div>
												</c:if>
												<div style="float: right">
													...
													<c:choose>
														<c:when test="${parentId==0}">
															<a href="javascript:scrollToY('p');"
																onclick="javascript:leftnavExpand('<c:out value="${boxId}"/>');javascript:addText('a0','<b><fmt:message key="begin_new_thread" bundle="${resword}"/></b>');"><img
																name="close_box"
														...
														<c:otherwise>
															<a
																href="javascript:scrollToY('msg<c:out value="${parentId}"/>');"
																onclick="javascript:leftnavExpand('<c:out value="${boxId}"/>');"><img
																name="close_box"
																alt="<fmt:message key="Close_Box" bundle="${resword}"/>"
																src="images/bt_Remove.gif" class="icon_dnBox"></a>
														</c:otherwise>
												...
												<div class="dnBoxCol2">
													<div class="formtextareaXL4_BG">
														<textarea name="detailedDes${parentId}" rows="4" cols="50"
															class="formtextareaXL4"><c:out
																value="${discrepancyNote.detailedNotes}" /></textarea>
													</div>
													<jsp:include page="../showMessage.jsp"><jsp:param
															name="key" value="detailedDes${parentId}" /></jsp:include>
												</div>

												<c:choose>
													<c:when test="${parentId > 0}">
														<input type="hidden" name="typeId${parentId}"
															value="${param.typeId}" />
													</c:when>
													<c:otherwise>
														<div class="dnBoxCol1">
														...
														<div class="dnBoxCol2">
															<div class="formfieldL_BG">
																...
																<c:choose>
																	<c:when
																		test="${whichResStatus == 22 || whichResStatus == 1}">
																		<select name="typeId${parentId}"
																			id="typeId${parentId}" class="formfieldL"
																...
																<jsp:include page="../showMessage.jsp"><jsp:param
																		name="key" value="typeId${parentId}" /></jsp:include>
															</div>
														</div>
													</c:otherwise>
												</c:choose>
												...
												<span id="res1${parentId}">
													...
													<div class="dnBoxCol2">
														<div class="formfieldL_BG">
															...
															<select name="resStatusId${parentId}"
																id="resStatusId${parentId}" class="formfieldL">
																<c:choose>
																	<c:when
																		test="${(parentId>0 || whichResStatus==2 && discrepancyNote.discrepancyNoteTypeId==3) || whichResStatus==1}">
																		<c:set var="resStatuses" value="${resolutionStatuses}" />
																	</c:when>
																	<c:otherwise>
																		<%-- for FVC; for Query, it will be set per setElements function --%>
																		<c:set var="resStatuses"
																			value="${resolutionStatuses2}" />
																	</c:otherwise>
																</c:choose>
												...
												<div class="dnBoxCol2" class="formfieldL_BG">
													<div class="formfieldL_BG">
														...
														<select name="userAccountId${parentId}"
															id="userAccountId${parentId}" class="formfieldL">
															<option value="0">
																<c:forEach var="user" items="${userAccounts}">
																	<c:choose>
																		<c:when test="${userAccountId1 == user.userAccountId}">
																			<option
																				value="<c:out value="${user.userAccountId}"/>"
																		...
																		<c:otherwise>
																			<option
																				value="<c:out value="${user.userAccountId}"/>"><c:out
																					value="${user.lastName}" />,
																				<c:out value="${user.firstName}" /> (
																				<c:out value="${user.userName}" />)
																		</c:otherwise>
													...
													<jsp:include page="../showMessage.jsp"><jsp:param
															name="key" value="userAccountId${parentId}" /></jsp:include>
												</div>
												</span>
												...
												<div class="dnBoxCol2">
													<input name="sendEmail${parentId}" value="1"
														type="checkbox" />
												</div>
												</span>
												...
												<c:if
													...
													<div class="dnBoxCol3">
														<input type="submit" id="submitBtn${parentId}"
															name="Submit${parentId}"
															value="<fmt:message key="submit" bundle="${resword}"/>"
															class="button_medium"
															onclick="javascript:setYPos('<c:out value="${parentId}"/>');">
														<input type="submit" name="SubmitExit${parentId}"
															value="<fmt:message key="submit_exit" bundle="${resword}"/>"
															class="button_medium"
															onclick="javascript:setValue('close<c:out value="${parentId}"/>','true');javascript:setYPos('<c:out value="${parentId}"/>');">
													</div>
												</c:if>
												<c:if test="${parentId==0}">
													<div style="float: left">
														<jsp:include page="../showMessage.jsp"><jsp:param
																name="key" value="newChildAdded${parentId}" /></jsp:include>
													</div>
												</c:if>
											</table>
										</div>
