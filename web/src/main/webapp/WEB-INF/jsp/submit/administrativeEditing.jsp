...
<head>
<title>OpenClinica <fmt:message key="administrative_editing"
...
<body class="aka_bodywidth"
	onload="
        <c:if test='${popUpURL != ""}'>
        openDNoteWindow('<c:out value="${popUpURL}" />');</c:if>document.getElementById('CRF_infobox_closed').style.display='block';document.getElementById('CRF_infobox_open').style.display='none';javascript:enableSubmit();"
...
margin-top:20px; updateTabs(<c:out value="${tabId}"/>);--%>
	...
	<div id="centralContainer"
		style="padding-left: 3em; margin-top: 10px; background-color: white; color: black;">
		...
		<form id="mainForm" name="crfForm" method="POST"
			action="AdministrativeEditing"
			onLoad="javascript:leftnavExpand('CRF_infobox_open');">
			<input type="hidden" name="eventCRFId"
				...
				type="hidden" name="fromViewNotes"
				value="<c:out value="${fromViewNotes}"/>" />
			<%-- We have to feed this value to the method giveFirstElementFocus()--%>
			<input id="formFirstField" type="hidden" name="formFirstField"
				value="${requestScope['formFirstField']}" /> <input id="hasPopUp"
				type="hidden" name="hasPopUp" value="${requestScope['hasPopUp']}" />
				...
				<div id="errorMessagesContainer" class="aka_err_message">
					<ul>
						<c:forEach var="formMsg" items="${formMessages}">
							<li style="color: #ff0000"><span
								style="text-decoration: underline"><strong><label
										for="<c:out value="${formMsg.key}" />"><c:out
							...
							<div class="formfieldM_BG_noMargin">
								<select class="formfieldM" name="sectionName" size="1"
									...
									<c:forEach var="sec" items="${toc.sections}">
										<c:set var="tabUrl"
											value="AdministrativeEditing?eventCRFId=${section.eventCRF.id}&sectionId=${sec.id}&tab=${tabCount}&exitTo=${exitTo}" />
																									...
																									<table border="0" cellpadding="0"
																										...
																										<tr>
																											<td><input type="submit" id="srh"
																												...
																												class="button_medium"
																												onClick="disableSubmit('DataStatus_top'); this.form.submit();" /></td>
																							...
																							<tr>
																								...
																								<td align="right" valign="bottom">
																									<table border="0" cellpadding="0"
																										cellspacing="0">
																										<tr>
																											<td><input type="submit" id="srm"
																												name="submittedResume"
																												value="<fmt:message key="save" bundle="${resword}"/>"
																												class="button_medium"
																												onClick="disableSubmit('DataStatus_top'); this.form.submit();" /></td>
																											<td><input type="submit" id="sem"
																												name="submittedExit"
																												value="<fmt:message key="exit" bundle="${resword}"/>"
																												class="button_medium"
																												onClick="return checkEntryStatus('DataStatus_top');" /></td>
																																		...
																																		<td
																																			...
																																			<c:set var="responseOptionBean"
																																				...
																																				<c:param name="originJSP"
																																					value="administrativeEditing" />
																																				<c:param name="isForcedRFC"
																																					value="${study.studyParameterConfig.adminForcedReasonForChange}" />
																																			</c:import>
																																		</td>
																																	</c:forEach>
																																</c:when>
               ...
               will be in the name --%> <c:choose>
																											...
																											<c:if test="${status.last}">
																												...
																												<tr id="<c:out value="${repeatParentId}"/>"
																													...
																													<input type="hidden"
																														name="<c:out value="${repeatParentId}"/>_[<c:out value="${repeatParentId}"/>].existing"
																														value="<c:out value="${uniqueId+1}"/>">
					...
					<table border="0" cellpadding="0" cellspacing="0" width="100%"
                      ...
                      <c:otherwise>
                        <td colspan="2">&nbsp;</td>
										...
										<td><input type="submit" id="srl" name="submittedResume"
											value="<fmt:message key="save" bundle="${resword}"/>"
											class="button_medium"
											onclick="disableSubmit(); this.form.submit();" /></td>
										<td><input type="hidden" name="fromResolvingNotes"
											value="${fromResolvingNotes}" /> <input type="submit"
											id="sel" name="submittedExit"
											value="<fmt:message key="exit" bundle="${resword}"/>"
											class="button_medium"
