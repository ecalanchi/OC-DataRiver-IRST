...
<jsp:useBean scope="request" id="toc"
	class="org.akaza.openclinica.bean.submit.DisplayTableOfContentsBean" />
...
<c:choose>
	<c:when test="${toc.action == 'ae'}">
		<h1>
			<span class="title_manage"><fmt:message
					key="administrative_editing_on_event_CRF" bundle="${resword}" /></span>
		</h1>
	...
	<c:otherwise>
		<h1>
			<span class="title_submit"><fmt:message
					key="event_CRF_data_submission" bundle="${resword}" /> <a
...
<form name="contentForm" action="TableOfContents" method="post">
	...
	<input type="hidden" name="ecid"
		value="<c:out value="${toc.eventCRF.id}"/>" /> <input type="hidden"
		name="action" value="<c:out value="${toc.action}"/>" /> <input
		type="hidden" name="editInterview" value="1" /> <input type="hidden"
		name="interviewer" value="<c:out value="${interviewer}"/>" /> <input
	...
	<table border="0" cellpadding="0" cellspacing="0">
		<tr>
												...
												<div class="box_BR">
													...
													<div class="tablebox_center">
														...
														<table border="0" cellpadding="0" cellspacing="0"
															width="100%">
															<tr>
																<td class="table_cell"><b><fmt:message
																			key="interviewer_name" bundle="${resword}" />:</b></td>
																<td class="table_cell"><c:out
																		value="${interviewer}" /></td>
																<td class="table_cell"><b><fmt:message
																			key="interview_date" bundle="${resword}" />:</b></td>
																<td class="table_cell"><c:out
			...
			<td valign="top"><input type="submit"
				value="<fmt:message key="edit_interview_info" bundle="${resword}"/>"
...
<div class="table_title_submit">
	<fmt:message key="section_properties" bundle="${resword}" />
</div>

<c:choose>
	<c:when test="${empty toc.sections}">
		<br />
		<fmt:message key="there_are_no_sections" bundle="${resword}" />
	</c:when>
	<c:otherwise>
											...
											<div class="tablebox_center">
												...
												<table border="0" cellpadding="0" cellspacing="0"
													...
													<tr>
														...
														<td class="table_header_row"><fmt:message key="title"
																bundle="${resword}" /></td>
														<td class="table_header_row"><fmt:message
																key="incomplete_items" bundle="${resword}" /></td>

														<c:if test="${toc.eventDefinitionCRF.doubleEntry}">
															<td class="table_header_row"><fmt:message
																	key="items_pending_validation" bundle="${resword}" /></td>
														</c:if>

														<td class="table_header_row"><fmt:message
																key="completed_items" bundle="${resword}" /></td>
														<td class="table_header_row"><fmt:message
																key="total_items" bundle="${resword}" /></td>
														<td class="table_header_row"><fmt:message
																key="actions" bundle="${resword}" /></td>
													</tr>
													<c:set var="rowCount" value="${0}" />
													<c:forEach var="section" items="${toc.sections}">
														<c:set var="actionLink"
															value="${toc.actionServlet}?eventCRFId=${toc.eventCRF.id}&sectionId=${section.id}" />

														<%-- set the action label --%>
														<c:choose>
															<c:when test='${toc.eventCRF.stage.uncompleted}'>
																<c:set var="action"
																	value="<fmt:message key="begin_data_entry" bundle="${resword}"/>" />
															</c:when>
															<c:otherwise>
																<c:choose>
																	<c:when test='${toc.eventCRF.stage.initialDE}'>
																		<c:choose>
																			...
																			<c:otherwise>
																				<c:set var="action"
																					value="<fmt:message key="continue_data_entry" bundle="${resword}"/>" />
																			</c:otherwise>
																		</c:choose>
																	</c:when>
																	<c:otherwise>
																		<c:choose>
																			<c:when
																				test='${toc.eventCRF.stage.initialDE_Complete}'>
																				<c:set var="action"
																					value="<fmt:message key="begin_validation" bundle="${resword}"/>" />
																			</c:when>
																			<c:otherwise>
																				<c:choose>
																					<c:when test='${toc.eventCRF.stage.doubleDE}'>
																						<c:choose>
																							...
																							<c:otherwise>
																								<c:set var="action"
																									value="<fmt:message key="continue_validation" bundle="${resword}"/>" />
																							</c:otherwise>
																						</c:choose>
																					</c:when>
																					<c:otherwise>
																						<c:choose>
																							<c:when
																								test='${(toc.eventCRF.stage.doubleDE_Complete) || (toc.eventCRF.stage.admin_Editing)}'>
																								<c:set var="action"
																									value="<fmt:message key="perform_administrative_edits" bundle="${resword}"/>" />
																							</c:when>
																							<c:otherwise>
																								<c:set var="action"
																									value="<fmt:message key="view_data" bundle="${resword}"/>" />
																							</c:otherwise>
																						</c:choose>
																					</c:otherwise>
																				</c:choose>
														...
														<tr>
															...
															<td class="table_cell"><c:out
																	value="${section.title}" /></td>
															<td class="table_cell"><c:out
																	value="${section.numItems - section.numItemsNeedingValidation - section.numItemsCompleted}" /></td>

															<c:if test="${toc.eventDefinitionCRF.doubleEntry}">
																<td class="table_cell"><c:out
																		value="${section.numItemsNeedingValidation}" /></td>
															</c:if>

															<td class="table_cell"><c:out
																	value="${section.numItemsCompleted}" /></td>
															<td class="table_cell"><c:out
																	value="${section.numItems}" /></td>
															<%--			<td class="table_cell"><a href="<c:out value="${actionLink}"/>"><c:out value="${action}"/></a>--%>
															<td class="table_cell"><a
																href="<c:out value="${actionLink}"/>"
		...
		<td>
			<form method="POST" action="EnterDataForStudyEvent">
				<input type="hidden" name="eventId"
					value="<c:out value="${toc.studyEvent.id}"/>" /> <input
					type="submit" name="Submit"
					value="<fmt:message key="view_study_event_overview" bundle="${resword}"/>"
					class="button_xlong" />
			</form>
		</td>
		<c:if test="${toc.action != 'ae'}">
			<td>
				<form method="POST" action="MarkEventCRFComplete">
					<input type="hidden" name="eventCRFId"
						value="<c:out value="${toc.eventCRF.id}"/>" /> <input
						type="submit" name="Submit"
						value="<fmt:message key="mark_this_CRF_complete" bundle="${resword}"/>"
						class="button_xlong">
				</form>
			</td>
		</c:if>
																			...
																			<div class="box_BL">
																				<div class="box_BR">
																					...
																					<div class="textbox_center" align="center">
																						...
																						<span class="title_submit"> <fmt:message
																								key="submit_data" bundle="${resword}" />
																			...
																			<div class="box_BL">
																				<div class="box_BR">
																					...
																					<div class="textbox_center" align="center">
																						...
																						<span class="title_submit"> <fmt:message
																								key="view_subjects" bundle="${resword}" />
																			...
																			<div class="box_BL">
																				<div class="box_BR">
																					...
																					<div class="textbox_center" align="center">
																						...
																						<span class="title_submit"> <b><fmt:message
																									key="data_entry" bundle="${resword}" /></b>
