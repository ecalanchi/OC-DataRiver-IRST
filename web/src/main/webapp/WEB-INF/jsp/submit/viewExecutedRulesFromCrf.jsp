											...
											<table border="0" cellpadding="0" cellspacing="0"
												...
												<tr valign="top">
													<td class="table_header_column_top"><fmt:message
															key="rule_crf_version" bundle="${resword}" />:</td>
													<td class="table_cell"><c:out
															value="${status.current.key.crfVersion}" /></td>
												</tr>
												<tr valign="top">
													<td class="table_header_column"><fmt:message
															key="rule_name" bundle="${resword}" />:</td>
													<td class="table_cell"><c:out
															value="${status.current.key.ruleName}" /></td>
												</tr>
												<tr valign="top">
													<td class="table_header_column"><fmt:message
															key="rule_result" bundle="${resword}" />:</td>
													<td class="table_cell"><c:out
															value="${status.current.key.result}" /></td>
												</tr>

												<tr valign="top">
													<td class="table_header_column"><fmt:message
															key="rule_actions" bundle="${resword}" />:</td>
													<td class="table_cell"><c:forEach
															items="${status.current.key.actions}" var="action">
									...
									<div class="box_BR">
										...
										<div class="tablebox_center">
											<table border="0" cellpadding="0" cellspacing="0"
												...
												<c:forEach items="${status.current.value}"
													varStatus="status1">
													<c:set var="count" value="${count+1}" />
													<tr valign="top">
														<td class="table_cell_left"><c:out
																value="${status1.current.key.studyEventDefinitionName}" /></td>
														<td class="table_cell">${status1.current.key.itemGroupName}</td>
														<td class="table_cell">${status1.current.key.itemName}</td>

														<td class="table_cell"><span id="a${count}"
															style="color: BLUE; text-decoration: underline;"
															onClick="showOrHideSubjects(${count},'<fmt:message key="rule_show_subjects" bundle="${resword}" />','<fmt:message key="rule_hide_subjects" bundle="${resword}" />')"><u><fmt:message
													...
													<tr id="tr${count}" style="display: none;">
														<td class="table_cell" colspan="7"><span
															style="color: #D4A718; font-weight: bold; font-size: 12px;"><fmt:message
																...
																var="subject" items="${status1.current.value}">
																<c:out value="${subject}" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        </c:forEach></td>
													</tr>
												</c:forEach>
...
<table border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td><input type="button" name="Submit" id="submit"
			...
			class="button_long"
			onClick="window.location.href='RunRule?${submitLinkParams}';" /></td>
		</td>
