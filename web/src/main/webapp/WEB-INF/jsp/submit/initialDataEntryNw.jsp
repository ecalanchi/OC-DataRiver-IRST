...
<head>
<title>OpenClinica <fmt:message key="initial_data_entry"
...
<script type="text/JavaScript" language="JavaScript"
	src="includes/Tabs.js"></script>
<script type="text/JavaScript" language="JavaScript"
	src="includes/CalendarPopup.js"></script>
<script type="text/javascript" language="JavaScript"
	src="includes/repetition-model/repetition-model.js"></script>
<script type="text/JavaScript" language="JavaScript"
	src="includes/prototype.js"></script>
<script type="text/JavaScript" language="JavaScript"
	src="includes/scriptaculous.js?load=effects"></script>
<script type="text/JavaScript" language="JavaScript"
	src="includes/effects.js"></script>

<!-- Added for the new Calender -->

<link rel="stylesheet" type="text/css" media="all"
	href="includes/new_cal/skins/aqua/theme.css" title="Aqua" />
<script type="text/javascript" src="includes/new_cal/calendar.js"></script>
<script type="text/javascript"
	src="includes/new_cal/lang/calendar-en.js"></script>
<script type="text/javascript"
	src="includes/new_cal/lang/<fmt:message key="jscalendar_language_file" bundle="${resformat}"/>"></script>
<script type="text/javascript" src="includes/new_cal/calendar-setup.js"></script>
<!-- End -->

</head>
<body class="aka_bodywidth"
	onload=" document.getElementById('here').style.display='none'; document.getElementById('CRF_infobox_closed').style.display='block';document.getElementById('CRF_infobox_open').style.display='none';"
	onunload="javascript:clsWin();">
	...
	<div id="centralContainer"
		...
		<form id="mainForm" name="crfForm" method="post"
			action="InitialDataEntry">
			<input type="hidden" name="eventCRFId"
				value="<c:out value="${section.eventCRF.id}"/>" /> <input
				type="hidden" name="sectionId"
				value="<c:out value="${section.section.id}"/>" /> <input
				type="hidden" name="checkInputs"
				value="<c:out value="${checkInputsValue}"/>" /> <input
				type="hidden" name="tab" value="<c:out value="${tabId}"/>" />
			<%-- We have to feed this value to the method giveFirstElementFocus()--%>
			<input id="formFirstField" type="hidden" name="formFirstField"
				value="${requestScope['formFirstField']}" /> <input type="hidden"
				name="exitTo" value="${exitTo}" /> <input type="hidden"
				name="sectionId" value="<c:out value="${section.section.id}"/>" />
			<input type="hidden" name="isFirstTimeOnSection"
				value="<c:out value="${section.section.id}"/>" />
    ...
    improve the style as well -->
				<div id="errorMessagesContainer" class="aka_err_message">
					<ul>
						<c:forEach var="formMsg" items="${formMessages}">
							...
							<c:if test="${hasShown}">
								<c:set var="error_color" value="#006633" />
							</c:if>
							<li style="color:  ${error_color}"><span
								style="text-decoration: underline"><strong> <label
										onclick="getFocused('<c:out value="${formMsg.key}" />');"><c:out
												value="${formMsg.value}" /></label>
								</strong></span></li>
      ...
      error messages look like:
       ...
       <strong><label for="formElementName">formElementName</label></strong>.<br/>-->
			...
			<%-- error messages are not null --%>
			<div id="box" class="dialog" style="display: none;">
				<span id="mbm"> <c:choose>
						<c:when
							test="${section.eventDefinitionCRF.electronicSignature == true}">
							<fmt:message key="crf_data_entry_password_required"
								bundle="${restext}" />
						</c:when>
						<c:otherwise>
							<fmt:message key="marking_CRF_complete_finalize_DE"
								bundle="${restext}" />
						</c:otherwise>
					</c:choose>
				</span><br>
				<div style="text-align: center; width: 100%;">
					<input align="center" type="password" name="password"
						id="passwordId" /> <input type="button"
						onclick="hm('box');requestSignatureFromCheckbox(document.getElementById('passwordId').value, checkboxObject);"
						value="OK" />
				</div>
			</div>
			<!-- section tabs here -->
				...
				<tr>
					...
					<c:if test="${fn:length(toc.sections) gt 1}">
						...
						<td align="right" valign="middle"
							style="padding-left: 12px; display: none" id="TabsBack"><a
							href="javascript:TabsBack()"><img src="images/arrow_back.gif"
								border="0" style="margin-top: 10px"></a></td>
						<td align="right" style="padding-left: 12px" id="TabsBackDis">
							<img src="images/arrow_back_dis.gif" border="0" />
						</td>
					</c:if>
					<script type="text/JavaScript" language="JavaScript">
<!--
...
//-->
					...
					<c:if test="${fn:length(toc.sections) gt 1}">
						...
						<td align="right" id="TabsNextDis" style="display: none"><img
							src="images/arrow_next_dis.gif" border="0" /></td>
						<td align="right" id="TabsNext"><a
							href="javascript:TabsForward()"><img
								src="images/arrow_next.gif" border="0"
								style="margin-top: 10px; margin-right: 6px" /></a></td>
						<td>&nbsp;
							<div class="formfieldM_BG_noMargin">
								<select id="sectionTabSelectElement" class="formfieldM"
									name="sectionName" size="1" onchange="gotoLink();">
									<c:set var="tabCount" value="1" />
									<option selected>--
										<fmt:message key="select_to_jump" bundle="${resword}" /> --
									</option>
									<c:forEach var="sec" items="${toc.sections}">
										...
										<option value="<c:out value="${tabUrl}"/>"><c:out
												value="${sec.name}" /></option>
															...
															<div class="tablebox_center">
																...
																<table border="0" cellpadding="0" cellspacing="0">
																	...
																	<c:forEach var="displayItem"
																		...
																		<c:if
																			test="${displayItem.itemGroup.groupMetaBean.showGroup}">
																								...
																								<td align="right" valign="bottom">
																									<table border="0" cellpadding="0"
																										...
																										<tr>
																											...
																											<td><input type="submit" id="srh"
																												disabled name="submittedResume"
																												value="<fmt:message key="save" bundle="${resword}"/>"
																												class="button_medium"
																												onclick="disableSubmit(); this.form.submit();" /></td>
																											<td><input type="submit" id="seh"
																												disabled name="submittedExit"
																												value="<fmt:message key="exit" bundle="${resword}"/>"
																												class="button_medium"
																												onClick="return checkEntryStatus('DataStatus_top');" /></td>
																											<c:choose>
																												<c:when test="${! empty formMessages}">
																													<td valign="bottom"><img
																														name="DataStatus_top" id="status_top"
																														alt="<fmt:message key="data_status" bundle="${resword}"/>"
																														title="<fmt:message key="changed_not_saved" bundle="${restext}"/>"
																														src="images/icon_UnsavedData.gif"></td>
																												</c:when>
																												<c:otherwise>
																													<td valign="bottom"><img
																														name="DataStatus_top" id="status_top"
																														...
																														src="images/icon_UnchangedData.gif"></td>
																												</c:otherwise>
																											</c:choose>
	...
	one of the error keys should equal the item group's OID --%>
																						...
																						<c:forEach var="formMsg" items="${formMessages}">
																							<c:if
																								test="${formMsg.key eq displayItem.itemGroup.itemGroupBean.oid}">
																								<c:set var="isSectionShown" value="true" />
																							</c:if>
																						</c:forEach>

																						<td
																							<c:if test="${hasShown && isSectionShown}"> class="aka_group_show" </c:if>>

																							<%-- end place where we highlight sections --%> <c:set
																								var="uniqueId" value="0" /> <c:set
																								var="repeatParentId"
																							...
																							<c:choose>
																								...
																								<c:when
																									test="${isFirstTimeOnSection == section.section.id}">
																									<c:set var="repeatNumber" value="0" />
																								</c:when>
																								<c:otherwise>
																									<c:set var="repeatNumber"
...
but the custom tag uses that, not this jstl code--%> <c:if
																								...
																								<table border="0" cellspacing="0"
																									cellpadding="0" class="aka_form_table"
																									width="100%">
																									<thead>
																										<tr>
																									...
																									<tbody>
																										...
																										<c:forEach var="bodyItemGroup"
																											...
																											<c:forEach var="bodyItem"
																												...
																												<c:choose>
																													<c:when
																														test="${bodyItem.metadata.showItem}">
																														...
																														<c:forEach var="formMsg"
																															...
																															<c:set var="inputValue">
																																<c:out value="${repeatParentId}" />_<c:out
      ...
      bodyItem.metadata.responseLayout eq 'Horizontal'}">
           ...
           bodyItem.metadata.responseSet.responseType.name eq 'checkbox')}">
																																...
																																<c:forEach var="respOption"
																																	...
																																	<td
																																		class="aka_padding_norm aka_cellBorders <c:out value="${extraClass}"/>">
               ...
               will be in the name --%> <c:choose>
																													...
																													</c:choose> <c:if test="${isButtonRemShow == true }">
																														...
																														<c:choose>
																															<c:when
																																...
																																<c:forEach var="formMsg"
																																	...
																																	<c:if
																																		test="${formMsg.key eq inputValue}">
																																		<c:set var="isItemShown" value="true" />
      ...
      bodyItem.metadata.responseLayout eq 'Horizontal'}">
           ...
           bodyItem.metadata.responseSet.responseType.name eq 'checkbox')}">
																				...
																				<c:otherwise>
																					...
																					<c:if
																						test="${displayItem.singleItem.metadata.showItem || cdisplay>0}">
				...
				<c:if test="${!empty displayItem.singleItem.metadata.header}">
					<c:choose>
						<c:when test="${cdisplay>0}">
							<c:set var="scdShowStatus"
								value="${displayItem.singleItem.scdData.scdDisplayInfo.scdShowStatus}" />
				...
				<c:set var="rowDisplay"
					value="${displayItem.singleItem.scdData.scdDisplayInfo.rowDisplayStatus}" />
				<c:set var="rowSCDShowIDStr"
					value="${displayItem.singleItem.scdData.scdDisplayInfo.rowSCDShowIDStr}" />
				<input type="hidden" id="rowSCDShowIDs${numOfTr}"
					value="${rowSCDShowIDStr}" />
				<c:choose>
				...
				<td class="table_cell_left">
					<table border="0">
						<tr>
							...
							<c:set var="isItemShown" value="false" />
							<c:forEach var="formMsg" items="${formMessages}">
								<c:set var="inputValue">input<c:out
										value="${displayItem.singleItem.item.id}" />
								</c:set>
								<c:if test="${formMsg.key eq inputValue}">
									<c:set var="isItemShown" value="true" />
								</c:if>
                            ...
                            -->
														<td valign="top" class="aka_text_block"><c:import
																url="../submit/generateLeftItemTxt.jsp">
																<c:param name="itemId"
																	value="${displayItem.singleItem.item.id}" />
																<c:param name="inputType"
																	value="${displayItem.singleItem.metadata.responseSet.responseType.name}" />
																<c:param name="function"
																	value="${displayItem.singleItem.metadata.responseSet.options[0].value}" />
																<c:param name="linkText"
								...
								<td class="table_cell">
									<table border="0">
										...
										<c:forEach var="childItem"
											...
											<c:set var="ccdisplay"
												value="${childItem.scdData.scdItemMetadataBean.id}" />
												...
												<c:if test="${currColumn == 1}">
													...
													<c:choose>
														<c:when test="${ccdisplay > 0}">
															<c:set var="scdShowStatus"
																value="${childItem.scdData.scdDisplayInfo.scdShowStatus}" />
                        ...
                        --%>
                                ...
                                -->
															...
															<td valign="top" nowrap="nowrap">
																<%-- display the HTML input tag --%> <c:set
																	var="itemNum" value="${itemNum + 1}" /> <c:set
																	var="displayItem" scope="request" value="${childItem}" />
															...
															<c:if test='${childItem.item.units != ""}'>
																<td valign="top"><c:out
																		value="(${childItem.item.units})" escapeXml="false" />
																</td>
															</c:if>
															<!--
...
table-->
					...
					<table border="0" cellpadding="0" cellspacing="0" width="100%"
						...
						<tr>
							<td valign="bottom" nowrap="nowrap"><a href="#top">&nbsp;&nbsp;<fmt:message
										key="return_to_top" bundle="${resword}" /></a></td>
							<td align="right" valign="bottom">
								<table border="0" cellpadding="0" cellspacing="0">
									<tr>
										<c:choose>
											<c:when test="${stage !='adminEdit' && section.lastSection}">
												<c:choose>
													<c:when
														test="${section.eventDefinitionCRF.electronicSignature == true}">
														<td valign="bottom"><input type="checkbox"
															id="markCompleteId" name="markComplete" value="Yes"
															<c:if test="${markComplete=='Yes'}"> checked </c:if>
															onclick="sm('box', this, 730,100);"></td>
														<td valign="bottom" nowrap="nowrap">&nbsp; <fmt:message
																key="mark_CRF_complete" bundle="${resword}" />&nbsp;&nbsp;&nbsp;
														</td>
													</c:when>
													<c:otherwise>
														<td valign="bottom"><input type="checkbox"
										...
										<td><input type="submit" id="srl" disabled
											name="submittedResume"
											value="<fmt:message key="save" bundle="${resword}"/>"
											class="button_medium"
											onclick="disableSubmit(); this.form.submit();" /></td>
										<td><input type="submit" id="sel" disabled
											name="submittedExit"
											value="<fmt:message key="exit" bundle="${resword}"/>"
											class="button_medium"
											onClick="return checkEntryStatus('DataStatus_bottom');" /></td>
										<c:choose>
											<c:when test="${! empty formMessages}">
