...
<jsp:useBean scope='session' id='mayProcessUploading'
...
<jsp:useBean scope='request' id='attachedFilePath'
	class='java.lang.String' />
<jsp:useBean scope='request' id='uploadFileStatus'
	class='java.lang.String' />
...
<body>
	...
	<form name="uploadForm" action="UploadFile" method="post"
		...
		<input type="hidden" name="itemId" value="${fileItemId}"> <input
			type="hidden" name="inputName" value="${inputName}">
		<div style="position: absolute; left: 20px; width: 600px;">
			<c:choose>
					...
					processUpload('<c:out value="${inputName}"/>','<c:out value="${fileName}"/>','true');
							...
							<P>
								<input type="button" name="close"
									...
									onClick="javascript:window.close();" class="button_long">
							</P>
						</c:when>
						<c:otherwise>
							...
							<br>
							<br>
							<fmt:message key="select_cancel_upload_button"
								bundle="${restext}" />
							<br>
							<br>
							<br>
							...
							<P>
								<input type="submit" name="upload"
									value="<fmt:message key="upload_file" bundle="${resword}"/>"
									class="button_long"> <input type="button" name="cancel"
									value="<fmt:message key="cancel_upload" bundle="${resword}"/>"
									onClick="cleanFile()" class="button_long">
							</P>
				...
				<c:otherwise>
					<fmt:message key="uploading_not_process_because_permission"
						bundle="${restext}" />
				</c:otherwise>
			</c:choose>
		</div>
