												...
												<div class="tablebox_center">
													...
													<table border="0" cellpadding="0" cellspacing="0">
														...
														<c:if test='${section.section.subtitle != ""}'>
															<tr class="aka_stripes">
																<td class="aka_header_border"><fmt:message
																		...
																		value="${section.section.subtitle}" escapeXml="false" />
																</td>
															</tr>
														</c:if>
														...
														<c:forEach var="displayItem" items="${section.items}"
																			...
																			<tr>
																				...
																				<td valign="bottom" nowrap="nowrap"
																					style="padding-right: 50px"><a name="top"><fmt:message
																							...
																							value="${displayItem.metadata.pageNumberLabel}"
															...
															<c:if
																test="${currPage != displayItem.metadata.pageNumberLabel && displayItemNum >0}">
																		...
																		<table border="0" cellpadding="0" cellspacing="0"
																			...
																			<tr>
																				...
																				<td valign="bottom" nowrap="nowrap"
																					...
																					name="item<c:out value="${displayItem.item.id}"/>">
																						<fmt:message key="page" bundle="${resword}" />: <c:out
																							value="${displayItem.metadata.pageNumberLabel}"
																							escapeXml="false" />
																				</a></td>
																				<td align="right" valign="bottom">
																						...
																						<tr>
																							<c:choose>
																								<c:when
																									...
																									<td valign="bottom" nowrap>&nbsp; <fmt:message
	...
	<c:if test="${!empty displayItem.metadata.header}">
		<tr class="aka_stripes">
			...
			<td class="table_cell_left aka_stripes"><b><c:out
						value="${displayItem.metadata.header}" escapeXml="false" /></b></td>
		</tr>
	</c:if>
	<c:if test="${!empty displayItem.metadata.subHeader}">
		<tr class="aka_stripes">
			<td class="table_cell_left"><c:out
					value="${displayItem.metadata.subHeader}" escapeXml="false" /></td>
		</tr>
	</c:if>
	<tr>
			...
			<table border="0">
				<tr>
					<td valign="top"></c:if> <c:if
							test="${displayItem.metadata.columnNumber >1}">
							<td valign="top">
						</c:if>
						<table border="0">
							<tr>
             ...
             <td valign="top"><c:out value="${displayItem.metadata.leftItemText}" escapeXml="false"/></td>--%>
								<td valign="top" class="text_block"><c:out
										value="${displayItem.metadata.questionNumberLabel}"
										escapeXml="false" /> <c:out
										value="${displayItem.metadata.leftItemText}" escapeXml="false" /></td>
								<td valign="top">
									<%-- display the HTML input tag --%> <c:set var="displayItem"
										scope="request" value="${displayItem}" /> <c:import
									...
									<c:import url="../showMessage.jsp">
										<c:param name="key" value="input${displayItem.item.id}" />
									</c:import>
								</td>
								<c:if test='${displayItem.item.units != ""}'>
									<td valign="top"><c:out
											value="(${displayItem.item.units})" escapeXml="false" /></td>
								</c:if>
								<td valign="top"><c:out
										value="${displayItem.metadata.rightItemText}"
										escapeXml="false" /></td>
							</tr>
						</table></td>
							...
							<table border="0">
								<tr>
									<td valign="top" class="text_block"><c:out
											...
											escapeXml="false" /> <c:out
											value="${childItem.metadata.leftItemText}" escapeXml="false" /></td>
									<td valign="top">
									...
									<c:if test='${childItem.item.units != ""}'>
										<td valign="top"><c:out value="(${childItem.item.units})"
												escapeXml="false" /></td>
									</c:if>
									<td valign="top"><c:out
											value="${childItem.metadata.rightItemText}" escapeXml="false" />
									</td>
								</tr>
							</table>
						</td>
				...
				<tr>
					<c:choose>
						<c:when test="${stage !='adminEdit' && section.lastSection}">
							...
							<td valign="bottom" nowrap>&nbsp; <fmt:message
									key="mark_complete" bundle="${resword}" />&nbsp;&nbsp;&nbsp;
							</td>
						</c:when>
						<c:otherwise>
							<td colspan="2">&nbsp;</td>
					...
					<td><input type="submit" name="submittedResume"
						value="<fmt:message key="save" bundle="${resword}"/>"
						class="button_medium" /></td>
					<td><input type="submit" name="submittedExit"
						value="<fmt:message key="exit" bundle="${resword}"/>"
						class="button_medium"
