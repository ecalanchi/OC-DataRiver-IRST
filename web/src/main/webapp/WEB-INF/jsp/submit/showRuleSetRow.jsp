...
<jsp:useBean scope="request" id="currRow"
	class="org.akaza.openclinica.control.submit.ViewRuleAssignmentRow" />
...
<tr valign="top" bgcolor="#F5F5F5">
	<td rowspan="<c:out value="${count}"/>" class="table_cell_left"><c:out
			value="${currRow.bean.studyEventDefinitionName}" /></td>
	<td rowspan="<c:out value="${count}"/>" class="table_cell"><c:out
			value="${currRow.bean.crfWithVersionName}" /></td>
	<td rowspan="<c:out value="${count}"/>" class="table_cell"><c:out
			value="${currRow.bean.groupLabel}" />&nbsp;</td>
	<td rowspan="<c:out value="${count}"/>" class="table_cell"><c:out
			value="${currRow.bean.itemName}" />&nbsp;</td>
  ...
  <td class="table_cell">&nbsp;</td>--%>
	<td class="table_cell">
		<table border="0" cellpadding="0" cellspacing="0">
			<tr>
				<td><a
					href="ViewRuleSet?ruleSetId=<c:out value="${currRow.bean.id}"/>"
				...
				<c:choose>
					<c:when test="${currRow.bean.status.name eq 'AVAILABLE' }">
						<c:if test="${userBean.sysAdmin || (userRole.manageStudy)}">
							<td><a
								href="RunRuleSet?ruleSetId=<c:out value="${currRow.bean.id}"/>"
								...
								onMouseUp="javascript:setImage('bt_Run1','images/bt_ExexuteRules.gif');"><img
									name="bt_Edit1" src="images/bt_ExexuteRules.gif" border="0"
									alt="<fmt:message key="rule_run" bundle="${resword}"/>"
									title="<fmt:message key="rule_run" bundle="${resword}"/>"
									align="left" hspace="6"></a></td>
							<c:if test="${ readOnly != 'true' }">
								<td><a
									href="RemoveRuleSet?action=confirm&ruleSetId=<c:out value="${currRow.bean.id}"/>"
					...
					<c:otherwise>
						<c:if test="${ readOnly != 'true' }">
							<td><a
								href="RestoreRuleSet?action=confirm&ruleSetId=<c:out value="${currRow.bean.id}"/>"
				...
				<td><a
					href="ViewRuleSetAudit?ruleSetId=<c:out value="${currRow.bean.id}"/>"
					...
					onMouseUp="javascript:setImage('bt_Audit1','images/icon_NotStarted.gif');"><img
						name="bt_Remove1" src="images/icon_NotStarted.gif" border="0"
						alt="<fmt:message key="audit_logs" bundle="${resword}"/>"
						title="<fmt:message key="audit_logs" bundle="${resword}"/>"
						align="left" hspace="6"></a></td>
			</tr>
		</table>
	</td>
...
<c:forEach var="ruleSetRule" items="${currRow.bean.ruleSetRules}">
  ...
  <c:choose>
	...
	<tr valign="top">
		...
		<td class="table_cell"><c:forEach var="action"
