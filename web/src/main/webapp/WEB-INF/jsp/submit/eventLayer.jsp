...
<jsp:useBean scope="request" id="currEvent"
...
<c:set var="no_startedi18n">
	<fmt:message key="not_started" bundle="${resword}" />
</c:set>
<c:set var="data_entry_completei18n">
	<fmt:message key="data_entry_complete" bundle="${resterm}" />
</c:set>
<c:set var="initial_data_entry_completei18n">
	<fmt:message key="initial_data_entry_complete" bundle="${resterm}" />
</c:set>
<c:set var="double_data_entryi18n">
	<fmt:message key="double_data_entry" bundle="${resterm}" />
</c:set>
<c:set var="lockedi18n">
	<fmt:message key="locked" bundle="${resterm}" />
</c:set>
<c:set var="completedi18n">
	<fmt:message key="completed" bundle="${resterm}" />
</c:set>
<c:set var="availablei18n">
	<fmt:message key="available" bundle="${resterm}" />
</c:set>
<c:set var="removedi18n">
	<fmt:message key="removed" bundle="${resterm}" />
</c:set>
<c:set var="notscheduledi18n">
	<fmt:message key="not_scheduled" bundle="${resterm}" />
</c:set>
<c:set var="signedi18n">
	<fmt:message key="signed" bundle="${resterm}" />
</c:set>
...
<div
	id="Lock_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>"
	style="position: absolute; visibility: hidden; z-index: 3; width: 50px; height: 30px; top: 0px;">

	<c:choose>
		<c:when test="${currEvent.repeatingNum>1}">
			<a
				href="javascript:ExpandEventOccurrences('<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>',<c:out value="${currEvent.repeatingNum}"/>);
	  </c:when>	 
	 <c:otherwise>
        ...
        <!-- skip forward/back links: variables represent Table ID for this event, number of occurrences of repeating event, and number to skip to -->
	             ...
	             <!-- start to get the other events in currEvent.repeatEvents array-->                
		              ...
		              <c:forEach var="reEvent" items="${currEvent.repeatEvents}">
		               <td id="Event_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_<c:out value="${reNum}"/>" valign="top" width="180" <c:if test="${reNum > 3}"> style="
				display:none;" </c:if>>
				<table border="0" cellpadding="0" cellspacing="0" width="100%">
					<tr valign="top">
						<td class="table_header_row" colspan="2"><b> <fmt:message
									key="occurrence_x_of" bundle="${resword}">
									<fmt:param>#<c:out value="${reNum}" />
									</fmt:param>
								</fmt:message> <c:out value="${currEvent.repeatingNum}" /> <br> <fmt:formatDate
									value="${reEvent.dateStarted}" pattern="${dteFormat}" /> <br>
								<fmt:message key="status" bundle="${resword}" />: <c:out
									value="${reEvent.subjectEventStatus.name}" />
						</b></td>
					</tr>
					...
					<tr
						id="Menu_on_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_<c:out value="${reNum}"/>"
						style="display: none">
						<td colspan="2">
							<table border="0" cellpadding="0" cellspacing="0" width="100%">
								...
								<c:choose>
									<c:when
										test="${eventSysStatus == 'available' || eventSysStatus == availablei18n || eventSysStatus == 'signed' || eventSysStatus == signedi18n }">
										<c:choose>
											<c:when
												test="${reEvent.subjectEventStatus.name =='completed' || reEvent.subjectEventStatus.name ==completedi18n}">
												<tr valign="top">
													<td class="table_cell"><a
														href="EnterDataForStudyEvent?eventId=<c:out value="${reEvent.id}"/>"><img
															src="images/bt_View.gif" border="0" align="left"></a>&nbsp;&nbsp;
														<a
														href="EnterDataForStudyEvent?eventId=<c:out value="${reEvent.id}"/>"><fmt:message
																key="view" bundle="${resword}" /></a></td>
												</tr>

												<c:if
													test="${(userRole.role.name=='director' || userBean.sysAdmin) && (study.status.available)}">
													<tr valign="top">
														<td class="table_cell_left"><a
															href="UpdateStudyEvent?event_id=<c:out value="${reEvent.id}"/>&ss_id=<c:out value="${subjectId}"/>">
																<img src="images/bt_Edit.gif" border="0" align="left">
														</a> &nbsp;&nbsp; <a
															href="UpdateStudyEvent?event_id=<c:out value="${reEvent.id}"/>&ss_id=<c:out value="${subjectId}"/>"><fmt:message
																	key="edit" bundle="${resword}" /></a></td>
													</tr>

													<tr valign="top">
														<td class="table_cell"><a
															href="RemoveStudyEvent?action=confirm&id=<c:out value="${reEvent.id}"/>&studySubId=<c:out value="${subjectId}"/>"><img
																src="images/bt_Remove.gif" border="0" align="left"></a>&nbsp;&nbsp;
															<a
															href="RemoveStudyEvent?action=confirm&id=<c:out value="${reEvent.id}"/>&studySubId=<c:out value="${subjectId}"/>"><fmt:message
																	key="remove" bundle="${resword}" /></a></td>
													</tr>
												</c:if>
											</c:when>
											<c:when
												test="${reEvent.subjectEventStatus.name =='locked' || reEvent.subjectEventStatus.name ==lockedi18n }">
												<c:if
													test="${userRole.role.name=='director' || userBean.sysAdmin}">
													<tr valign="top">
														<td class="table_cell"><a
															href="EnterDataForStudyEvent?eventId=<c:out value="${currEvent.id}"/>"><img
																src="images/bt_View.gif" border="0" align="left"></a>&nbsp;&nbsp;
															<a
															href="EnterDataForStudyEvent?eventId=<c:out value="${currEvent.id}"/>"><fmt:message
																	key="view" bundle="${resword}" /></a></td>
													</tr>
													<c:if test="${study.status.available}">
														<tr valign="top">
															<td class="table_cell"><a
																href="RemoveStudyEvent?action=confirm&id=<c:out value="${reEvent.id}"/>&studySubId=<c:out value="${subjectId}"/>">
																	<img src="images/bt_Remove.gif" border="0" align="left">
															</a>&nbsp;&nbsp; <a
																href="RemoveStudyEvent?action=confirm&id=<c:out value="${reEvent.id}"/>&studySubId=<c:out value="${subjectId}"/>"><fmt:message
																		key="remove" bundle="${resword}" /></a></td>
														</tr>
											...
											<c:otherwise>
												<tr valign="top">
													<td class="table_cell"><a
														...
														<a
														href="EnterDataForStudyEvent?eventId=<c:out value="${reEvent.id}"/>"><fmt:message
																key="view" bundle="${resword}" />/<fmt:message
																key="enter_data" bundle="${resword}" /></a></td>
												</tr>
												...
												<c:if
													...
													<tr valign="top">
														<td class="table_cell"><a
															href="UpdateStudyEvent?event_id=<c:out value="${reEvent.id}"/>&ss_id=<c:out value="${subjectId}"/>"><img
																src="images/bt_Edit.gif" border="0" align="left"></a>
															&nbsp;&nbsp; <a
															href="UpdateStudyEvent?event_id=<c:out value="${reEvent.id}"/>&ss_id=<c:out value="${subjectId}"/>&module=<c:out value="${module}"/>"><fmt:message
																	key="edit" bundle="${resword}" /></a></td>
													</tr>
									...
									<c:when
										test="${reEvent.subjectEventStatus.name == 'removed' || reEvent.subjectEventStatus.name == removedi18n}">
										<tr valign="top">
											<td class="table_cell_left"><a
												href="EnterDataForStudyEvent?eventId=<c:out value="${reEvent.id}"/>"><img
													src="images/bt_View.gif" border="0" align="left"></a>&nbsp;&nbsp;
										...
										<c:if
											test="${userRole.role.name=='director' || userBean.sysAdmin}">
											<c:if
												test="${eventSysStatus != 'removed' && eventSysStatus != 'auto-removed' && eventSysStatus != removedi18n}">
												<tr valign="top">
													<td class="table_cell_left"><a
														href="RestoreStudyEvent?action=confirm&id=<c:out value="${reEvent.id}"/>&studySubId=<c:out value="${subjectId}"/>"><img
															src="images/bt_Restore.gif" border="0" align="left"></a>&nbsp;&nbsp;
														<a
														href="RestoreStudyEvent?action=confirm&id=<c:out value="${reEvent.id}"/>&studySubId=<c:out value="${subjectId}"/>"><fmt:message
																key="restore" bundle="${resword}" /></a></td>
												</tr>
											</c:if>
										</c:if>
									</c:when>
				...
				</td> <c:set var="reNum" value="${reNum+1}" /> </c:forEach>

				<td
				id="Scroll_off_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_next"
				width="20" class="statusbox_scroll_R_dis"><img
					src="images/arrow_status_next_dis.gif" border="0"></td>

				<td
				id="Scroll_on_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_next"
				width="20" class="statusbox_scroll_R" style="display: none;">
					<!-- scroll next arrow: variables represent Table ID for this event, and number of occurrences of repeating event -->

					<div
						id="bt_Scroll_Event_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_next">
						<a
							href="javascript:StatusBoxNext('<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>',<c:out value="${currEvent.repeatingNum}"/>);"><img
							src="images/arrow_status_next.gif" border="0"></a>
					</div>

					<div
						id="bt_Scroll_Event_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_next_dis"
						style="display: none;">
				...
				<tr
					id="Menu_off_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>"
					style="">

					<td class="table_cell_left"
						colspan="<c:out value="${currEvent.repeatingNum}"/>"><i><fmt:message
								key="click_for_more_options" bundle="${restext}" /></i></td>
				</tr> <!--  end of repeating events      -->
		</c:when>

		<c:otherwise>
			<!-- start of non-repeating events-->
			<td class="table_header_row_left"><fmt:message key="subject"
					bundle="${resword}" />: <c:out value="${subjectName}" /> <br>
				<fmt:message key="event" bundle="${resword}" />: <c:out
					value="${eventName}" /> <br> <c:choose>

					<c:when
						test="${currEvent.studyEventDefinition.repeating == 'false'}">
						<fmt:message key="status" bundle="${resword}" />: <c:out
							value="${eventStatus}" /></td>
			<td class="table_header_row_left" align="right"><a
				href="javascript:leftnavExpand('Menu_on_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>'); javascript:leftnavExpand('Menu_off_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>');"
				onClick="layersShowOrHide('hidden','Lock_all'); 
								layersShowOrHide('hidden','Event_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>');
								layersShowOrHide('hidden','Lock_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>');
			...
			<c:otherwise>
				...
				<tr valign="top">
					<td class="table_header_row" colspan="2"><b> <fmt:message
								key="occurrence_x_of" bundle="${resword}">
								<fmt:param>#<c:out value="1" />
								</fmt:param>
							</fmt:message> <c:out value="${currEvent.repeatingNum}" /> <br> <fmt:formatDate
								value="${currEvent.dateStarted}" pattern="${dteFormat}" /> <br>
							<fmt:message key="status" bundle="${resword}" />: <c:out
								value="${currEvent.subjectEventStatus.name}" />
					</b></td>
				</tr>
				<c:if
					test="${
					...
					<tr>
						<td class="table_cell_left"><a
							href="CreateNewStudyEvent?studySubjectId=<c:out value="${subjectId}"/>&studyEventDefinition=<c:out value="${eventDefId}"/>"><fmt:message
									key="add_another_occurrence" bundle="${resword}" /></a></td>
					</tr>
				</c:if>
			</c:otherwise>
	</c:choose>
	...
	<tr
		id="Menu_off_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>"
		style="display: all">
		<td class="table_cell_left" colspan="2"><i><fmt:message
					key="click_for_more_options" bundle="${restext}" /></i></td>
	</tr>
	<tr
		id="Menu_on_<c:out value="${subjectName}"/>_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>"
		style="display: none">
		<td colspan="2">
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<c:choose>
					<c:when
						...
						<c:choose>
							...
							<c:when
								test="${eventStatus =='not scheduled' || eventStatus == notscheduledi18n}">
								<c:if test="${!userRole.monitor && (study.status.available)}">
									<tr valign="top">
										<td class="table_cell_left"><a
											href="CreateNewStudyEvent?studySubjectId=<c:out value="${subjectId}"/>&studyEventDefinition=<c:out value="${eventDefId}"/>">
												<img src="images/bt_Schedule.gif" border="0" align="left">
										</a>&nbsp;&nbsp; <a
											href="CreateNewStudyEvent?studySubjectId=<c:out value="${subjectId}"/>&studyEventDefinition=<c:out value="${eventDefId}"/>">
												<fmt:message key="schedule" bundle="${resword}" />
										</a></td>
									</tr>
								</c:if>
							</c:when>
							...
							<c:when
								test="${eventStatus =='completed' || eventStatus==completedi18n}">
								<tr valign="top">
									<td class="table_cell_left"><a
										href="EnterDataForStudyEvent?eventId=<c:out value="${eventId}"/>"><img
											src="images/bt_View.gif" border="0" align="left"></a>&nbsp;&nbsp;
										<a
										href="EnterDataForStudyEvent?eventId=<c:out value="${eventId}"/>"><fmt:message
												key="view" bundle="${resword}" /></a></td>
								</tr>
								...
								<c:if
									...
									<tr valign="top">
										<td class="table_cell_left"><a
											href="UpdateStudyEvent?event_id=<c:out value="${eventId}"/>&ss_id=<c:out value="${subjectId}"/>"><img
												src="images/bt_Edit.gif" border="0" align="left"></a>
											&nbsp;&nbsp; <a
											href="UpdateStudyEvent?event_id=<c:out value="${eventId}"/>&ss_id=<c:out value="${subjectId}"/>"><fmt:message
													key="edit" bundle="${resword}" /></a></td>
									</tr>

									<tr valign="top">
										<td class="table_cell_left"><a
											href="RemoveStudyEvent?action=confirm&id=<c:out value="${eventId}"/>&studySubId=<c:out value="${subjectId}"/>"><img
												src="images/bt_Remove.gif" border="0" align="left"></a>&nbsp;&nbsp;
											<a
											href="RemoveStudyEvent?action=confirm&id=<c:out value="${eventId}"/>&studySubId=<c:out value="${subjectId}"/>"><fmt:message
													key="remove" bundle="${resword}" /></a></td>
									</tr>
								</c:if>
							</c:when>
							...
							<c:when
								test="${eventStatus =='locked' || eventStatus == lockedi18n}">
								<c:if test="${userRole.director || userBean.sysAdmin}">
									...
									<c:if test="${study.status.available}">
										<tr valign="top">
											<td class="table_cell_left"><a
												href="RemoveStudyEvent?action=confirm&id=<c:out value="${eventId}"/>&studySubId=<c:out value="${subjectId}"/>">
													<img src="images/bt_Remove.gif" border="0" align="left">
											</a>&nbsp;&nbsp; <a
												href="RemoveStudyEvent?action=confirm&id=<c:out value="${eventId}"/>&studySubId=<c:out value="${subjectId}"/>"><fmt:message
														key="remove" bundle="${resword}" /></a></td>
