...
<tr id="sidebar_Instructions_open" style="display: all">
	<td class="sidebar_tab"><a
		...
		<div class="sidebar_tab_content"></div> <fmt:message
			key="test_rules_instructions_1" bundle="${resword}" /></td>
...
<h1>
	<span class="title_manage"> <fmt:message key="test_rules_title"
			bundle="${resword}" /></span>
</h1>
...
<form action="TestRule?action=${action}" method="post">
	<h3>
		<fmt:message key="test_rules_step_1" bundle="${resword}" />
	</h3>
										...
										<div class="textbox_center">
											<table border="0" cellpadding="0" cellspacing="0">
												...
												<c:if test="${itemName != null}">
													...
													<input type="hidden" name="ruleSetId"
														value="<c:out value="${ruleSetId}"/>" />
													<tr valign="top">
														<td class="formlabel"><fmt:message
																key="test_rules_item_name" bundle="${resword}" />:</td>
														<td class="formValue"><c:out value='${itemName}' />&nbsp;
															<c:if test="${ruleSetId != null  && ruleSetId != '' }">
																<a href="ViewRuleSet?ruleSetId=${ruleSetId}" />(<fmt:message
																	key="test_rules_view_all_rules_for_item"
																	bundle="${resword}" />)</a>
															</c:if></td>
													</tr>
												...
												<c:if test="${itemDefinition != null}">
													<input type="hidden" name="itemDefinition"
														value="<c:out value="${itemDefinition}"/>" />
													<tr valign="top">
														<td class="formlabel"><fmt:message
																key="test_rules_item_definition" bundle="${resword}" />:</td>
														<td class="formValue"><c:out
																value='${itemDefinition}' /></td>
													</tr>
												...
												<tr valign="top">
													<td class="formlabel"><fmt:message
															key="test_rules_target" bundle="${resword}" />:</td>
													<td><textarea name="target" rows="3" cols="70"
															wrap="hard" style="font-size: 11px;"><c:out
																value='${target}' /></textarea></td>
												</tr>
												...
												<tr valign="top">
													<td class="formlabel"><fmt:message
															key="test_rules_expression" bundle="${resword}" />:</td>
													<td><textarea name="rule" rows="3" cols="70"
															wrap="hard" style="font-size: 11px;"><c:out
																value='${rule}' /></textarea></td>
												</tr>
...
-->
												...
												<tbody>
													...
													<c:forEach items="${sessionScope.testRuleActions}"
														...
														<tr valign="top">
															<td></td>
															<td class="formlabel"><fmt:message
																	key="test_rules_if_expression_evaluates"
																	bundle="${resword}" /></td>
															<td class="formValue"><select
																name="actions${status.count -1}">
																	<option value="1"
														...
														<c:forEach items="${testRuleAction.propertiesForDisplay}"
														...
														<c:if
															test="${testRuleAction.actionType.code!=1 && testRuleAction.actionType.code !=2 && testRuleAction.actionType.code !=7 && fn:length(testRuleAction.properties)>0}">
															<c:set var="props" value="" />
															<c:forEach items="${testRuleAction.properties}"
	...
	<c:if test="${fn:length(testValues) > 0}">
		<h3>
			<fmt:message key="test_rules_step_2" bundle="${resword}" />
		</h3>
		<div style="width: 650px">
			<!-- These DIVs define shaded box borders -->
			<div class="box_T">
										...
										<div class="box_BR">
											...
											<div class="textbox_center">
												<table border="0" cellpadding="0" cellspacing="0"
													...
													<c:forEach items="${sessionScope.testValues}"
														var="mapEntry">
														<c:set var="tooltipKey" value="${mapEntry.key}-tooltip" />
														<c:set var="dibKey" value="${mapEntry.key}-dib" />
														<c:choose>
															<c:when test="${dibKey}">
																<c:set var="dibItemDataType"
																	value='<%= ((org.akaza.openclinica.bean.submit.DisplayItemBean)request.getAttribute((String)pageContext.getAttribute("dibKey"))).getItem().getItemDataTypeId() %>' />
															</c:when>
															<c:otherwise>
																<c:set var="dibItemDataType"
																	value="${studyEventProperty}" />
															</c:otherwise>
														</c:choose>
														...
														<c:choose>
															<c:when test="${fn:endsWith(mapEntry.key,'.STATUS')}">
																<tr valign="top">
																	<td class="formlabel"><script>spliceAndReturn('${mapEntry.key}')</script>:</td>
																	<td style="color: #789EC5;"><input
																		name="${mapEntry.key}" value="${mapEntry.value}"
																		id="${mapEntry.key}" />
															</c:when>
															...
															<c:when test="${fn:endsWith(mapEntry.key,'.STARTDATE')}">
																...
																<tr valign="top">
																	<td class="formlabel"><a
																		href="javascript: displayEventLink('${mapEntry.key}')"
																		onmouseover="Tip('<%= request.getAttribute((String)pageContext.getAttribute("tooltipKey") ) %>')"
																		onmouseout="UnTip()"> <script>spliceAndReturn('${mapEntry.key}')</script></a>:</td>
																	<td style="color: #789EC5;"><input
																		name="${mapEntry.key}" value="${mapEntry.value}"
																		id="${mapEntry.key}" />
															</c:when>
														...
														<tr>
															...
															<td colspan="2">&nbsp;<jsp:include
																	page="../showMessage.jsp"><jsp:param
																		name="key" value="${mapEntry.key}" /></jsp:include></td>
														</tr>
													</c:forEach>
	...
	<c:if test="${ruleEvaluatesTo != null}">
		<h3>
			<fmt:message key="test_rules_step_3" bundle="${resword}" />
		</h3>
		<div style="width: 650px">
			<!-- These DIVs define shaded box borders -->
			<div class="box_T">
										...
										<div class="box_BR">
											...
											<div class="textbox_center">
												<table border="0" cellpadding="0" cellspacing="0">
													...
													<c:choose>
														<c:when
															test="${targetFail == 'on' || ruleValidation == 'rule_invalid' }">
															<tr valign="top">
																<td class="formlabel"><fmt:message
																		key="test_rules_rule_validation" bundle="${resword}" /></td>
																<td class="formValue"><fmt:message
																		key="test_rules_${ruleValidation}" bundle="${resword}" />
																	<c:if test="${ruleValidationFailMessage != null }"> - ${ruleValidationFailMessage}</c:if></td>
															</tr>
												...
												<c:when test="${validate == null}">
													...
													<tr valign="top">
														<td class="formlabel"><fmt:message
																key="test_rules_expression_evaluates_to"
																bundle="${resword}" /></td>
													...
													<tr valign="top">
														<td class="formlabel"><fmt:message
																key="test_rules_ran_in" bundle="${resword}" /></td>
													...
													<c:if test="${actionFired == 'Y'}">
														<tr valign="top">
															...
															<td class="formValue"><fmt:message
																	key="test_rules_will_execute" bundle="${resword}" /></td>
														</tr>
													</c:if>
													<c:if test="${actionFired == 'N'}">
														<tr valign="top">
															<td class="formlabel"><fmt:message
																	key="test_rules_action_summary" bundle="${resword}" /></td>
															<td class="formValue"><fmt:message
																	key="test_rules_will_not_execute" bundle="${resword}" /></td>
														</tr>
													</c:if>
													...
													<table border="0" cellpadding="0" cellspacing="0">
														...
														<c:forEach items="${sessionScope.testRuleActions}"
															...
															<c:if
																...
																<c:forEach
																	items="${testRuleAction.propertiesForDisplay}"
																	var="mapEntry">
																	<tr valign="top">
												...
												<c:otherwise>
													...
													<tr valign="top">
														<td class="formlabel"><fmt:message
																key="test_rules_actions_fired" bundle="${resword}" /></td>
														<td class="formValue"><fmt:message
																key="test_rules_validate_message" bundle="${resword}" /></td>
													</tr>
...
<br>
<p>
	<a href="ViewRuleAssignment?restore=true" />
	<fmt:message key="test_rules_back_to_rule_assignments"
		bundle="${resword}" />
	</a>
</p>
<br>
