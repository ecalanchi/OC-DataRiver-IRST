...
<jsp:useBean scope='request' id='strUserAccountId'
...
<title><fmt:message key="openclinica" bundle="${resword}" />- <fmt:message
		key="add_discrepancy_note" bundle="${resword}" /></title>
...
<body style="margin: 0px 12px 0px 12px;"
	onload="javascript:setStatus('<c:out value="${discrepancyNote.discrepancyNoteTypeId}"/>','<c:out value="${whichResStatus}"/>','<fmt:message key="New" bundle="${resterm}"/>','<fmt:message key="Updated" bundle="${resterm}"/>','<fmt:message key="Resolution_Proposed" bundle="${resterm}"/>','<fmt:message key="Closed" bundle="${resterm}"/>','<fmt:message key="Not_Applicable" bundle="${resterm}"/>');">
	...
	<div style="float: right;">
		<a href="#" onclick="javascript:window.close();"><img
			name="close_box"
			alt="<fmt:message key="Close_Box" bundle="${resword}"/>"
			src="images/bt_Remove.gif" class="icon_dnBox"></a>
	</div>
	...
	<form name="noteForm" method="POST" action="CreateDiscrepancyNote">
		...
		<input type="hidden" name="name"
			value="<c:out value="${discrepancyNote.entityType}"/>"> <input
			type="hidden" name="column"
			value="<c:out value="${discrepancyNote.column}"/>"> <input
			type="hidden" name="parentId"
			value="<c:out value="${discrepancyNote.parentDnId}"/>"> <input
			type="hidden" name="id"
			value="<c:out value="${discrepancyNote.entityId}"/>"> <input
			type="hidden" name="subjectId"
			value="<c:out value="${param.subjectId}"/>"> <input
			type="hidden" name="field"
			value="<c:out value="${discrepancyNote.field}"/>"> <input
			type="hidden" name="writeToDB" value="<c:out value="${writeToDB}" />">
		<input type="hidden" name="monitor"
			value="<c:out value="${monitor}" />"> <input type="hidden"
			name="enterData" value="<c:out value="${enterData}" />"> <input
			type="hidden" name="eventCRFId"
			value="<c:out value="${eventCRFId}"/>">
													...
													<table border="0" cellpadding="0" cellspacing="0">
														<tr>
															...
															<td class="table_cell_noborder"><b><c:out
																		value="${discrepancyNote.subjectName}" /></b></td>
															<td class="table_cell_noborder"
																style="padding-left: 40px;"><fmt:message
																	key="event" bundle="${resword}" />:&nbsp;&nbsp;</td>
															<td class="table_cell_noborder"><b><c:out
																		value="${discrepancyNote.eventName =='' ? 'N/A' : discrepancyNote.eventName}" /></b></td>
														</tr>
														<tr>
															<td class="table_cell_noborder"><fmt:message
															...
															<td class="table_cell_noborder"><c:choose>
																	...
																	<c:otherwise>
																		<b><fmt:formatDate
																				value="${discrepancyNote.eventStart}"
																				pattern="${dteFormat}" /></b>
																	</c:otherwise>
																</c:choose></td>
															<td class="table_cell_noborder"
																style="padding-left: 40px;"><fmt:message key="CRF"
																	bundle="${resword}" />:&nbsp;&nbsp;</td>
															<td class="table_cell_noborder"><b><c:out
																		value="${discrepancyNote.crfName == '' ? 'N/A' : discrepancyNote.crfName}" /></b></td>
														</tr>
														<tr>
															<td class="table_cell_noborder"><fmt:message
															...
															<td class="table_cell_noborder"
																style="padding-left: 40px;"><fmt:message key="More"
																	bundle="${resword}" />:&nbsp;&nbsp;</td>
															<td class="table_cell_noborder"><c:choose>
																	<c:when
																		...
																		<a
																			...
																			<fmt:message key="Data_Dictionary"
																				bundle="${resword}" />
																		</a>
																	</c:when>
																	<c:otherwise>
		...
		<h3 class="title_manage">
			<fmt:message key="add_note" bundle="${resword}" />
		</h3>
								...
								<div class="box_BL">
									<div class="box_BR">
										<div class="textbox_center">
												...
												<span id="description">
													<div class="formfieldXL_BG">
														<input type="text" name="description"
															value="<c:out value="${discrepancyNote.description}"/>"
															class="formfieldXL">
											...
											<div class="dnBoxCol1 dnBoxText">
												<fmt:message key="detailed_note" bundle="${resword}" />
												:
											</div>
											<div class="dnBoxCol2 dnBoxText">
												<div class="formtextareaXL4_BG">
													<textarea name="detailedDes" rows="4" cols="50"
														class="formtextareaXL4"><c:out
															value="${discrepancyNote.detailedNotes}" /></textarea>
												</div>
												<jsp:include page="../showMessage.jsp"><jsp:param
														name="key" value="detailedDes" /></jsp:include>
											</div>
												...
												<div class="formfieldL_BG">
													<c:choose>
														...
														<c:otherwise>
															...
															<jsp:include page="../showMessage.jsp"><jsp:param
																	name="key" value="typeId" /></jsp:include>
														</c:otherwise>
													</c:choose>
												</div>
											</div>
											...
											<span id="res1${parentId}">
												<div class="dnBoxCol1 dnBoxText">
													<fmt:message key="Set_to_Status" bundle="${resword}" />
													:<span class="alert">*</span>
												</div>
												<div class="dnBoxCol2 dnBoxText">
													<div class="formfieldL_BG">
														<c:set var="resStatusIdl"
															value="${discrepancyNote.resolutionStatusId}" />
														<select name="resStatusId" id="resStatusId"
															class="formfieldL">
													...
													<jsp:include page="../showMessage.jsp"><jsp:param
															name="key" value="resStatusId" /></jsp:include>
												</div>
											</span>

											<c:choose>
												<c:when
													test="${(parent == null || parent.id ==0 || unlock == 1) && autoView == 0}">
											...
											<div class="dnBoxCol2 dnBoxText">
												<div class="formfieldL_BG">
													<c:choose>
														<c:when test='${discrepancyNote.assignedUserId != ""}'>
															<c:set var="userAccountId1"
																value="${discrepancyNote.assignedUserId}" />
														</c:when>
														<c:otherwise>
															<c:set var="userAccountId1" value="0" />
														</c:otherwise>
													</c:choose>
													<select name="userAccountId" id="userAccountId"
														class="formfieldL">
														<option value="0">
															<c:forEach var="user" items="${userAccounts}">
																<c:choose>
																	<c:when test="${userAccountId1 == user.userAccountId}">
																		<option value="<c:out value="${user.userAccountId}"/>"
																			selected><c:out value="${user.lastName}" />,
																			<c:out value="${user.firstName}" /> (
																			<c:out value="${user.userName}" />)
																	</c:when>
																	<c:otherwise>
																		<option value="<c:out value="${user.userAccountId}"/>"><c:out
																				value="${user.lastName}" />,
																			<c:out value="${user.firstName}" /> (
																			<c:out value="${user.userName}" />)
																	</c:otherwise>
												...
												<jsp:include page="../showMessage.jsp"><jsp:param
														name="key" value="userAccountId" /></jsp:include>
											</div>
											</span>
											...
											<div class="dnBoxCol1 dnBoxText">
												<fmt:message key="email_assigned_user" bundle="${resword}" />
												:
											</div>
											<div class="dnBoxCol2 dnBoxText">
											...
											<c:set var="noteEntityType"
												value="${discrepancyNote.entityType}" />
											<c:if
												test="${enterData == '1' || canMonitor == '1' || noteEntityType != 'itemData' }">
												<c:choose>
													<c:when test="${writeToDB eq '1'}">
														<div class="dnBoxCol2">
															<input type="submit" name="SubmitExit"
																value="<fmt:message key="submit_close" bundle="${resword}"/>"
																class="button_medium"
																onclick="javascript:setValue('close<c:out value="${parentId}"/>','true');">
														</div>
													</c:when>
													<c:otherwise>
														<div class="dnBoxCol2">
