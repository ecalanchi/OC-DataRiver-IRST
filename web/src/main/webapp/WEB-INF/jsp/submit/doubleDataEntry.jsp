...
<head>
<title>OpenClinica <fmt:message key="double_data_entry"
...
<body class="aka_bodywidth"
	onload="document.getElementById('CRF_infobox_closed').style.display='block';document.getElementById('CRF_infobox_open').style.display='none';"
    ...
    improve the style as well -->
			...
			<div id="box" class="dialog" style="display: none;">
				<span id="mbm"> <fmt:message
						key="crf_data_entry_password_required" bundle="${restext}" />
				</span><br>
					...
					<c:if test="${fn:length(toc.sections) gt 1}">
						...
						<td>&nbsp;
							<div class="formfieldM_BG_noMargin">
								<select class="formfieldM" name="sectionName" size="1"
									onchange="gotoLink();">
									...
									<c:forEach var="sec" items="${toc.sections}">
										<c:set var="tabUrl"
											value="DoubleDataEntry?eventCRFId=${section.eventCRF.id}&sectionId=${sec.id}&tab=${tabCount}&exitTo=${exitTo}" />
																		...
																		<c:if
																			...
																			<c:if test="${displayItemNum ==0}">
																				...
																				<tr class="aka_stripes">
																					...
																					<td class="aka_header_border" colspan="2">
																						<table border="0" cellpadding="0" cellspacing="0"
																							width="100%" style="margin-bottom: 6px;">
																							<tr>
																						...
																						<c:if test="${! (repeatParentId eq 'Ungrouped')}">
																							...
																							<c:if
																								...
																								<div class="aka_group_header">
																									<strong><c:out
																											value="${displayItem.itemGroup.groupMetaBean.header}" /></strong>
																								</div>
																							</c:if>
     ...
     horizontal for checkboxes. --%>
      ...
      thItem.metadata.responseLayout eq 'Horizontal'}">
																											...
																											<c:choose>
																												...
																												<c:when
																													test="${isHorizontalCellLevel &&  (thItem.metadata.responseSet.responseType.name eq 'checkbox' ||
      ...
      bodyItem.metadata.responseLayout eq 'Horizontal'}">
																														...
																														<c:choose>
																															...
																															<c:when
																																test="${isHorizontalCellLevel && (bodyItem.metadata.responseSet.responseType.name eq 'radio' ||
           bodyItem.metadata.responseSet.responseType.name eq 'checkbox')}">
               ...
               will be in the name --%> <c:choose>
																													...
																													</c:choose> <!--  <button stype="remove" type="button" template="<c:out value="${repeatParentId}"/>" class="button_remove"></button> -->
																												</td>
      ...
      bodyItem.metadata.responseLayout eq 'Horizontal'}">
           ...
           bodyItem.metadata.responseSet.responseType.name eq 'checkbox')}">
																																...
																																<c:otherwise>
																																	<td
																																		...
																																		<!-- link text here --> <c:set
																																			...
																																			<c:param name="originJSP"
																																				value="doubleDataEntry" />
																																		</c:import> <!-- link text here --> <c:import
																																			url="../submit/generateGroupItemTxt.jsp">
																																			<c:param name="itemId"
																																				value="${bodyItem.item.id}" />
																																			<c:param name="inputType"
				...
				<td class="table_cell_left">
					...
					</table> <%-- END IF NEW TABLE --%> <!--   return to top table:
