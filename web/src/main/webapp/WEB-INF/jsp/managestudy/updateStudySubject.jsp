...
<body class="aka_bodywidth"
		...
		<c:when
			...
			<h1>
				<span class="title_manage"> <fmt:message
						key="update_study_subject_details" bundle="${resword}" />
				</span>
			</h1>
		</c:when>
		<c:otherwise>
			<h1>
				<span class="title_manage"> <fmt:message
						key="assign_subject_to_group" bundle="${resworkflow}" />
				</span>
			</h1>
		</c:otherwise>
	</c:choose>
	...
	<form action="UpdateStudySubject" method="post">
		<input type="hidden" name="action" value="confirm"> <input
			type="hidden" name="id" value="<c:out value="${studySub.id}"/>">
		<c:choose>
			<c:when
				test="${userBean.techAdmin || userBean.sysAdmin || userRole.manageStudy || userRole.investigator 
    || (study.parentStudyId > 0 && userRole.researchAssistant ||study.parentStudyId > 0 && userRole.researchAssistant2)}">
																...
																<td>
																	<div class="formfieldXL_BG">
																		<input type="text" name="label"
																			value="<c:out value="${studySub.label}"/>"
																			class="formfieldXL">
																	</div> <br> <jsp:include page="../showMessage.jsp"><jsp:param
																			name="key" value="label" /></jsp:include>
																</td>
																<td><fmt:message key="field_required"
																		bundle="${resword}" /></td>
															</tr>
															<tr valign="top">
																<td class="formlabel"><fmt:message
																...
																<td>
																	<div class="formfieldXL_BG">
																		...
																		<input type="text" name="enrollmentDate"
																			...
																			class="formfieldXL" id="enrollmentDateField">
																	</div> <br><jsp:include page="../showMessage.jsp"><jsp:param
	      ...
	      Calendar.setup({inputField  : "enrollmentDateField", ifFormat    : "<fmt:message key="date_format_calender" bundle="${resformat}"/>", button      : "enrollmentDateTrigger" });
																		...
																		<a href="#"
																			onClick="openDSNoteWindow('CreateDiscrepancyNote?subjectId=${studySub.id}&id=<c:out value="${studySub.id}"/>&name=studySub&field=enrollmentDate&column=enrollment_date','spanAlert-enrollmentDate'); return false;"><img
																			id="flag_enrollmentDate" name="flag_enrollmentDate"
														...
														<table border="0" cellpadding="0" cellspacing="0"
															...
															<tr valign="top">
																<td class="table_header_column"><fmt:message
																		key="label" bundle="${resword}" />:</td>
																<td class="table_cell"><input type="text"
																	name="label" value="<c:out value="${studySub.label}"/>"
																	disabled="disabled" class="formfieldM"></td>
															</tr>
															<tr valign="top">
																<td class="table_header_column"><fmt:message
																		key="secondary_ID" bundle="${resword}" />:</td>
																		...
																		<tr valign="top">
																			...
																			<td><div class="formfieldM_BG">
																					<select
																						...
																						class="formfieldM">
																						<c:if
																							test="${group.subjectAssignment=='Optional'}">
																				...
																				</div> <c:if test="${group.subjectAssignment=='Required'}">
																					<td align="left">*</td>
																				</c:if></td>
																		</tr>
																		<tr valign="top">
		...
		<br> <input type="submit" name="Submit"
			value="<fmt:message key="confirm_changes" bundle="${resword}"/>"
