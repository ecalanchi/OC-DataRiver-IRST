...
<tr valign="top">
	...
	<td class="table_cell">
		<table border="0" cellpadding="0" cellspacing="0">
			<tr>
				<td><a
					href="ViewSubjectGroupClass?id=<c:out value="${currRow.bean.id}"/>"
					...
					<c:choose>
						<c:when test="${!currRow.bean.status.deleted}">
							<c:if test="${!study.status.locked}">
								<td><a
									href="UpdateSubjectGroupClass?id=<c:out value="${currRow.bean.id}"/>"
								...
								<td><a
									href="RemoveSubjectGroupClass?action=confirm&id=<c:out value="${currRow.bean.id}"/>"
						...
						<c:otherwise>
							<td><c:if test="${!study.status.locked}">
									<a
										href="RestoreSubjectGroupClass?action=confirm&id=<c:out value="${currRow.bean.id}"/>"
