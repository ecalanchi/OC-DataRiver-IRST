...
<tr valign="top">
		...
		<c:when
			...
			<td class="table_cell"
				id="Groups_0_1_<c:out value="${eblRowCount+1}"/>"><c:choose>
					<c:when
						test="${currRow.bean.studySubject.status.id==1 || currRow.bean.studySubject.status.id==8 }">
						<c:if test="${currRow.bean.studySubject.status.id==1}">
							<fmt:message key="active" bundle="${resword}" />
						</c:if>
						<c:if test="${currRow.bean.studySubject.status.id==8}">
	...
	<c:forEach var="event" items="${currRow.bean.studyEvents}">
		...
		<c:set var="discBeanList" scope="request"
			value="${event.discBeanList}" />


		<c:if test="${! (empty discBeanList)}">
...
<tr valign="top">
    <td>--%> <%-- ${currRow.bean.studySubject.id} : displaystudysubject
   event: studyevent--%>
			<div style="margin-bottom: 5px;">
				<c:choose>
					<c:when test="${event.subjectEventStatus.id == 1}">
						<img name="icon_Note" src="images/icon_Scheduled.gif" border="0"
					...
					<c:when test="${event.subjectEventStatus.id == 2}">
						<img name="icon_Note" src="images/icon_NotStarted.gif" border="0"
							alt="<fmt:message key="not_scheduled" bundle="${resterm}"/>"
							title="<fmt:message key="not_scheduled" bundle="${resterm}"/>"
							align="left" />
					</c:when>
					<c:when test="${event.subjectEventStatus.id == 3}">
						<img name="icon_Note" src="images/icon_InitialDE.gif" border="0"
							alt="<fmt:message key="data_entry_started" bundle="${resterm}"/>"
							title="<fmt:message key="data_entry_started" bundle="${resterm}"/>"
							align="left" />
					</c:when>
					<c:when test="${event.subjectEventStatus.id == 4}">
						<img name="icon_Note" src="images/icon_DEcomplete.gif" border="0"
							alt="<fmt:message key="completed" bundle="${resterm}"/>"
							title="<fmt:message key="completed" bundle="${resterm}"/>"
							align="left" />
					</c:when>
					<c:when test="${event.subjectEventStatus.id == 5}">
						<img name="icon_Note" src="images/icon_Stopped.gif" border="0"
							alt="<fmt:message key="stopped" bundle="${resterm}"/>"
							title="<fmt:message key="stopped" bundle="${resterm}"/>"
							align="left" />
					</c:when>
					<c:when test="${event.subjectEventStatus.id == 6}">
						<img name="icon_Note" src="images/icon_Skipped.gif" border="0"
							alt="<fmt:message key="skipped" bundle="${resterm}"/>"
							title="<fmt:message key="skipped" bundle="${resterm}"/>"
							align="left" />
					</c:when>
					<c:when test="${event.subjectEventStatus.id == 7}">
						<img name="icon_Note" src="images/icon_Locked.gif" border="0"
					...
					<c:when test="${event.subjectEventStatus.id == 8}">
						<img name="icon_Note" src="images/icon_Signed.gif" border="0"
							alt="<fmt:message key="signed" bundle="${resterm}"/>"
							title="<fmt:message key="signed" bundle="${resterm}"/>"
							align="left" />
					</c:when>
				</c:choose>
			</div> <br /> <%-- keep track of the number of disc notes of each status --%>
			<c:set var="newDN" value="0" /> <c:set var="updatedDN" value="0" /> <c:set
				var="resolvedDN" value="0" /> <c:set var="closedDN" value="0" /> <c:set
				var="not_applicableDN" value="0" /> <c:set var="newDisplayed"
				value="false" /> <c:set var="updatedDisplayed" value="false" /> <c:set
				var="resolvedDisplayed" value="false" /> <c:set
				var="closedDisplayed" value="false" /> <c:set
				var="not_applicableDisplayed" value="false" /> <%--    dn: ${hasDiscNotes}--%>
			...
			</c:forEach> <c:forEach var="discNote" items="${discBeanList}">
					...
					<c:when test="${discNote.resolutionStatusId == 4}">
						<c:if test="${closedDisplayed eq 'false'}">
							<img name="icon_flagBlack" src="images/icon_flagBlack.gif"
								border="0" alt="<fmt:message key="Closed" bundle="${resterm}"/>"
   ...
   <td valign="top">--%> <c:if test="${event.repeatingNum>1}">
				<!--&nbsp;-->
				<br />
				<strong style="text-align: left">[<c:out
						value="${event.repeatingNum}" />]
				</strong>
			</c:if> <!--</td>

    </tr>
</table>&nbsp;--> <%-- For Internet Explorer, which doesn't display some table cell's borders if they don't
  have any content--%> <c:if test="${! hasDiscNotes}">&nbsp;</c:if>
	...
	<td class="table_cell" style="white-space: nowrap"><c:set
			var="boolResStatus" value="${false}" /> <%-- If this HashSet has any content, then the boolResStatus value is set to true--%>
		<c:forEach var="resStatus" items="${resolutionStatusFromServlet}">
			<c:set var="boolResStatus" value="${true}" />
		</c:forEach> <c:set var="booldiscNoteType"
			value="${discNoteType >= 1 && discNoteType <= 7}" /> <%--    <table border="0" cellpadding="0" cellspacing="0">
<tr>
    <td>--%> <c:choose>
			<c:when test="${! boolResStatus}">
				<a
					...
					src="images/bt_View.gif" width="24 " height="15" border="0"
					alt="<fmt:message key="view" bundle="${resword}"/>"
					title="<fmt:message key="view" bundle="${resword}"/>" align="left"></a>

				<!--align="left" -->
				<c:if test="${hasDiscNotes}">
					<a
						href="javascript:openDocWindow('ChooseDownloadFormat?subjectId=<c:out value="${currRow.bean.studySubject.id}"/>&discNoteType=${discNoteType}&module=${module}')"><img
						hspace="2" width="24 " height="15" name="bt_View1"
						src="images/bt_Download.gif" border="0"
			...
			<c:otherwise>
				<a
					href="ViewNotes?viewForOne=y&id=<c:out value="${currRow.bean.studySubject.id}"/>&resolutionStatus=${param.resolutionStatus}&discNoteType=${discNoteType}&module=${module}"
					onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"
					onMouseUp="javascript:setImage('bt_View1','images/bt_View.gif');"><img
					hspace="4" style="float: left" width="24 " height="15"
				...
				<c:if test="${hasDiscNotes}">
					<a
						href="javascript:openDocWindow('ChooseDownloadFormat?subjectId=<c:out value="${currRow.bean.studySubject.id}"/>&discNoteType=${discNoteType}&resolutionStatus=${param.resolutionStatus}')"><img
						hspace="4" width="24 " height="15" name="bt_View1"
						src="images/bt_Download.gif" border="0"
						alt="<fmt:message key="download_discrepancy_notes" bundle="${resword}"/>"
						title="<fmt:message key="download_discrepancy_notes" bundle="${resword}"/>"></a>
