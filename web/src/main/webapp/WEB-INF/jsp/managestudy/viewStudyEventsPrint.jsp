...
<html>
<head>
<title><fmt:message key="openclinica_print_study_events"
...
<body
	...
	<c:forEach var="eventView" items="${allEvents}">
		...
		<p>
			<b><fmt:message key="event_type" bundle="${resword}" /></b>:
			<fmt:message key="${eventView.definition.type}" bundle="${resword}" />
			,
			...
			<br> <b><fmt:message key="subjects_who_scheduled"
					bundle="${resword}" /></b>:
			<c:out value="${eventView.subjectScheduled}" />
			(
			<fmt:message key="start_date_of_first_event" bundle="${resword}" />
			:
			<c:choose>
			...
			) <b><fmt:message key="completed" bundle="${resword}" /></b>:
			<c:out value="${eventView.subjectCompleted}" />
			(
			<fmt:message key="completion_date_of_last_event" bundle="${resword}" />
			:
			<c:choose>
			...
			) <b><fmt:message key="discontinued" bundle="${resword}" /></b>:
			<c:out value="${eventView.subjectDiscontinued}" />
			<br>
		</p>
											...
											<div class="textbox_center">
												...
												<table border="0" cellpadding="0" cellspacing="0"
													...
													<tr>
														...
														<td class="table_header_row"><fmt:message
																key="event_date_started" bundle="${resword}" /></td>
														<td class="table_header_row"><fmt:message
													...
													<c:forEach var="currRow" items="${events}">
														...
														<td class="table_cell"><c:out
																value="${currRow.studySubjectLabel}" /></td>
														<td class="table_cell"><fmt:formatDate
																value="${currRow.dateStarted}" type="both"
																timeStyle="short" pattern="${dteFormat}" /></td>
														<td class="table_cell"><c:out
																value="${currRow.subjectEventStatus.name}" /></td>
