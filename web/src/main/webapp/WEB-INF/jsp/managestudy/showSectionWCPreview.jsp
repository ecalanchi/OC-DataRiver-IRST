...
<table border="0" cellpadding="0" cellspacing="0">
														...
														<c:otherwise>
															...
															<table border="0" cellpadding="0" cellspacing="0"
																...
																<c:forEach var="displayItem" items="${section.items}"
																	...
																	<c:if
																		test="${currPage != displayItem.metadata.pageNumberLabel}">
																				...
																				<table border="0" cellpadding="0" cellspacing="0"
																					...
																					<tr>
																						...
																						<td width="75%" style="padding-left: 12px;">
																							<b> <c:if
																									test="${section.section.parent.active}">
																									<c:out value="${section.section.parent.title}"
																										escapeXml="false" /> &gt;
              </c:if> <c:out value="${section.section.title}" escapeXml="false" />
																						</b>
																						</td>
																						<td width="20%" align="right" valign="top"
																							nowrap="nowrap"><a
																							name="item<c:out value="${displayItem.item.id}"/>">
																								<fmt:message key="form_page" bundle="${resword}" />:
																								<c:out
																					...
																					<c:if test='${section.section.instructions != ""}'>
																						<tr class="aka_stripes">
																							<td width="5%" nowrap><fmt:message
																									key="instructions" bundle="${resword}" />:</td>
																							<td width="75%" style="padding-left: 12px;">
																								<c:out value="${section.section.instructions}"
																									escapeXml="false" />
																							</td>
																						</tr>
																					</c:if>
	...
	<c:if test="${!empty displayItem.metadata.header}">
		<tr class="aka_stripes">
			<td class="table_cell_left aka_header_border" bgcolor="#F5F5F5"><b><c:out
						value="${displayItem.metadata.header}" escapeXml="false" /></b></td>
		</tr>
	</c:if>
	<c:if test="${!empty displayItem.metadata.subHeader}">
		<tr class="aka_stripes">
			<td class="table_cell_left aka_header_border"><c:out
						...
						<table border="0">
							<tr>
								<td valign="top" class="text_block"><c:out
										value="${displayItem.metadata.questionNumberLabel}" /> <c:out
								...
								<td valign="top">
									<%-- display the HTML input tag --%> <c:set var="displayItem"
										...
										url="../submit/showItemInputPreview.jsp">
										<c:param name="defaultValue"
											value="${displayItem.metadata.defaultValue}" />
										<c:param name="respLayout"
											value="${displayItem.metadata.responseLayout}" />
									</c:import> <c:import url="../showMessage.jsp">
						...
						<td valign="top">
							<table border="0">
								<tr>
									...
									<td valign="top" class="text_block"><c:out
											value="${childItem.metadata.questionNumberLabel}" /> <c:out
											value="${childItem.metadata.leftItemText}" escapeXml="false" /></td>
									<%--<td style="width: 200px;" valign="top">--%>
									<td valign="top">
										<%-- display the HTML input tag --%> <c:set var="displayItem"
											scope="request" value="${childItem}" /> <!--  changed?--> <c:import
											url="../submit/showItemInputPreview.jsp">
											<c:param name="defaultValue"
												value="${displayItem.metadata.defaultValue}" />
											<c:param name="respLayout"
