...
<jsp:useBean scope="request" id="currRow"
	class="org.akaza.openclinica.web.bean.StudyRow" />
...
<tr valign="top">
	...
	<td class="table_cell"><c:out
			value="${currRow.bean.principalInvestigator}" /></td>
					...
					<c:choose>
						<c:when test="${(!currRow.bean.status.deleted)}">
							<c:if test="${!study.status.locked}">
								<td><a
									href="InitUpdateSubStudy?id=<c:out value="${currRow.bean.id}"/>"
								...
								<td><a
									href="RemoveSite?action=confirm&id=<c:out value="${currRow.bean.id}"/>"
						...
						<c:otherwise>
							<c:if test="${!study.status.locked}">
								<td><a
									href="RestoreSite?action=confirm&id=<c:out value="${currRow.bean.id}"/>"
