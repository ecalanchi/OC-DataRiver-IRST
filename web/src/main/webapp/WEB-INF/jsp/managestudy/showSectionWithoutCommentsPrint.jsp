											...
											<c:forEach var="displayItem" items="${section.items}"
													...
													<tr>
														<td class="table_header_row_left">
															<table border="0" cellpadding="0" cellspacing="0"
																width="100%">
																<tr>
																	<td width="5%" nowrap><fmt:message key="section"
																			bundle="${resword}" />:</td>
																	<td width="75%" style="padding-left: 12px;"><b>
																...
																<c:if test='${section.section.subtitle != ""}'>
																	<tr>
																		<td><fmt:message key="subtitle"
																				bundle="${resword}" />:</td>
																		<td><c:out value="${section.section.subtitle}"
																				escapeXml="false" /></td>
																	</tr>
																</c:if>
																<c:if test='${section.section.instructions != ""}'>
																	<tr>
																		...
																		<td width="75%" style="padding-left: 12px;"><c:out
												...
												<c:if test="${displayItem.metadata.parentId == 0}">
													<c:if test="${!empty displayItem.metadata.header}">
														<tr>
															<td class="table_cell_left"><b><c:out
																		value="${displayItem.metadata.header}"
													...
													<c:if test="${!empty displayItem.metadata.subHeader}">
														<tr>
															<td class="table_cell_left"><c:out
																	value="${displayItem.metadata.subHeader}"
																	escapeXml="false" /></td>
														</tr>
													</c:if>
													...
													<tr>
														<td valign="top"></c:if> <c:if
															...
															<table border="0">
																<tr>
																	<td valign="top"><c:out
																			value="${displayItem.metadata.questionNumberLabel}" /></td>
																	<td valign="top"><c:out
																			value="${displayItem.metadata.leftItemText}" /></td>
																	<td valign="top">
																		<%-- display the HTML input tag --%> <c:set
																			var="displayItem" scope="request"
																			value="${displayItem}" /> <c:import
																			url="../submit/showItemInputPrint.jsp" /> <br />
																	<c:import url="../showMessage.jsp">
																			<c:param name="key"
																				value="input${displayItem.item.id}" />
																		</c:import>
																	</td>
																	<c:if test='${displayItem.item.units != ""}'>
																		<td valign="top"><c:out
																				value="(${displayItem.item.units})" /></td>
																	</c:if>
																	<td valign="top"><c:out
																			value="${displayItem.metadata.rightItemText}" /></td>
																</tr>
															</table></td>
					...
					--%>
															...
															<td valign="top">
																<table border="0">
																	<tr>
																		<td valign="top"><c:out
																				value="${childItem.metadata.questionNumberLabel}" /></td>
																		<td valign="top"><c:out
																				value="${childItem.metadata.leftItemText}" /></td>
																		<td valign="top">
																			<%-- display the HTML input tag --%> <c:set
																				var="displayItem" scope="request"
																				value="${childItem}" /> <c:import
																				url="../submit/showItemInputPrint.jsp" /> <br />
																		<c:import url="../showMessage.jsp">
																				<c:param name="key"
																					value="input${childItem.item.id}" />
																			</c:import>
																		</td>
																		<c:if test='${childItem.item.units != ""}'>
																			<td valign="top"><c:out
																					value="(${childItem.item.units})" /></td>
																		</c:if>
																		<td valign="top"><c:out
																				value="${childItem.metadata.rightItemText}" /></td>
																	</tr>
																</table>
															</td>
														</c:forEach>
