...
<jsp:useBean scope="request" id="currRow"
	class="org.akaza.openclinica.web.bean.StudyEventDefinitionRow" />
...
<tr valign="top">
	<td class="table_cell_left">
		<%--<c:out value="${currRow.bean.ordinal}"/>--%> <c:choose>
					...
					<c:when test="${count==0}">
						<c:choose>
							<c:when test="${defSize>1}">
								<a
									href="ChangeDefinitionOrdinal?current=<c:out value="${nextRow.bean.id}"/>&previous=<c:out value="${currRow.bean.id}"/>"><img
									src="images/bt_sort_descending.gif" border="0"
									alt="<fmt:message key="move_down" bundle="${resword}"/>"
									title="<fmt:message key="move_down" bundle="${resword}"/>" /></a>
							</c:when>
							<c:otherwise>
            &nbsp;
           </c:otherwise>
					...
					<c:when test="${count==last}">
						<a
							href="ChangeDefinitionOrdinal?current=<c:out value="${currRow.bean.id}"/>&previous=<c:out value="${prevRow.bean.id}"/>"><img
							src="images/bt_sort_ascending.gif"
							alt="<fmt:message key="move_up" bundle="${resword}"/>"
					...
					<c:otherwise>
						<a
							...
							border="0" /></a>
						<a
							href="ChangeDefinitionOrdinal?previous=<c:out value="${currRow.bean.id}"/>&current=<c:out value="${nextRow.bean.id}"/>"><img
	...
	<td class="table_cell"><c:choose>
			<c:when test="${currRow.bean.repeating == true}">
	...
	<td class="table_cell"><fmt:message key="${currRow.bean.type}"
			bundle="${resword}" />&nbsp;</td>
	<td class="table_cell"><c:out value="${currRow.bean.category}" />&nbsp;</td>
	<td class="table_cell"><c:choose>
			<c:when test="${currRow.bean.populated == true}">
				<fmt:message key="yes" bundle="${resword}" />
			</c:when>
			<c:otherwise>
				<fmt:message key="no" bundle="${resword}" />
	...
	<td class="table_cell"><fmt:formatDate
			value="${currRow.bean.createdDate}" pattern="${dteFormat}" /><br>(<c:out
			value="${currRow.bean.owner.name}" />)</td>
	<td class="table_cell"><fmt:formatDate
			value="${currRow.bean.updatedDate}" pattern="${dteFormat}" /><br>(<c:out
			value="${currRow.bean.updater.name}" />)</td>
	<td class="table_cell" style="display: none; white-space: nowrap"
		id="Groups_0_9_<c:out value="${eblRowCount+1}"/>"><c:forEach
			var="entry" items="${currRow.bean.crfsWithDefaultVersion}"
			varStatus="status">
	...
	<td class="table_cell" style="display: none; white-space: nowrap"
		id="Groups_0_10_<c:out value="${eblRowCount+1}"/>"><c:forEach
			var="entry" items="${currRow.bean.crfsWithDefaultVersion}"
			varStatus="status">
	...
	<td class="table_cell">
		<table border="0" cellpadding="0" cellspacing="0">
			<tr>
				<td><a
					href="ViewEventDefinition?id=<c:out value="${currRow.bean.id}"/>"
						...
						<c:choose>
							<c:when test="${currRow.bean.status.available}">
								<c:if test="${!study.status.locked}">
									<td><a
										href="InitUpdateEventDefinition?id=<c:out value="${currRow.bean.id}"/>"
									...
									<td><a
										href="RemoveEventDefinition?action=confirm&id=<c:out value="${currRow.bean.id}"/>"
		...
		<c:if test="${currRow.bean.lockable}">
		  <td><a href="LockEventDefinition?action=confirm&id=<c:out value="${currRow.bean.id}"/>"
			...
			name="bt_Lock1" src="images/bt_Lock.gif" border="0" alt="<fmt:message key="lock" bundle="${resword}"/>" title="<fmt:message key="lock" bundle="${resword}"/>" align="left" hspace="6"></a>
							...
							<c:otherwise>
								<c:if
									test="${currRow.bean.status.deleted && (!study.status.locked)}">
									<td><a
										href="RestoreEventDefinition?action=confirm&id=<c:out value="${currRow.bean.id}"/>"
