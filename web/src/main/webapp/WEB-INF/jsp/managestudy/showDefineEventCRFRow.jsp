...
<jsp:useBean scope="request" id="currRow"
	class="org.akaza.openclinica.web.bean.CRFRow" />
...
<tr valign="top">
	...
	<td class="table_cell"><c:choose>
			<c:when test="${currRow.bean.updatedDate != null}">
				<fmt:formatDate value="${currRow.bean.updatedDate}"
					pattern="${dteFormat}" />
			</c:when>
			<c:otherwise>
          &nbsp;
	...
	<td class="table_cell" align="center"><c:choose>
			<c:when test="${!currRow.bean.selected}">
				<input type="checkbox"
					<c:if test="${not empty tmpCRFIdMap[currRow.bean.id]}">checked="checked" </c:if>
					name="selected<c:out value="${count}"/>" value="yes">
			</c:when>
			<c:otherwise>
				<img src="images/icon_DEcomplete.gif" alt="CRF Selected"
					title="CRF Selected">
			</c:otherwise>
		</c:choose></td>
