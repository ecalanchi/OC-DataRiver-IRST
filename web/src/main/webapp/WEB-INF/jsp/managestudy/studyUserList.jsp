...
<tr id="sidebar_Instructions_open" style="display: all">
	<td class="sidebar_tab"><a
		...
		<div class="sidebar_tab_content">
			...
			<fmt:message key="please_select_users_in_the_following_table"
				bundle="${resword}" />
...
<h1>
	<span class="title_manage"> <c:choose>
		...
		</c:choose> <c:out value="${study.name}" /> <a
		href="javascript:openDocWindow('https://docs.openclinica.com/3.1/manage-users/manage-user-roles#content-title-4354')">
...
<c:choose>
	<c:when test="${study.parentStudyId > 0}">
		...
		<a href="${pageContext.request.contextPath}/ChangeStudy"><fmt:message
				key="that_study" bundle="${resword}" /></a>
	</c:when>
	<c:otherwise>
		<fmt:message key="assign_study_user_note" bundle="${resword}" />
		<a href="${pageContext.request.contextPath}/ChangeStudy"><fmt:message
				key="that_site" bundle="${resword}" /></a>
	</c:otherwise>
</c:choose>
