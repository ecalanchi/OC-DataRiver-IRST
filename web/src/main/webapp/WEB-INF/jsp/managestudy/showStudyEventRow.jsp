...
<jsp:useBean scope="request" id="currRow"
	class="org.akaza.openclinica.web.bean.DisplayStudyEventRow" />
...
<tr>
	<td class="table_cell_left"><c:out
			value="${currRow.bean.studyEvent.studyEventDefinition.name}" /> <c:if
			test="${currRow.bean.studyEvent.studyEventDefinition.repeating}">
            (<c:out value="${currRow.bean.studyEvent.sampleOrdinal}" />)
        </c:if></td>
	<td class="table_cell"><fmt:formatDate
			value="${currRow.bean.studyEvent.dateStarted}" pattern="${dteFormat}" />
	</td>

	<td class="table_cell"><c:out
			value="${currRow.bean.studyEvent.location}" /></td>
	<td class="table_cell" width="20"><c:out
			value="${currRow.bean.studyEvent.subjectEventStatus.name}" /></td>
	<td class="table_cell">
		<table border="0" cellpadding="0" cellspacing="0">
			<tr>
				<td><a
					href="EnterDataForStudyEvent?eventId=<c:out value="${currRow.bean.studyEvent.id}"/>"
					onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"
					onMouseUp="javascript:setImage('bt_View1','images/bt_View.gif');"><img
						name="bt_View1" src="images/bt_View.gif" border="0"
						alt="<fmt:message key="view" bundle="${resword}"/>"
						title="<fmt:message key="view" bundle="${resword}"/>" align="left"
						hspace="6"></a></td>
			</tr>
			<tr>
				<td>&nbsp;</td>
			...
			<tr>
				<td><c:if
						test="${(studySub.status.name != 'removed' && studySub.status.name != 'auto-removed' && !currRow.bean.studyEvent.status.deleted && currRow.bean.studyEvent.editable) && (study.status.available)}">
						<a
							href="UpdateStudyEvent?event_id=<c:out value="${currRow.bean.studyEvent.id}"/>&ss_id=<c:out value="${studySub.id}"/>"
							...
							title="<fmt:message key="edit" bundle="${resword}"/>"
				...
				<td><c:choose>
						<c:when test="${!currRow.bean.studyEvent.status.deleted}">
							<c:if test="${userRole.manageStudy && study.status.available}">
								<a
									href="RemoveStudyEvent?action=confirm&id=<c:out value="${currRow.bean.studyEvent.id}"/>&studySubId=<c:out value="${studySub.id}"/>"
						...
						<c:otherwise>
							<c:if
								test="${(userRole.manageStudy && studySub.status.name != 'removed' && studySub.status.name != 'auto-removed') && (study.status.available)}">
								<a
									href="RestoreStudyEvent?action=confirm&id=<c:out value="${currRow.bean.studyEvent.id}"/>&studySubId=<c:out value="${studySub.id}"/>"
						...
						test="${currRow.bean.studyEvent.subjectEventStatus.scheduled && userRole.manageStudy && study.status.available}">
						<tr>
							<td>&nbsp;</td>
						</tr>
						<tr>
							<td><a
								href="DeleteStudyEvent?action=confirm&id=<c:out value="${currRow.bean.studyEvent.id}"/>&studySubId=<c:out value="${studySub.id}"/>"
								onMouseDown="javascript:setImage('bt_Remove1','images/bt_Remove_d.gif');"
								onMouseUp="javascript:setImage('bt_Remove1','images/bt_Remove.gif');"><img
	...
	<td class="table_cell"><c:choose>
			...
			<c:when
				test="${empty currRow.bean.uncompletedCRFs && empty currRow.bean.displayEventCRFs}">
			...
			<c:otherwise>
				...
				<table border="0" cellpadding="0" cellspacing="0">
					...
					<c:forEach var="dedc" items="${currRow.bean.uncompletedCRFs}">
						...
						<form
							name="startForm<c:out value="${currRow.bean.studyEvent.id}"/><c:out value="${dedc.edc.crf.id}"/>"
							action="InitialDataEntry?<c:out value="${getQuery}"/>"
							method="POST">

							<tr valign="top">

								<td class="table_cell" width="180"><c:out
										value="${dedc.edc.crf.name}" /></td>

								<td class="table_cell" width="100"><c:choose>
									...
									</c:choose> <c:set var="versionCount" value="0" /> <c:forEach
										...
										<c:when test="${dedc.eventCRF.id == 0}">
											...
											<select
												name="versionId<c:out value="${currRow.bean.studyEvent.id}"/><c:out value="${dedc.edc.crf.id}"/>"
												onchange="javascript:changeQuery<c:out value="${currRow.bean.studyEvent.id}"/><c:out value="${dedc.edc.crf.id}"/>();">
								...
								<c:choose>
									...
									<c:when test="${dedc.status.name=='locked'}">
										<td class="table_cell" bgcolor="#F5F5F5" align="center"
											width="20"><img src="images/icon_Locked.gif"
									...
									<c:when
										...
										<c:choose>
											<c:when test="${dedc.eventCRF.id>0}">
												<td class="table_cell" bgcolor="#F5F5F5" align="center"><img
									...
									<c:otherwise>
										<td class="table_cell" bgcolor="#F5F5F5" align="center"
											width="20"><img src="images/icon_Invalid.gif"
									...
									<table cellspacing="0" cellpadding="0" border="0">
										<tr>
											<c:choose>
												...
												<c:when
													test="${dedc.status.name=='locked' || (currRow.bean.studyEvent.subjectEventStatus.signed && !userRole.manageStudy)}">
													<td>&nbsp;</td>
												</c:when>

												<c:when
													test="${studySub.status.name != 'removed' && studySub.status.name != 'auto-removed'}">
													<td><c:if
															test="${study.status.available && !currRow.bean.studyEvent.status.deleted && !userRole.monitor}">

															<a href="#"
																onclick="checkCRFLockedInitial('<c:out value="${dedc.eventCRF.id}"/>', document.startForm<c:out value="${currRow.bean.studyEvent.id}"/><c:out value="${dedc.edc.crf.id}"/>);"
																...
																<img name="bt_EnterData1" src="images/bt_EnterData.gif"
																border="0"
											...
											<td><a
												href="ViewSectionDataEntry?eventDefinitionCRFId=<c:out value="${dedc.edc.id}"/>&crfVersionId=<c:out value="${dedc.edc.defaultVersionId}"/>&tabId=1&studySubjectId=<c:out value="${studySub.id}"/>&ecId=<c:out value="${dedc.eventCRF.id}"/>&exitTo=ViewStudySubject?id=${studySub.id}"
												onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"
											...
											<td><a
												href="javascript:openPrintCRFWindow('rest/clinicaldata/html/print/${study.oid}/${studySub.oid}/${currRow.bean.studyEvent.studyEventDefinition.oid}<c:if test="${currRow.bean.studyEvent.studyEventDefinition.repeating}">[${currRow.bean.studyEvent.sampleOrdinal}]</c:if>/${crfVersionOID}')"
...
&& !(currRow.bean.studyEvent.subjectEventStatus.locked || currRow.bean.studyEvent.subjectEventStatus.skipped)}">
					...
					<c:forEach var="dec" items="${currRow.bean.displayEventCRFs}">
						<tr>
							...
							<td class="table_cell" width="100"><c:out
							...
							<td class="table_cell" bgcolor="#F5F5F5" align="center"
								width="20"><c:choose>
									...
									<c:when
										test="${dec.stage.locked || dec.eventCRF.status.locked || dec.locked}">
							...
							<td class="table_cell" width="80"><c:if
									test="${dec.eventCRF.updatedDate != null}">
									<fmt:formatDate value="${dec.eventCRF.updatedDate}"
										pattern="${dteFormat}" />
									<br>
								</c:if> <c:choose>
		   ...
		   (<c:out value="${dec.eventCRF.updater.name}" />)
							...
							<td class="table_cell" width="140">
								<table border="0" cellpadding="0" cellspacing="0">
									<tr valign="top">
										<td><c:if
												test="${!dec.eventCRF.status.deleted && !dec.eventCRF.status.locked && study.status.available && !currRow.bean.studyEvent.status.deleted && !userRole.monitor}">
												...
												<c:if
													test="${(dec.performAdministrativeEditingPermitted) &&(study.status.available)}">
												...
												<%-- locked status here --%>
												<c:if
													test="${dec.locked || dec.eventCRF.status.locked || dec.stage.locked || currRow.bean.studyEvent.subjectEventStatus.locked}">
	    			&nbsp;
		    	</c:if>
											</c:if></td>
										<td><a
											href="ViewSectionDataEntry?ecId=<c:out value="${dec.eventCRF.id}"/>&eventDefinitionCRFId=<c:out value="${dec.eventDefinitionCRF.id}"/>&tabId=1&studySubjectId=<c:out value="${studySub.id}"/>&exitTo=ViewStudySubject?id=${studySub.id}"
											onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"
											onMouseUp="javascript:setImage('bt_View1','images/bt_View.gif');"><img
												name="bt_View1" src="images/bt_View.gif" border="0"
												alt="<fmt:message key="view" bundle="${resword}"/>"
												...
												align="left" hspace="6"></a></td>
										<td><a
											href="javascript:openPrintCRFWindow('rest/clinicaldata/html/print/${study.oid}/${studySub.oid}/${currRow.bean.studyEvent.studyEventDefinition.oid}<c:if test="${currRow.bean.studyEvent.studyEventDefinition.repeating}">[${currRow.bean.studyEvent.sampleOrdinal}]</c:if>/${dec.eventCRF.crfVersion.oid}')"
											onMouseDown="javascript:setImage('bt_Print1','images/bt_Print_d.gif');"
											onMouseUp="javascript:setImage('bt_Print1','images/bt_Print.gif');"><img
												name="bt_Print1" src="images/bt_Print.gif" border="0"
										...
										<c:choose>
											<c:when test="${!dec.eventCRF.status.deleted}">
												<c:if
													test="${userRole.manageStudy && study.status.available}">
													<td><a
														href="RemoveEventCRF?action=confirm&id=<c:out value="${dec.eventCRF.id}"/>&studySubId=<c:out value="${studySub.id}"/>"
														onMouseDown="javascript:setImage('bt_Remove1','images/bt_Remove_d.gif');"
														onMouseUp="javascript:setImage('bt_Remove1','images/bt_Remove.gif');"><img
															name="bt_Remove1" src="images/bt_Remove.gif" border="0"
															alt="<fmt:message key="remove" bundle="${resword}"/>"
											...
											<c:otherwise>
												<c:if
													test="${(userRole.manageStudy && currRow.bean.studyEvent.status.name!='auto-removed' && dec.eventCRF.status.name != 'auto-removed') && (study.status.available)}">
													<td><a
														href="RestoreEventCRF?action=confirm&id=<c:out value="${dec.eventCRF.id}"/>&studySubId=<c:out value="${studySub.id}"/>"
														onMouseDown="javascript:setImage('bt_Restor3','images/bt_Restore_d.gif');"
														onMouseUp="javascript:setImage('bt_Restore3','images/bt_Restore.gif');"><img
															name="bt_Restore3" src="images/bt_Restore.gif" border="0"
															alt="<fmt:message key="restore" bundle="${resword}"/>"
										...
										<c:if
											...
											<td><a
												href="DeleteEventCRF?action=confirm&ssId=<c:out value="${studySub.id}"/>&ecId=<c:out value="${dec.eventCRF.id}"/>"
