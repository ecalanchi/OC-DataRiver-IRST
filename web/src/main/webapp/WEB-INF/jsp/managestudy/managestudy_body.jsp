...
<body>
	<h1>
		<span class="title_manage"><fmt:message key="manage_study"
	...
	<noscript class="noscript">
		<ul>
			...
			<li><a href="ListDiscNotesSubjectServlet?module=manage">Notes
					& Discrepancies</a></li>
	...
	<h2>
		<fmt:message key="recent_activity" bundle="${restext}" />
		${studyIdentifier}
	</h2>
		...
		<tr>
			<td valign="top" width="330" style="padding-right: 20px">
				...
				<div class="table_title_Manage">
					<fmt:message key="subjects" bundle="${resworkflow}" />
					<c:choose>
						<c:when test="${subsCount>0}">
                        (<c:out value="${subsCount}" />
							<fmt:message key="of" bundle="${restext}" />
							<c:out value="${allSubsCount}" />
							<fmt:message key="shown" bundle="${restext}" />)
                    </c:when>
						<c:otherwise>
							<fmt:message key="currently_no_subjects" bundle="${restext}" />
						</c:otherwise>
					</c:choose>
				</div> <!-- These DIVs define shaded box borders -->
				<div class="box_T">
					<div class="box_L">
						<div class="box_R">
							<div class="box_B">
													...
													<table border="0" cellpadding="0" cellspacing="0"
														...
														<tr valign="top">
															...
															<td class="table_header_row"><fmt:message
																	key="date_updated" bundle="${resword}" /></td>
															<td class="table_header_row"><fmt:message
															...
															<tr valign="top">
																...
																<td class="table_cell"><c:choose>
																		<c:when test="${sub.updatedDate != null}">
																			<fmt:formatDate value="${sub.updatedDate}"
																				pattern="${dteFormat}" />
																		</c:when>
																		<c:otherwise>
																			<fmt:formatDate value="${sub.createdDate}"
																				pattern="${dteFormat}" />
																		</c:otherwise>
																	</c:choose></td>
																<td class="table_cell"><c:out
																		value="${sub.status.name}" /></td>
															</tr>
														</c:forEach>
														<tr valign="top">
															<td class="table_cell" align="right" colspan="3"><c:if
																	test="${subsCount>0}">
																	<a href="ListStudySubjects"><fmt:message
																			key="show_all" bundle="${resword}" /></a> |
                                </c:if> <a href="AddNewSubject"><fmt:message
																		key="add_new" bundle="${resword}" /></a></td>
														</tr>
													</table>
			...
			<td valign="top" width="330" style="padding-right: 20px">
				...
				<div class="table_title_Manage">
					<fmt:message key="users" bundle="${resword}" />
					<c:choose>
						<c:when test="${usersCount>0}">
                        (<c:out value="${usersCount}" />
							<fmt:message key="of" bundle="${restext}" />
							<c:out value="${allUsersCount}" />
							<fmt:message key="shown" bundle="${restext}" />)
                    </c:when>
						<c:otherwise>
												...
												<div class="tablebox_center">
													<table border="0" cellpadding="0" cellspacing="0"
														...
														<tr valign="top">
															<td class="table_cell" align="right" colspan="3"><c:if
																	test="${usersCount>0}">
																	<a href="ListStudyUser"><fmt:message key="show_all"
																			bundle="${resword}" /></a> | </c:if><a href="AssignUserToStudy"><fmt:message
																		key="add_new" bundle="${resword}" /></a></td>
														</tr>
													</table>
	...
	<table border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td valign="top" width="330" style="padding-right: 20px">
				...
				<div class="table_title_Manage">
					<fmt:message key="sites" bundle="${resword}" />
					<c:choose>
						<c:when test="${sitesCount>0}">
                (<c:out value="${sitesCount}" />
							<fmt:message key="of" bundle="${restext}" />
							<c:out value="${allSitesCount}" />
							<fmt:message key="shown" bundle="${restext}" />)
            </c:when>
						<c:otherwise>
													...
													<table border="0" cellpadding="0" cellspacing="0"
															...
															<c:when test="${study.parentStudyId>0}">
																<tr valign="top">
																	<td class="table_cell" colspan="3"><fmt:message
																			key="site_itself_cannot_have_sites"
																			bundle="${restext}" /></td>
																</tr>
																	...
																	<tr valign="top">
																		...
																		<td class="table_cell"><c:choose>
																				<c:when test="${site.updatedDate != null}">
																					<fmt:formatDate value="${site.updatedDate}"
																						pattern="${dteFormat}" />
																				</c:when>
																				<c:otherwise>
																					<fmt:formatDate value="${site.createdDate}"
																						pattern="${dteFormat}" />
																				</c:otherwise>
																			</c:choose></td>
																		<td class="table_cell"><c:out
														...
														<c:if test="${study.parentStudyId==0}">
															<tr valign="top">
																<td class="table_cell" align="right" colspan="3"><c:if
																		test="${sitesCount>0}">
																		<a href="ListSite"><fmt:message key="show_all"
																				bundle="${resword}" /></a> |
                            </c:if> <a href="CreateSubStudy"><fmt:message
																			key="add_new" bundle="${resword}" /></a></td>
															</tr>
														</c:if>
													</table>
			...
			<td valign="top" width="330" style="padding-right: 20px">
				...
				<div class="table_title_Manage">
					<fmt:message key="study_event_definitions" bundle="${resworkflow}" />
					<c:choose>
						<c:when test="${sedsCount>0}">
                (<c:out value="${sedsCount}" />
							<fmt:message key="of" bundle="${restext}" />
							<c:out value="${allSedsCount}" />
							<fmt:message key="shown" bundle="${restext}" />)
            </c:when>
						<c:otherwise>
													...
													<table border="0" cellpadding="0" cellspacing="0"
														...
														<c:choose>
															<c:when test="${study.parentStudyId>0}">
																<tr valign="top">
																	<td class="table_cell" colspan="3"><fmt:message
																			key="site_itself_cannot_have_definitions"
																			bundle="${restext}" /></td>
																</tr>
															</c:when>
															<c:otherwise>
																	...
																	<tr valign="top">
																		...
																		<td class="table_cell"><c:choose>
																				<c:when test="${def.updatedDate != null}">
																					<fmt:formatDate value="${def.updatedDate}"
																						pattern="${dteFormat}" />
																				</c:when>
																				<c:otherwise>
																					<fmt:formatDate value="${def.createdDate}"
																						pattern="${dteFormat}" />
																				</c:otherwise>
																			</c:choose></td>
																		<td class="table_cell"><c:out
																				value="${def.status.name}" /></td>
																	</tr>
																</c:forEach>
															</c:otherwise>
														</c:choose>
															...
															<tr valign="top">
																<td class="table_cell" align="right" colspan="3"><c:if
																		...
																		<a href="ListEventDefinition"><fmt:message
																				key="show_all" bundle="${resword}" /></a> |
                            </c:if> <a href="DefineStudyEvent"><fmt:message
																			key="add_new" bundle="${resword}" /></a></td>
															</tr>
														</c:if>
		...
		<tr>
			<!-- extra row added by tbh, to support direct link to study audit logs -->
			<td colspan="2">
