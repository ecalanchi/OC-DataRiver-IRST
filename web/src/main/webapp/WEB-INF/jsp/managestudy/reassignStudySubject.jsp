...
<h1>
	<span class="title_manage"> <fmt:message
			key="reassign_study_subject" bundle="${resworkflow}" />
	</span>
</h1>
<p>
	<fmt:message key="you_choose_to_reassign_the_following_subject2"
		bundle="${restext}" />
</p>
<form action="ReassignStudySubject" method="post">
	...
	<p>
		<strong><fmt:message
				key="please_choose_a_study_in_the_following_list2"
				bundle="${restext}" /></strong>
	</P>
		...
		<tr>
			<td><c:choose>
					<c:when test="${displayStudy.parent.id==studySub.studyId }">
						...
						<input type="radio" checked name="studyId"
							value="<c:out value="${displayStudy.parent.id}"/>">
						<b><c:out value="${displayStudy.parent.name}" /> <fmt:message
								key="currently_in" bundle="${restext}" /></b>

					</c:when>
					<c:otherwise>
						<c:if test="${displayStudy.status.available}">
							<input type="radio" name="studyId"
								value="<c:out value="${displayStudy.parent.id}"/>">
							<b><c:out value="${displayStudy.parent.name}" /> </b>
						</c:if>
						<c:if test="${displayStudy.status.locked}">
							<input type="radio" disabled="true" name="studyId"
								value="<c:out value="${displayStudy.parent.id}"/>">
							<b><c:out value="${displayStudy.parent.name}" /> </b>
						</c:if>
					</c:otherwise>
	...
	<p>
		<input type="submit" name="Submit"
			value="<fmt:message key="reassign_subject" bundle="${resword}"/>"
