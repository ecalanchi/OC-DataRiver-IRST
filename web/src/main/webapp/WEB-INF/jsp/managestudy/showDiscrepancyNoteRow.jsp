...
<jsp:useBean scope="request" id="currRow"
	class="org.akaza.openclinica.web.bean.DiscrepancyNoteRow" />
<%--
...
<tr valign="top">
	<td class="table_cell_left"><c:out
			value="${currRow.bean.subjectName}" /></td>
	...
	<td class="table_cell"><c:if
			test="${currRow.bean.eventStart != null}">
			<fmt:formatDate value="${currRow.bean.eventStart}"
				pattern="${dteFormat}" />
		</c:if>&nbsp;</td>
	...
	<td class="table_cell"><c:choose>
			<c:when test="${currRow.bean.entityType=='itemData'}">
				<a
					href="javascript: openDocWindow('ViewItemDetail?itemId=<c:out value="${currRow.bean.itemId}"/>')"><c:out
						value="${currRow.bean.entityName}" /></a>&nbsp;
	  </c:when>
			<c:otherwise>
	...
	<td class="table_cell" width="400"><c:out
			value="${currRow.bean.detailedNotes}" />&nbsp;</td>
	<td class="table_cell" align="right"><c:choose>
	...
	<td class="table_cell" align="right"><c:choose>
			<c:when test="${currRow.bean.assignedUserId > 0}">
				<c:out value="${currRow.bean.assignedUser.firstName}" />
				<c:out value="${currRow.bean.assignedUser.lastName}" /> (<c:out
					value="${currRow.bean.assignedUser.name}" />)
			</c:when>
			<c:otherwise></c:otherwise>
		</c:choose></td>
	<td class="table_cell" style="display: none"
		id="Groups_0_10_<c:out value="${eblRowCount+1}"/>"><c:out
			value="${currRow.status.name}" /></td>
	<td class="table_cell" style="display: none"
		id="Groups_0_11_<c:out value="${eblRowCount+1}"/>"><c:out
			value="${currRow.type.name}" /></td>

	<td class="table_cell" style="display: none"
		id="Groups_0_12_<c:out value="${eblRowCount+1}"/>"><c:out
			value="${currRow.bean.entityType}" />&nbsp;</td>

	<td class="table_cell" style="display: none"
		id="Groups_0_13_<c:out value="${eblRowCount+1}"/>"><c:out
	...
	<td class="table_cell">
		<table border="0" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td><a href="#"
					onclick="openVNoteWindow('ViewNote?id=<c:out value="${currRow.bean.id}"/>')"
					onMouseDown="javascript:setImage('bt_View1','images/bt_View_d.gif');"
					onMouseUp="javascript:setImage('bt_View1','images/bt_View.gif');"><img
						name="bt_View1" src="images/bt_View.gif" border="0"
						alt="<fmt:message key="view_discrepancy_note" bundle="${resword}"/>"
						title="<fmt:message key="view_discrepancy_note" bundle="${resword}"/>"
						align="left" hspace="6"></a></td>
				<c:if
					test="${((!currRow.status.closed && !currRow.status.notApplicable))&&(!study.status.locked)}">
					<td><c:choose>
							<c:when test="${currRow.bean.entityType != 'eventCrf'}">
								<a
									href="ResolveDiscrepancy?noteId=<c:out value="${currRow.bean.id}"/>"
									...
									name="bt_Resolve1" src="images/bt_Reassign.gif" border="0"
									alt="<fmt:message key="resolve_discrepancy_note" bundle="${resword}"/>"
									title="<fmt:message key="resolve_discrepancy_note" bundle="${resword}"/>"
