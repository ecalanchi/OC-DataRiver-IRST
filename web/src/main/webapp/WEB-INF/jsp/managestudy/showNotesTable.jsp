...
<c:set var="firstPageQuery"
	value="${table.baseGetQuery}&module=${module}&id=${param.id}&viewForOne=${param.viewForOne}&ebl_page=1&ebl_sortColumnInd=${table.sortingColumnInd}&ebl_sortAscending=${ascending}&ebl_filtered=${filtered}&ebl_filterKeyword=${table.keywordFilter}&ebl_paginated=1" />
<c:set var="prevPageQuery"
	value="${table.baseGetQuery}&module=${module}&id=${param.id}&viewForOne=${param.viewForOne}&ebl_page=${table.currPageNumber - 1}&ebl_sortColumnInd=${table.sortingColumnInd}&ebl_sortAscending=${ascending}&ebl_filtered=${filtered}&ebl_filterKeyword=${table.keywordFilter}&ebl_paginated=1" />
<c:set var="nextPageQuery"
	value="${table.baseGetQuery}&module=${module}&id=${param.id}&viewForOne=${param.viewForOne}&ebl_page=${table.currPageNumber + 1}&ebl_sortColumnInd=${table.sortingColumnInd}&ebl_sortAscending=${ascending}&ebl_filtered=${filtered}&ebl_filterKeyword=${table.keywordFilter}&ebl_paginated=1" />
<c:set var="lastPageQuery"
	value="${table.baseGetQuery}&module=${module}&id=${param.id}&viewForOne=${param.viewForOne}&ebl_page=${table.totalPageNumbers}&ebl_sortColumnInd=${table.sortingColumnInd}&ebl_sortAscending=${ascending}&ebl_filtered=${filtered}&ebl_filterKeyword=${table.keywordFilter}&ebl_paginated=1" />
<c:set var="viewAllQuery"
	value="${table.baseGetQuery}&module=${module}&id=${param.id}&viewForOne=${param.viewForOne}&ebl_page=${1}&ebl_sortColumnInd=${table.sortingColumnInd}&ebl_sortAscending=${ascending}&ebl_filtered=0&ebl_filterKeyword=&ebl_paginated=0" />
<c:set var="doNotPaginateQuery"
	value="${table.baseGetQuery}&module=${module}&id=${param.id}&viewForOne=${param.viewForOne}&ebl_page=1&ebl_sortColumnInd=${table.sortingColumnInd}&ebl_sortAscending=${ascending}&ebl_filtered=${filtered}&ebl_filterKeyword=${table.keywordFilter}&ebl_paginated=0" />
<c:set var="paginateQuery"
	value="${table.baseGetQuery}&module=${module}&id=${param.id}&viewForOne=${param.viewForOne}&ebl_page=1&ebl_sortColumnInd=${table.sortingColumnInd}&ebl_sortAscending=${ascending}&ebl_filtered=${filtered}&ebl_filterKeyword=${table.keywordFilter}&ebl_paginated=1" />
<c:set var="removeFilterQuery"
	value="${table.baseGetQuery}&module=${module}&id=${param.id}&viewForOne=${param.viewForOne}&ebl_page=${table.currPageNumber}&ebl_sortColumnInd=${table.sortingColumnInd}&ebl_sortAscending=${ascending}&ebl_filtered=0&ebl_filterKeyword=&ebl_paginated=${paginated}" />
				...
				<div class="box_L">
					<div class="box_R">
													...
													<tr>
																...
																<tr>
																	...
																	<td valign="top" class="table_tools"><c:if
																			...
																			<a href="<c:out value="${nextPageQuery}"/>"><img
																				src="images/arrow_next.gif" border="0" alt="next"
																				title="next"></a>
																			<a href="<c:out value="${lastPageQuery}"/>"><img
																				src="images/arrow_last.gif" border="0"
																				alt="last page" title="last page"></a>
																		</c:if></td>
																</tr>
															</table>
														</td>
														...
														<td width="55%" valign="top" align="center"
															class="table_actions"><jsp:include
																page="../include/showSubmitted.jsp" /> <c:forEach
																var="postArg" items="${table.postArgs}">
																<input type="hidden"
                        ...
                        as in type=1, or the table.keywordFilter --%>
																			<c:set var="keyword_filter"
																				value="${table.keywordFilter}" />

																			<c:if test="${fn:length(keyword_filter) == 0}">
																				<c:if test="${fn:length(param.discNoteType) > 0}">
																					<c:choose>
																						<c:when test="${param.discNoteType == 1}">
																							<c:set var="keyword_filter">
																								<fmt:message key="Failed_Validation_Check"
																									bundle="${resterm}" />
																							</c:set>
																						</c:when>
																						<c:when test="${param.discNoteType == 2}">
																							<c:set var="keyword_filter">
																								<fmt:message key="Annotation"
																									bundle="${resterm}" />
																							</c:set>
																						</c:when>
																						<c:when test="${param.discNoteType == 3}">
																							<c:set var="keyword_filter">
																								<fmt:message key="query" bundle="${resterm}" />
																							</c:set>
																						</c:when>
																						<c:when test="${param.discNoteType == 4}">
																							<c:set var="keyword_filter">
																								<fmt:message key="reason_for_change"
																									bundle="${resterm}" />
																							</c:set>
																						</c:when>
																					</c:choose>
																			...
																			<input id="ebl_filterKeyword"
																				name="ebl_filterKeyword" type="text"
																				class="formfieldM"
																				value="<c:out value="${keyword_filter}"/>" />
																		</div>
																	</td>
                        ...
                        <select class="formfieldM" name="resolutionStatus" onchange="document.location='ViewNotes?resolutionStatus='+this.value;">
                            ...
                            <option value="4" <c:choose><c:when test="${fn:length(param.resolutionStatus) < 2 && param.resolutionStatus==4}">selected="selected"</c:when></c:choose>><fmt:message key="Closed" bundle="${resterm}"/>
																	...
																	<td class="table_tools" nowrap><b> <!-- These Numbers represent the tab/table number, number of columns needs to hide, and number of data rows in each table -->
																			<a href="javascript:HideGroups(0,14,10);">
																				<div id="HideGroups" style="display: none">
																					&nbsp;&nbsp;
																					<fmt:message key="show_less" bundle="${resword}" />
																				</div>
																				<div id="ShowGroups">
																					&nbsp;&nbsp;
																	...
																	<td class="table_tools" nowrap>
																		<div id="clear_search">
																			&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)"
																				onclick="document.getElementById('ebl_filterKeyword').value=''"><b><fmt:message
																						key="clear_search_keywords" bundle="${resword}" /></b></a>
																		</div>
																	</td>
															</table></td>
