				...
				<c:choose>
					<c:when test="${line.title=='Study Event'}">
						...
						<c:if test="${eventCount==0}">
							<table class="infoside_table" border="0" cellpadding="0"
		          ...
		          javascript:setImage('ExpandGroup<c:out value="${eventCount}"/>','images/bt_Collapse.gif');"><img
		            ...
		            javascript:setImage('ExpandGroup<c:out value="${eventCount}"/>','images/bt_Collapse.gif');"><b><c:out
										...
										<c:otherwise>
											<td valign="top" class="leftmenu"><b><span
													class='alert'><c:out value="${line.info}"
															escapeXml="false" /></span></b></td>
										</c:otherwise>
									</c:choose>
												...
												<tr>
													...
													<c:choose>
														<c:when
															...
															<td valign="top" class="leftmenu"><c:out
																	value="${line.title}" escapeXml="false" />&nbsp;<c:out
														...
														<c:otherwise>
															<c:choose>
																<c:when test="${first == 0}">
																	<td valign="top" class="leftmenu"><c:out
																			value="<img src='images/icon_Invalid.gif' alt='Invalid'>"
																			escapeXml="false" />&nbsp;<c:out value="${line.info}"
																			escapeXml="false" /></td>
																</c:when>
																<c:otherwise>
