...
<c:choose>
	<c:when test="${userBean != null && userBean.id>0}">
			...
			<td class="sidebar_tab"><a
				...
				<div class="sidebar_tab_content">
					...
					<span style="color: #789EC5"> <%-- begin standard study info --%>
						<%--	<c:if test="${panel.studyInfoShown}">--%> <c:choose>
	 ...
	 <a href="ViewStudy?id=<c:out value="${study.id}"/>&viewFull=yes">
						...
						</c:choose> <c:out value="${study.name}" /></a> <br>
					<br> <b><fmt:message key="subject" bundle="${resword}" />:</b>&nbsp;
						<c:out value="${studySubject.label}" /> <br>
					<br> <b><fmt:message key="study_event" bundle="${resword}" /></b>:
						&nbsp; <c:choose>
							<c:when test="${toc != null}">
								<a
									href="EnterDataForStudyEvent?eventId=<c:out value="${toc.studyEvent.id}"/>"><c:out
										value="${toc.studyEventDefinition.name}" /></a>
							</c:when>
							<c:otherwise>
								<c:out value="${studyEvent.studyEventDefinition.name}" />
							</c:otherwise>
						</c:choose> <br>
					<br> <b><fmt:message key="location" bundle="${resword}" /></b>:
						<c:choose>
							<c:when test="${toc != null}">
					...
					<br> <b><fmt:message key="start_date1" bundle="${resword}" /></b>:
						<c:choose>
							<c:when test="${toc != null}">
								<fmt:formatDate value="${toc.studyEvent.dateStarted}"
									pattern="${dteFormat}" />
							</c:when>
							<c:otherwise>
					...
					<br> <c:if test="${toc != null}">
							<b><fmt:message key="CRF" bundle="${resword}" /></b>: 
      <a href="ViewCRF?crfId=<c:out value="${toc.crf.id}"/>"> <c:out
									value="${toc.crf.name}" /> <c:out
									value="${toc.crfVersion.name}" />
							</a>
							<br>
							<br>
						</c:if> <%-- </c:if>--%> <script language="JavaScript">
       <!--
       ...
       //-->
		...
		<c:if test="${panel.submitDataModule}">
			<tr id="sidebar_StudyEvents_open">
				<td class="sidebar_tab"><a
					href="javascript:leftnavExpand('sidebar_StudyEvents_open'); leftnavExpand('sidebar_StudyEvents_closed');"><img
						src="images/sidebar_collapse.gif" border="0" align="right"
						hspace="10"></a> <b><fmt:message key="study_events"
							bundle="${resword}" /></b>
