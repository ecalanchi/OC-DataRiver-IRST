	...
	<c:when test='${(auditTable != null)}'>
		...
		<c:if test="${auditTable == 'Subject'}">
			...
			<a href="ViewSubject?id=<c:out value="${entityId}"/>"
				...
				alt="<fmt:message key="view_subject" bundle="${resworkflow}"/>"
				title="<fmt:message key="view_subject" bundle="${resworkflow}"/>"
		...
		<c:if test="${auditTable == 'User Account'}">
			...
			<c:if test="${userBean.sysAdmin}">
				<a href="ViewUserAccount?userId=<c:out value="${entityId}"/>"
					...
					onMouseUp="javascript:setImage('bt_View1','images/bt_View.gif');"><img
					name="bt_View1" src="images/bt_View.gif" border="0"
					alt="<fmt:message key="view_user_account" bundle="${resworkflow}"/>"
					title="<fmt:message key="view_user_account" bundle="${resworkflow}"/>"
					align="left" hspace="6"></a>
			</c:if>
		</c:if>
	</c:when>
