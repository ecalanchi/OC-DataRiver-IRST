...
<td valign="top">
	<div class="formfieldS_BG">
		<input type="text" name="<c:out value="${dateFieldName}"/>"
			value="<c:out value="${date}" />" class="formfieldS"
			id="${prefix}dateField" />
	</div>
</td>
<td valign="top"><a href="#"> <img
		...
		border="0" id="${prefix}dateTrigger" /> <script
			...
			<c:forEach var="currHour" begin="0" end="23" step="1">
				<c:choose>
					<c:when test="${hour == currHour}">
						<option value="<c:out value="${currHour}"/>" selected><c:out
								value="${currHour}" /></option>
					</c:when>
					<c:otherwise>
						<option value="<c:out value="${currHour}"/>"><c:out
								value="${currHour}" /></option>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:forEach var="currMinute" begin="0" end="59" step="1">
				<c:choose>
					<c:when test="${minute == currMinute}">
						<option value="<c:out value="${currMinute}"/>" selected><c:out
								value="${currMinute}" /></option>
					</c:when>
					<c:otherwise>
						<option value="<c:out value="${currMinute}"/>"><c:out
								value="${currMinute}" /></option>
					</c:otherwise>
				</c:choose>
			</c:forEach>
...
<td valign="top">
	...
	</div> <% } else { %> <input type="hidden"
	name="<c:out value="${halfFieldName}"/>" value="" /> <% } %>
</td>