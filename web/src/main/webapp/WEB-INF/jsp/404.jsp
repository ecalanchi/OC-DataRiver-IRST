...
<c:choose>
	<c:when
		test="${userBean != null && userRole != null && userRole.role.name != 'invalid' && passwordExpired == 'no'}">
	...
	<c:otherwise>
		...
		<table border="0" cellpadding=0 " cellspacing="0">
			<tr>
				...
				<b><a href="j_spring_security_logout"><fmt:message
								key="logout" bundle="${restext}" /></a></b></br></td>
				<td class="content" valign="top"></c:otherwise> </c:choose>

					<h1>
						<span class="title_manage"><fmt:message
								key="404_error_msg_header" bundle="${resword}" /></span>
					</h1> <c:if test="${! empty formMessages}">
    ...
    improve the style as well -->
      ...
      error messages look like:
       ...
       <strong><label for="formElementName">formElementName</label></strong>.<br/>-->
					...
					</c:if> <font class="bodytext"> <fmt:message
							key="404_error_msg_body" bundle="${resword}" />
